...
export class QwenLogger {
    ...
    enqueueLogEvent(event) {
        try {
            ...
            if (wasAtCapacity && this.config?.getDebugMode()) {
                console.debug(`QwenLogger: Dropped old event to prevent memory leak (queue size: ${this.events.size})`);
            }
        }
        catch (error) {
    ...
    async logStartSessionEvent(event) {
        ...
        await this.flushToRum().catch((error) => {
            if (this.config?.getDebugMode()) {
                console.debug('Error flushing pending events before session start:', error);
            }
        });
        ...
        const applicationEvent = this.createViewEvent('session', 'session_start', {
            properties: {
                ...
                telemetry_log_user_prompts_enabled: event.telemetry_log_user_prompts_enabled,
                skills: event.skills,
    ...
    requeueFailedEvents(eventsToSend) {
        ...
        if (eventsToSend.length > MAX_RETRY_EVENTS && this.config?.getDebugMode()) {
            console.warn(`QwenLogger: Dropping ${eventsToSend.length - MAX_RETRY_EVENTS} events due to retry queue limit. Total events: ${eventsToSend.length}, keeping: ${MAX_RETRY_EVENTS}`);
        }
        ...
        if (numEventsToRequeue === 0) {
            if (this.config?.getDebugMode()) {
                console.debug(`QwenLogger: No events re-queued (queue size: ${this.events.size})`);
            }
            return;
        }
        ...
        if (this.config?.getDebugMode()) {
            console.debug(`QwenLogger: Re-queued ${numEventsToRequeue} events for retry (queue size: ${this.events.size})`);
        }
    }
}
