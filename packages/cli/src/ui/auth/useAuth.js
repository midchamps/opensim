 ...
 */
import { AuthEvent, AuthType, clearCachedCredentialFile, getErrorMessage, logAuth, } from '@opengame/opengame-core';
import { useCallback, useEffect, useState } from 'react';
import { useQwenAuth } from '../hooks/useQwenAuth.js';
import { AuthState, MessageType } from '../types.js';
import { t } from '../../i18n/index.js';
export const useAuthCommand = (settings, config, addItem) => {
    const unAuthenticated = settings.merged.security?.auth?.selectedType === undefined;
    ...
    const handleAuthSuccess = useCallback(async (authType, scope, credentials) => {
        try {
    ...
    const performAuth = useCallback(async (authType, scope, credentials) => {
        try {
    ...
    const handleAuthSelect = useCallback(async (authType, scope, credentials) => {
        if (!authType) {
            setIsAuthDialogOpen(false);
            setAuthError(null);
            return;
    ...
    useEffect(() => {
        ...
        if (defaultAuthType &&
            ...
            onAuthError(t('Invalid QWEN_DEFAULT_AUTH_TYPE value: "{{value}}". Valid values are: {{validValues}}', {
