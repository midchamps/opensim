...
export class SubagentManager {
     ...
     */
    async updateSubagent(name, updates, level) {
     ...
     */
    async deleteSubagent(name, level) {
    ...
    parseSubagentContent(content, filePath, level) {
        try {
            ...
            const modelConfig = frontmatter['modelConfig'];
     ...
     */
    async createSubagentScope(config, runtimeContext, options) {
        try {
            const runtimeConfig = this.convertToRuntimeConfig(config);
            return await SubAgentScope.create(config.name, runtimeContext, runtimeConfig.promptConfig, runtimeConfig.modelConfig, runtimeConfig.runConfig, runtimeConfig.toolConfig, options?.eventEmitter, options?.hooks);
        }
        catch (error) {
            if (error instanceof Error) {
                throw new SubagentError(`Failed to create SubAgentScope: ${error.message}`, SubagentErrorCode.INVALID_CONFIG, config.name);
            }
            throw error;
        }
    }
    ...
    transformToToolNames(tools) {
        ...
        for (const toolIdentifier of tools) {
            ...
            console.warn(`Tool "${toolIdentifier}" not found in tool registry, preserving as-is`);
        }
        return result;
    }
    /**
    ...
    getSubagentPath(name, level) {
        ...
        const baseDir = level === 'project'
            ? path.join(this.config.getProjectRoot(), QWEN_CONFIG_DIR, AGENT_CONFIG_DIR)
