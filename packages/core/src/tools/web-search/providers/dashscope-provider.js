...
export class DashScopeProvider extends BaseWebSearchProvider {
    ...
    async getAuthConfig() {
        ...
        if (!credentials?.resource_url) {
            throw new Error('No resource_url found in credentials. Please authenticate using OAuth');
        }
    ...
    async performSearch(query, signal) {
        ...
        if (!accessToken) {
            throw new Error('No access token available. Please authenticate using OAuth');
        }
        const requestBody = {
            uq: query,
            page: 1,
...
//# sourceMappingURL=dashscope-provider.js.map