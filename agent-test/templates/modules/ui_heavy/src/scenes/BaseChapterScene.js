...
// BASE CLASS
// ============================================================================
export class BaseChapterScene extends Phaser.Scene {
    ...
    createDialogueUI() {
        ...
        this.choicePanel.on('selected', (index) => {
     ...
     */
    showDialogueText(speaker, text, expression) {
     ...
     */
    handleCharacterEnter(characterId, config, position) {
     ...
     * @param characterId - The character's registered ID
     */
    handleCharacterExit(characterId) {
        const portrait = this.activeCharacters.get(characterId);
        if (portrait) {
            portrait.exit(() => {
    ...
    onDialogueEvent(action, data) { }
    /** HOOK: Called when the player selects a choice option. */
    onChoiceMade(choiceId, option) { }
    /**
     ...
     */
    onCharacterEnter(characterId, position) { }
    /** HOOK: Called AFTER a character exits the scene. */
    onCharacterExit(characterId) { }
    ...
    resolveChoice(optionIndex) {
        ...
        const visibleOptions = (entry.options ?? []).filter((opt) => !opt.condition || opt.condition());
    ...
    showFloatingText(text, x, y, style) {
        const defaultStyle = {
            fontSize: '24px',
            color: '#FFD700',
            fontFamily: 'Arial',
    ...
    processCurrentEntry() {
        ...
        switch (entry.type) {
            case 'text':
                ...
                this.showDialogueText(entry.speaker ?? '', entry.text ?? '', entry.expression);
                break;
            case 'choice': {
...
//# sourceMappingURL=BaseChapterScene.js.map