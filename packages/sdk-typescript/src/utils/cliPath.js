...
export function findNativeCliPath() {
    const bundledCli = getBundledCliPath();
    if (bundledCli) {
        return bundledCli;
    }
    throw new Error('Bundled OpenGame CLI not found. The CLI should be included in the SDK package.\n' +
...
function validateRuntimeAvailability(runtime) {
...
function validateFileExtensionForRuntime(filePath, runtime) {
 ...
 */
export function parseExecutableSpec(executableSpec) {
    if (executableSpec === '' ||
    ...
    if (runtimeMatch) {
        ...
        if (runtime && supportedRuntimes.includes(runtime)) {
            ...
            if (!validateRuntimeAvailability(runtime)) {
                throw new Error(`Runtime '${runtime}' is not available on this system. Please install it first.`);
            }
            const resolvedPath = path.resolve(filePath);
            if (!fs.existsSync(resolvedPath)) {
                throw new Error(`Executable file not found at '${resolvedPath}' for runtime '${runtime}'. ` +
                    'Please check the file path and ensure the file exists.');
            }
            if (!validateFileExtensionForRuntime(resolvedPath, runtime)) {
                const ext = path.extname(resolvedPath);
                throw new Error(`File extension '${ext}' is not compatible with runtime '${runtime}'. ` +
    ...
    const isCommandName = !executableSpec.includes('/') && !executableSpec.includes('\\');
    if (isCommandName) {
        // It's a command name like 'opengame' - validate it's a reasonable command name
        if (!executableSpec || executableSpec.trim() === '') {
            throw new Error('Command name cannot be empty');
        }
        // Basic validation for command names
        if (!/^[a-zA-Z0-9._-]+$/.test(executableSpec)) {
            throw new Error(`Invalid command name '${executableSpec}'. Command names should only contain letters, numbers, dots, hyphens, and underscores.`);
        }
        return {
    ...
    if (!fs.existsSync(resolvedPath)) {
        throw new Error(`Executable file not found at '${resolvedPath}'. ` +
            'Please check the file path and ensure the file exists. ' +
            'You can also:\n' +
            '  • Set QWEN_CODE_CLI_PATH environment variable\n' +
            '  • Install OpenGame globally: npm install -g @opengame/opengame\n' +
    ...
    if (!stats.isFile()) {
        throw new Error(`Path '${resolvedPath}' exists but is not a file. Please provide a path to an executable file.`);
    }
    return {
        executablePath: resolvedPath,
        isExplicitRuntime: false,
...
function getExpectedExtensions(runtime) {
    switch (runtime) {
        case 'node':
        case 'bun':
            return ['.js', '.mjs', '.cjs'];
...
function detectRuntimeFromExtension(filePath) {
    const ext = path.extname(filePath).toLowerCase();
    ...
    if (['.ts', '.tsx'].includes(ext)) {
        ...
        throw new Error(`TypeScript file '${filePath}' requires 'tsx' runtime, but it's not available. ` +
            'Please install tsx: npm install -g tsx, or use explicit runtime: tsx:/path/to/file.ts');
    }
    // Native executable or unknown extension
    return undefined;
}
export function prepareSpawnInfo(executableSpec) {
    ...
    if (detectedRuntime) {
        const runtimeCommand = detectedRuntime === 'node' ? process.execPath : detectedRuntime;
        return {
            command: runtimeCommand,
            args: [executablePath],
...
//# sourceMappingURL=cliPath.js.map