...
export class MissingProviderConfigError extends Error {
    modality;
    hint;
    constructor(modality, hint) {
        super(`OpenGame is missing API configuration for the "${modality}" modality.\n\n${hint}\n\nSee docs/users/configuration/api-keys.md for the full list of supported variables.`);
...
function envName(modality, suffix) {
...
const LEGACY_BASEURL_FALLBACKS = {
 ...
 */
export function resolveProviderConfig(modality, fromConfig, env = process.env) {
    ...
    else if (settingsBlock?.provider &&
    ...
    if (!provider) {
        throw new MissingProviderConfigError(modality, `Set ${envName(modality, 'PROVIDER')} to one of "tongyi", "doubao", "openai-compat", or "gemini", ` +
            `or add an "openGame.providers.${modality}.provider" entry to your settings.json.`);
    }
    // -- 2. API key -----------------------------------------------------
    const apiKey = env[envName(modality, 'API_KEY')] ??
        settingsBlock?.apiKey ??
        firstEnv(env, LEGACY_KEY_FALLBACKS[modality]) ??
        '';
    if (!apiKey) {
        throw new MissingProviderConfigError(modality, `No API key found for "${modality}". Set ${envName(modality, 'API_KEY')} ` +
            `or add "openGame.providers.${modality}.apiKey" to your settings.json.`);
    }
    // -- 3. Base URL + default model -----------------------------------
    const defaults = provider === 'doubao'
        ...
        : provider === 'tongyi'
            ...
            : provider === 'gemini'
                ...
                : undefined;
    const baseUrl = env[envName(modality, 'BASE_URL')] ??
    ...
    if (!baseUrl) {
        ...
        throw new MissingProviderConfigError(modality, `No base URL configured for "${modality}". Set ${envName(modality, 'BASE_URL')} ` +
            `(e.g. https://api.openai.com/v1) or add ` +
            `"openGame.providers.${modality}.baseUrl" to your settings.json.`);
    }
    const model = env[envName(modality, 'MODEL')] ??
        settingsBlock?.model ??
        firstEnv(env, LEGACY_MODEL_FALLBACKS[modality]) ??
        defaults?.model;
    if (!model) {
        throw new MissingProviderConfigError(modality, `No model name configured for "${modality}". Set ${envName(modality, 'MODEL')} ` +
 ...
 */
export function tryResolveProviderConfig(modality, fromConfig, env = process.env) {
    try {
...
function detectSource(modality, fromConfig, env) {
    const explicitEnv = env[envName(modality, 'PROVIDER')] ??
 ...
 */
export function getProviderStatus(modality, fromConfig, env = process.env) {
    try {
...
export function getAllProvidersStatus(fromConfig, env = process.env) {
