...
describe('SubagentStatistics', () => {
    ...
    describe('tool usage statistics', () => {
        it('should track individual tool usage', () => {
            ...
            const summary = stats.getSummary();
            const fileReadTool = summary.toolUsage.find((t) => t.name === 'file_read');
            const webSearchTool = summary.toolUsage.find((t) => t.name === 'web_search');
    ...
    describe('formatDetailed', () => {
        ...
        it('should include quality assessment', () => {
            ...
            expect(result).toContain('✅ Quality: Poor execution (66.7% tool success)');
        });
        ...
        it('should include performance insights', () => {
            ...
            expect(result).toContain('Long execution time - consider breaking down complex tasks');
        });
    });
    ...
    describe('performance tips', () => {
        it('should suggest reviewing low success rate', () => {
            ...
            expect(result).toContain('Low tool success rate - review inputs and error messages');
        });
        ...
        it('should suggest optimizing high token usage', () => {
            ...
            expect(result).toContain('High token usage - consider optimizing prompts or narrowing scope');
        });
        it('should identify high token usage per call', () => {
            stats.recordToolCall('test', true, 100);
            stats.recordTokens(6000, 0);
            const result = stats.formatDetailed('Verbose task');
            expect(result).toContain('High token usage per tool call (~6000 tokens/call)');
        });
        it('should identify network failures', () => {
            ...
            expect(result).toContain('Network operations had failures - consider increasing timeout or checking connectivity');
        });
        it('should identify slow tools', () => {
            stats.recordToolCall('slow_tool', true, 15000);
            const result = stats.formatDetailed('Slow tool task');
            expect(result).toContain('Consider optimizing slow_tool operations (avg 15.0s)');
        });
    });
...
//# sourceMappingURL=subagent-statistics.test.js.map