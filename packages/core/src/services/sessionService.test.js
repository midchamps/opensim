...
import { SessionService, buildApiHistoryFromConversation, } from './sessionService.js';
...
describe('SessionService', () => {
    ...
    describe('listSessions', () => {
        ...
        it('should list sessions sorted by mtime descending', async () => {
            ...
            statSyncSpy.mockImplementation((filePath) => {
        ...
        it('should extract prompt text from first record', async () => {
            const now = Date.now();
            readdirSyncSpy.mockReturnValue([`${sessionIdA}.jsonl`]);
        ...
        it('should skip files from different projects', async () => {
            ...
            const differentProjectRecord = {
                ...recordA1,
                cwd: '/different/project',
            };
            vi.mocked(jsonl.readLines).mockResolvedValue([differentProjectRecord]);
            vi.mocked(getProjectHash).mockImplementation((cwd) => cwd === '/test/project/root'
                ? 'test-project-hash'
                : 'other-project-hash');
            const result = await sessionService.listSessions();
            expect(result.items).toHaveLength(0);
    ...
    describe('removeSession', () => {
        ...
        it('should return false when session does not exist', async () => {
            vi.mocked(jsonl.readLines).mockResolvedValue([]);
            const result = await sessionService.removeSession('00000000-0000-0000-0000-000000000000');
    ...
    describe('sessionExists', () => {
        ...
        it('should return false for non-existing session', async () => {
            vi.mocked(jsonl.readLines).mockResolvedValue([]);
            const exists = await sessionService.sessionExists('00000000-0000-0000-0000-000000000000');
