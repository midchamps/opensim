...
export class QuizManager {
    ...
    getRandomQuestion(filter) {
        ...
        if (filter) {
            ...
            if (filter.difficulty !== undefined) {
                candidates = candidates.filter((c) => c.q.difficulty === filter.difficulty);
            }
            if (filter.maxDifficulty !== undefined) {
                candidates = candidates.filter((c) => (c.q.difficulty ?? 1) <= filter.maxDifficulty);
            }
    ...
    getQuestionCount(filter) {
        ...
        return this.questions.filter((q, i) => {
            ...
            if (filter.maxDifficulty !== undefined &&
                ...
                return false;
            if (filter.excludeUsed && this.usedIndices.has(i))
                return false;
            return true;
        }).length;
    }
    ...
    checkAnswer(question, selectedIndex) {
        ...
        if (correct) {
            ...
            this.stats.streakBest = Math.max(this.stats.streakBest, this.stats.streakCurrent);
        }
        else {
