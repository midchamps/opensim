...
import { createContentGenerator, createContentGeneratorConfig, AuthType, } from '../core/contentGenerator.js';
    ...
    describe('SubAgentScope', () => {
        ...
        describe('runNonInteractive - Execution and Tool Use', () => {
            ...
            it('should execute external tools and provide the response to the model', async () => {
                ...
                vi.mocked(config.getToolRegistry().getTool).mockImplementation((name) => name === 'list_files' ? listFilesTool : undefined);
                const scope = await SubAgentScope.create('test-agent', config, promptConfig, defaultModelConfig, defaultRunConfig, toolConfig);
                await scope.runNonInteractive(new ContextState());
                // Check the response sent back to the model (functionResponse part)
                const secondCallArgs = mockSendMessageStream.mock.calls[1][1];
                const parts = secondCallArgs.message;
                expect(Array.isArray(parts)).toBe(true);
                const firstPart = parts[0];
                expect(firstPart.functionResponse?.response?.['output']).toBe('file1.txt\nfile2.ts');
                expect(scope.getTerminateMode()).toBe(SubagentTerminateMode.GOAL);
            });
        });
