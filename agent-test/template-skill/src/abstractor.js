import { getAbstractorConfig } from './config.js';
...
function buildUserPrompt(patterns) {
    const parts = [];
    parts.push(`## Archetype: ${patterns.archetype}`);
    parts.push(`## Physics: gravity=${patterns.physicsProfile.hasGravity}, ` +
...
// -----------------------------------------------------------------------------
function abstractByRules(patterns) {
    const templateFiles = [];
    ...
    return {
        ...
        summary: `Rule-based abstraction for ${patterns.archetype}: ` +
 ...
 */
export async function abstractPatterns(patterns) {
    ...
    if (config.apiKey) {
        try {
            ...
            const templateFiles = parsed.templateFiles.map((f) => ({
                relativePath: f.relativePath,
                content: f.content,
                role: f.role,
            }));
            console.log(`[Abstractor] LLM produced ${templateFiles.length} template files`);
            return {
                archetype: patterns.archetype,
                templateFiles,
                hooks: patterns.hooks,
    ...
    console.log(`[Abstractor] Rule-based: ${result.templateFiles.length} template files`);
    return result;
}
