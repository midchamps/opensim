 ...
 */
import { executeToolCall, ToolErrorType, shutdownTelemetry, GeminiEventType, OutputFormat, uiTelemetryService, FatalInputError, ApprovalMode, } from '@opengame/opengame-core';
...
describe('runNonInteractive', () => {
     ...
     */
    function setupMetricsMock(overrides) {
    ...
    it('should process input and write text output', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'Test input' }], expect.any(AbortSignal), 'prompt-id-1', { isContinuation: false });
    ...
    it('should handle a single tool call and respond', async () => {
        ...
        expect(mockCoreExecuteToolCall).toHaveBeenCalledWith(mockConfig, expect.objectContaining({ name: 'testTool' }), expect.any(AbortSignal), expect.objectContaining({
            outputUpdateHandler: expect.any(Function),
        }));
        // Verify first call has isContinuation: false
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(1, [{ text: 'Use a tool' }], expect.any(AbortSignal), 'prompt-id-2', { isContinuation: false });
        // Verify second call (after tool execution) has isContinuation: true
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(2, [{ text: 'Tool response' }], expect.any(AbortSignal), 'prompt-id-2', { isContinuation: true });
    ...
    it('should handle error during tool execution and should send error back to the model', async () => {
        ...
        ], expect.any(AbortSignal), 'prompt-id-3', { isContinuation: true });
    ...
    it('should preprocess @include commands before sending to the model', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith(processedParts, expect.any(AbortSignal), 'prompt-id-7', { isContinuation: false });
    ...
    it('should process input and write JSON output with stats', async () => {
        ...
        const outputCalls = processStdoutSpy.mock.calls.filter((call) => typeof call[0] === 'string');
    ...
    it('should write JSON output with stats for tool-only commands (no text response)', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledTimes(2);
        expect(mockCoreExecuteToolCall).toHaveBeenCalledWith(mockConfig, expect.objectContaining({ name: 'testTool' }), expect.any(AbortSignal), undefined);
        // JSON adapter emits array of messages, last one is result with stats
        const outputCalls = processStdoutSpy.mock.calls.filter((call) => typeof call[0] === 'string');
        expect(outputCalls.length).toBeGreaterThan(0);
        const lastOutput = outputCalls[outputCalls.length - 1][0];
    ...
    it('should write JSON output with stats for empty response commands', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'Empty response test' }], expect.any(AbortSignal), 'prompt-id-empty', { isContinuation: false });
        // JSON adapter emits array of messages, last one is result with stats
        const outputCalls = processStdoutSpy.mock.calls.filter((call) => typeof call[0] === 'string');
        expect(outputCalls.length).toBeGreaterThan(0);
        const lastOutput = outputCalls[outputCalls.length - 1][0];
    ...
    it('should handle API errors in text mode and exit with error code', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents([apiErrorEvent]));
        let thrownError = null;
        try {
            await runNonInteractive(mockConfig, mockSettings, 'Test input', 'prompt-id-api-error');
            // Should not reach here
    ...
    it('should execute a slash command that returns a prompt', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'Prompt from command' }], expect.any(AbortSignal), 'prompt-id-slash', { isContinuation: false });
    ...
    it('should handle command that requires confirmation by returning early', async () => {
        ...
        mockGetCommands.mockReturnValue([mockCommand]);
        await runNonInteractive(mockConfig, mockSettings, '/confirm', 'prompt-id-confirm');
        // Should write error message to stderr
        expect(processStderrSpy).toHaveBeenCalledWith('Shell command confirmation is not supported in non-interactive mode. Use YOLO mode or pre-approve commands.\n');
    });
    it('should treat an unknown slash command as a regular prompt', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: '/unknowncommand' }], expect.any(AbortSignal), 'prompt-id-unknown', { isContinuation: false });
    ...
    it('should handle known but unsupported slash commands like /help by returning early', async () => {
        ...
        await runNonInteractive(mockConfig, mockSettings, '/help', 'prompt-id-help');
        // Should write error message to stderr
        expect(processStderrSpy).toHaveBeenCalledWith('The command "/help" is not supported in non-interactive mode.\n');
    });
    it('should handle unhandled command result types by returning early with error', async () => {
        ...
        mockGetCommands.mockReturnValue([mockCommand]);
        await runNonInteractive(mockConfig, mockSettings, '/noaction', 'prompt-id-unhandled');
        // Should write error message to stderr
        expect(processStderrSpy).toHaveBeenCalledWith('Unknown command result type: unhandled\n');
    });
    ...
    it('should emit stream-json envelopes when output format is stream-json', async () => {
        mockConfig.getOutputFormat.mockReturnValue('stream-json');
        mockConfig.getIncludePartialMessages.mockReturnValue(false);
        setupMetricsMock();
        const writes = [];
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(events));
        await runNonInteractive(mockConfig, mockSettings, 'Stream input', 'prompt-stream');
    ...
    it.skip('should emit a single user envelope when userEnvelope is provided', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents([
        ...
        await runNonInteractive(mockConfig, mockSettings, 'ignored input', 'prompt-envelope', {
    ...
    it('should include usage metadata and API duration in stream-json result', async () => {
        ...
        await runNonInteractive(mockConfig, mockSettings, 'usage test', 'prompt-usage');
        const envelopes = writes
            .join('')
            .split('\n')
            .filter((line) => line.trim().length > 0)
    ...
    it('should not emit user message when userMessage option is provided (stream-json input binding)', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'Message from stream-json input' }], expect.any(AbortSignal), 'prompt-envelope', { isContinuation: false });
    });
    it('should emit tool results as user messages in stream-json format', async () => {
        ...
        mockGeminiClient.sendMessageStream
            .mockReturnValueOnce(createStreamFromEvents(firstCallEvents))
            .mockReturnValueOnce(createStreamFromEvents(secondCallEvents));
        await runNonInteractive(mockConfig, mockSettings, 'Use tool', 'prompt-id-tool');
        const envelopes = writes
            .join('')
            .split('\n')
            .filter((line) => line.trim().length > 0)
        ...
        const assistantEnvelope = envelopes.find((env) => env.type === 'assistant');
        expect(assistantEnvelope).toBeTruthy();
        const toolUseBlock = assistantEnvelope?.message?.content?.find((block) => typeof block === 'object' &&
            block !== null &&
            'type' in block &&
            block.type === 'tool_use');
        ...
        const toolResultUserMessages = envelopes.filter((env) => env.type === 'user' &&
            Array.isArray(env.message?.content) &&
            env.message.content.some((block) => typeof block === 'object' &&
                block !== null &&
                'type' in block &&
                block.type === 'tool_result'));
        expect(toolResultUserMessages).toHaveLength(1);
        const toolResultBlock = toolResultUserMessages[0]?.message?.content?.find((block) => typeof block === 'object' &&
            block !== null &&
            'type' in block &&
            block.type === 'tool_result');
    ...
    it('should emit tool errors in tool_result blocks in stream-json format', async () => {
        ...
        mockGeminiClient.sendMessageStream
            .mockReturnValueOnce(createStreamFromEvents([toolCallEvent]))
            .mockReturnValueOnce(createStreamFromEvents(finalResponse));
        await runNonInteractive(mockConfig, mockSettings, 'Trigger error', 'prompt-id-error');
        const envelopes = writes
            .join('')
            .split('\n')
            .filter((line) => line.trim().length > 0)
        ...
        const toolResultMessages = envelopes.filter((env) => env.type === 'user' &&
            Array.isArray(env.message?.content) &&
            env.message.content.some((block) => typeof block === 'object' &&
                block !== null &&
                'type' in block &&
        ...
        const toolResultBlock = toolResultMessages[0]?.message?.content?.find((block) => typeof block === 'object' &&
            block !== null &&
            'type' in block &&
            block.type === 'tool_result');
    ...
    it('should emit partial messages when includePartialMessages is true', async () => {
        mockConfig.getOutputFormat.mockReturnValue('stream-json');
        mockConfig.getIncludePartialMessages.mockReturnValue(true);
        setupMetricsMock();
        const writes = [];
        processStdoutSpy.mockImplementation((chunk) => {
            if (typeof chunk === 'string') {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(events));
        await runNonInteractive(mockConfig, mockSettings, 'Stream test', 'prompt-partial');
        const envelopes = writes
            .join('')
            .split('\n')
            .filter((line) => line.trim().length > 0)
        ...
        const messageStart = streamEvents.find((ev) => ev.event?.type === 'message_start');
        expect(messageStart).toBeTruthy();
        // Should have content_block_delta events for incremental text
        const textDeltas = streamEvents.filter((ev) => ev.event?.type === 'content_block_delta');
    ...
    it('should handle thinking blocks in stream-json format', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(events));
        await runNonInteractive(mockConfig, mockSettings, 'Thinking test', 'prompt-thinking');
        const envelopes = writes
            .join('')
            .split('\n')
            .filter((line) => line.trim().length > 0)
        ...
        const assistantEnvelope = envelopes.find((env) => env.type === 'assistant');
        expect(assistantEnvelope).toBeTruthy();
        const thinkingBlock = assistantEnvelope?.message?.content?.find((block) => typeof block === 'object' &&
            block !== null &&
            'type' in block &&
            block.type === 'thinking');
    ...
    it('should handle multiple tool calls in stream-json format', async () => {
        ...
        const firstCallEvents = [toolCall1, toolCall2];
        const secondCallEvents = [
        ...
        mockGeminiClient.sendMessageStream
            .mockReturnValueOnce(createStreamFromEvents(firstCallEvents))
            .mockReturnValueOnce(createStreamFromEvents(secondCallEvents));
        await runNonInteractive(mockConfig, mockSettings, 'Multiple tools', 'prompt-id-multi');
        const envelopes = writes
            .join('')
            .split('\n')
            .filter((line) => line.trim().length > 0)
        ...
        const assistantEnvelope = envelopes.find((env) => env.type === 'assistant');
        expect(assistantEnvelope).toBeTruthy();
        const toolUseBlocks = assistantEnvelope?.message?.content?.filter((block) => typeof block === 'object' &&
            block !== null &&
            'type' in block &&
            block.type === 'tool_use');
        expect(toolUseBlocks?.length).toBe(2);
        const toolNames = (toolUseBlocks ?? []).map((b) => {
            if (typeof b === 'object' &&
                b !== null &&
                'name' in b &&
    ...
    it('should handle userMessage with text content blocks in stream-json input mode', async () => {
        ...
        await runNonInteractive(mockConfig, mockSettings, 'ignored', 'prompt-string-content', {
            userMessage: userMessageString,
        });
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'Simple string content' }], expect.any(AbortSignal), 'prompt-string-content', { isContinuation: false });
        ...
        await runNonInteractive(mockConfig, mockSettings, 'ignored', 'prompt-blocks-content', {
            userMessage: userMessageBlocks,
        });
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'First part' }, { text: 'Second part' }], expect.any(AbortSignal), 'prompt-blocks-content', { isContinuation: false });
    });
    it('should print tool output to console in text mode (non-Task tools)', async () => {
        ...
        mockCoreExecuteToolCall.mockImplementation(async (_config, _request, _signal, options) => {
        ...
        mockGeminiClient.sendMessageStream
            .mockReturnValueOnce(createStreamFromEvents(firstCallEvents))
            .mockReturnValueOnce(createStreamFromEvents(secondCallEvents));
        await runNonInteractive(mockConfig, mockSettings, 'Check dependencies', 'prompt-id-tool-output');
        // Verify that executeToolCall was called with outputUpdateHandler
        expect(mockCoreExecuteToolCall).toHaveBeenCalledWith(mockConfig, expect.objectContaining({ name: 'run_in_terminal' }), expect.any(AbortSignal), expect.objectContaining({
            outputUpdateHandler: expect.any(Function),
        }));
