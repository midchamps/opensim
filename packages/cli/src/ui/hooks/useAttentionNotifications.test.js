...
import { AttentionNotificationReason, notifyTerminalAttention, } from '../../utils/attentionNotification.js';
...
describe('useAttentionNotifications', () => {
    ...
    it('notifies when tool approval is required while unfocused', () => {
        ...
        expect(mockedNotify).toHaveBeenCalledWith(AttentionNotificationReason.ToolApproval, { enabled: true });
    });
    ...
    it('sends a notification when a long task finishes while unfocused', () => {
        ...
        expect(mockedNotify).toHaveBeenCalledWith(AttentionNotificationReason.LongTaskComplete, { enabled: true });
    });
    it('does not notify about long tasks when the CLI is focused', () => {
        ...
        expect(mockedNotify).not.toHaveBeenCalledWith(AttentionNotificationReason.LongTaskComplete, expect.anything());
    });
    ...
    it('does not notify when terminalBell setting is disabled', () => {
        ...
        expect(mockedNotify).toHaveBeenCalledWith(AttentionNotificationReason.ToolApproval, { enabled: false });
    });
});
