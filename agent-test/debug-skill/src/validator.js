 ...
 */
export async function validateProject(projectDir, protocol) {
    const results = [];
 ...
 */
async function runEntryCheck(projectDir, entry) {
    const violations = [];
 ...
 */
async function runRuleCheck(projectDir, rule) {
    const violations = [];
    for (const check of rule.checks) {
        ...
        try {
            ...
            for (const filePath of files) {
                ...
                if (matches) {
                    violations.push(`${check.violationMessage} (${path.relative(projectDir, filePath)}: ${matches.length} occurrence(s))`);
                }
            }
        }
        catch {
 ...
 */
async function checkAssetKeyConsistency(projectDir) {
    const violations = [];
        ...
        for (const filePath of tsFiles) {
            ...
            for (const key of usedKeys) {
                if (!registeredKeys.has(key)) {
                    violations.push(`Asset key '${key}' used in ${path.relative(projectDir, filePath)} but not found in asset-pack.json`);
                }
            }
        }
    }
 ...
 */
async function checkConfigFieldConsistency(projectDir) {
    const violations = [];
    ...
    try {
        ...
        for (const filePath of tsFiles) {
            ...
            for (const match of fieldAccesses) {
                ...
                if (['json', 'value', 'type', 'description'].includes(field))
                    continue;
                if (!configKeys.has(field)) {
                    violations.push(`Config field '${field}' accessed in ${path.relative(projectDir, filePath)} but not defined in gameConfig.json`);
                }
            }
        }
    }
 ...
 */
async function checkSceneRegistration(projectDir) {
    const violations = [];
        ...
        for (const filePath of tsFiles) {
            ...
            for (const match of sceneStarts) {
                ...
                if (!registeredScenes.has(sceneKey)) {
                    violations.push(`Scene key '${sceneKey}' used in ${path.relative(projectDir, filePath)} but not registered in main.ts`);
                }
            }
        }
    }
 ...
 */
async function checkAnimationKeyConsistency(projectDir) {
    const violations = [];
        ...
        for (const filePath of tsFiles) {
            ...
            for (const match of animRefs) {
                ...
                if (!animKeys.has(key) && animKeys.size > 0) {
                    violations.push(`Animation key '${key}' used in ${path.relative(projectDir, filePath)} but not defined in animations.json`);
                }
            }
        }
    }
 ...
 */
async function checkImportTypeKeyword(projectDir) {
    const violations = [];
    const tsFiles = await collectFiles(path.join(projectDir, 'src', '**', '*.ts'));
    for (const filePath of tsFiles) {
        const content = await fs.readFile(filePath, 'utf-8');
        ...
        for (const match of importMatches) {
            ...
            for (const name of names) {
                ...
                if (/^I[A-Z]/.test(name) || /(?:Config|Props|Options|Params|State|Result)$/.test(name)) {
                    violations.push(`Possible interface '${name}' imported without 'type' keyword in ${path.relative(projectDir, filePath)}`);
                }
            }
        }
    }
 ...
 */
async function checkLevelOrder(projectDir) {
    const violations = [];
        ...
        if (orderMatch) {
            ...
            if (entries && entries.length > 0) {
                ...
                if (firstScene === 'Level1Scene' || firstScene === 'Level1') {
                    ...
                    violations.push(`LEVEL_ORDER[0] is '${firstScene}' — verify this is your actual first game scene and not a template default`);
                }
            }
        }
    }
...
async function collectFilesRecursive(dir, ext) {
    const result = [];
    try {
 ...
 */
function extractAssetKeys(assetPack) {
    const keys = new Set();
    function walk(obj) {
 ...
 */
function extractUsedAssetKeys(content) {
    const keys = new Set();
