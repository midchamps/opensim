...
import { MissingProviderConfigError, resolveProviderConfig, tryResolveProviderConfig, getProviderStatus, getAllProvidersStatus, ALL_MODALITIES, } from './providerConfig.js';
describe('resolveProviderConfig', () => {
    it('throws an actionable error when nothing is configured', () => {
        expect(() => resolveProviderConfig('image', undefined, {})).toThrow(MissingProviderConfigError);
        expect(() => resolveProviderConfig('image', undefined, {})).toThrow(/OPENGAME_IMAGE_PROVIDER/);
    });
    ...
    it('falls back to settings.json when env vars are absent', () => {
        const cfg = resolveProviderConfig('reasoning', {
            reasoning: {
                provider: 'doubao',
                apiKey: 'volc-key',
            },
    ...
    it('lets env vars override settings.json field-by-field', () => {
        const cfg = resolveProviderConfig('image', {
            image: {
                provider: 'tongyi',
                apiKey: 'settings-key',
    ...
    it('requires a base URL and model for openai-compat', () => {
        expect(() => resolveProviderConfig('image', undefined, {
    ...
    it('reads gemini image config with the Imagen default model', () => {
        ...
        expect(cfg.provider).toBe('gemini');
        expect(cfg.baseUrl).toBe('https://generativelanguage.googleapis.com/v1beta');
    ...
    it('accepts gemini via settings.json', () => {
        const cfg = resolveProviderConfig('audio', {
            audio: {
                provider: 'gemini',
    ...
    it('resolves all four modalities independently', () => {
        ...
        expect(resolveProviderConfig('reasoning', undefined, env).provider).toBe('openai-compat');
        expect(resolveProviderConfig('image', undefined, env).provider).toBe('tongyi');
        expect(resolveProviderConfig('video', undefined, env).provider).toBe('doubao');
        expect(resolveProviderConfig('audio', undefined, env).provider).toBe('tongyi');
    });
});
describe('getProviderStatus', () => {
    ...
    it('reports source=settings when only settings.json supplies the config', () => {
        const status = getProviderStatus('image', { image: { provider: 'tongyi', apiKey: 'sk-from-settings' } }, {});
...
//# sourceMappingURL=providerConfig.test.js.map