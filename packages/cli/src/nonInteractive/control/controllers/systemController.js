...
import { MCPServerConfig, AuthProviderType, } from '@opengame/opengame-core';
export class SystemController extends BaseController {
    ...
    async handleInitialize(payload, signal) {
            ...
            if (sdkServerCount > 0) {
                try {
                    ...
                    if (this.context.debugMode) {
                        console.error(`[SystemController] Added ${sdkServerCount} SDK MCP servers to config`);
                    }
                }
                catch (error) {
                    if (this.context.debugMode) {
                        console.error('[SystemController] Failed to add SDK MCP servers:', error);
                    }
                }
            }
        }
        if (payload.mcpServers &&
            ...
            if (externalCount > 0) {
                try {
                    ...
                    if (this.context.debugMode) {
                        console.error(`[SystemController] Added ${externalCount} external MCP servers to config`);
                    }
                }
                catch (error) {
                    if (this.context.debugMode) {
                        console.error('[SystemController] Failed to add external MCP servers:', error);
                    }
                }
            }
        }
        if (payload.agents && Array.isArray(payload.agents)) {
            try {
                ...
                if (this.context.debugMode) {
                    console.error(`[SystemController] Added ${payload.agents.length} session subagents to config`);
                }
            }
            catch (error) {
                if (this.context.debugMode) {
                    console.error('[SystemController] Failed to add session subagents:', error);
                }
            }
        }
    ...
    normalizeMcpServerConfig(serverName, config) {
        if (!config || typeof config !== 'object') {
            if (this.context.debugMode) {
                console.error(`[SystemController] Ignoring invalid MCP server config for '${serverName}'`);
            }
            return null;
        }
    ...
    normalizeAuthProviderType(value) {
        ...
        switch (value) {
            ...
            default:
                if (this.context.debugMode) {
                    console.error(`[SystemController] Unsupported authProviderType '${value}', skipping`);
                }
                return undefined;
        }
    }
    ...
    async loadSlashCommandNames(signal) {
        ...
        try {
            ...
            if (this.context.debugMode) {
                console.error('[SystemController] Failed to load slash commands:', error);
            }
            return [];
        }
    }
