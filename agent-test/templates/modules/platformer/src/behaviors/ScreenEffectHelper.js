...
export class ScreenEffectHelper {
    ...
    static createTrail(scene, owner, config) {
        for (let i = 0; i < config.count; i++) {
            scene.time.delayedCall(i * config.delay, () => {
                ...
                const trail = scene.add.image(owner.x, owner.y - 30, // Slightly above ground
                config.imageKey);
     ...
     */
    static createVortex(scene, x, y, config, onComplete) {
     ...
     */
    static createDefaultVortex(scene, x, y, imageKey, onComplete) {
        return ScreenEffectHelper.createVortex(scene, x, y, {
            imageKey,
            initialScale: 0.3,
            maxScale: 0.6,
            rotation: Math.PI * 16,
     ...
     */
    static createChargeEffect(scene, x, y, imageKey, duration = 500) {
