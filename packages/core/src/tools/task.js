...
import { SubAgentEventEmitter, SubAgentEventType, } from '../subagents/subagent-events.js';
/**
...
export class TaskTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        ...
        super(TaskTool.Name, ToolDisplayNames.TASK, 'Delegate tasks to specialized subagents. Loading available subagents...', // Initial description
        Kind.Other, initialSchema, true, // isOutputMarkdown
        true);
        this.config = config;
...
class TaskToolInvocation extends BaseToolInvocation {
    ...
    currentDisplay = null;
    currentToolCalls = [];
    constructor(config, subagentManager, params) {
        super(params);
        this.config = config;
    ...
    setupEventListeners(updateOutput) {
        ...
        this.eventEmitter.on(SubAgentEventType.TOOL_CALL, (...args) => {
            const event = args[0];
            const newToolCall = {
                callId: event.callId,
                name: event.name,
        ...
        this.eventEmitter.on(SubAgentEventType.TOOL_RESULT, (...args) => {
            const event = args[0];
        ...
        this.eventEmitter.on(SubAgentEventType.FINISH, (...args) => {
            const event = args[0];
            this.updateDisplay({
        ...
        this.eventEmitter.on(SubAgentEventType.ERROR, (...args) => {
            const event = args[0];
            this.updateDisplay({
                status: 'failed',
        ...
        this.eventEmitter.on(SubAgentEventType.TOOL_WAITING_APPROVAL, (...args) => {
            const event = args[0];
    ...
    async execute(signal, updateOutput) {
        try {
            ...
            const subagentScope = await this.subagentManager.createSubagentScope(subagentConfig, this.config, { eventEmitter: this.eventEmitter });
