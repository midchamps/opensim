...
export class IdeClient {
    ...
    async connect() {
        if (!this.currentIde) {
            this.setState(IDEConnectionStatus.Disconnected, `IDE integration is not supported in your current environment. To use this feature, run OpenGame in one of these supported IDEs: VS Code or VS Code forks`, false);
            return;
        }
    ...
    static validateWorkspacePath(ideWorkspacePath, cwd) {
        ...
        if (!isWithinWorkspace) {
            return {
                isValid: false,
                error: `Directory mismatch. OpenGame is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(', ')}`,
            };
        }
        return { isValid: true };
    }
        ...
        if (argsStr) {
            try {
                ...
                if (Array.isArray(parsedArgs)) {
                    ...
                    logger.error('QWEN_CODE_IDE_SERVER_STDIO_ARGS must be a JSON array string.');
                }
            }
            catch (e) {
    ...
    async getLegacyConnectionConfig(portFromEnv) {
        if (this.ideProcessInfo) {
            try {
                const portFile = path.join(os.tmpdir(), `qwen-code-ide-server-${this.ideProcessInfo.pid}.json`);
        ...
        if (portFromEnv) {
            try {
                const portFile = path.join(os.tmpdir(), `qwen-code-ide-server-${portFromEnv}.json`);
                const portFileContents = await fs.promises.readFile(portFile, 'utf8');
                return JSON.parse(portFileContents);
            }
            catch (_) {
    ...
    async getAllConnectionConfigs(ideDir) {
        ...
        let lockFiles;
        try {
            lockFiles = (await fs.promises.readdir(ideDir)).filter((file) => fileRegex.test(file));
        }
        catch (e) {
