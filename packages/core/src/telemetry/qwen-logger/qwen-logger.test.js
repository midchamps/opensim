...
import { StartSessionEvent, EndSessionEvent, IdeConnectionEvent, KittySequenceOverflowEvent, IdeConnectionType, } from '../types.js';
// Mock dependencies
...
const makeFakeConfig = (overrides = {}) => {
    const defaults = {
...
describe('QwenLogger', () => {
    ...
    describe('createRumPayload', () => {
        it('includes os metadata in payload', async () => {
            const logger = QwenLogger.getInstance(mockConfig);
            const payload = await logger.createRumPayload();
            expect(payload.os).toEqual(expect.objectContaining({
                type: os.platform(),
                version: os.release(),
            }));
        });
    ...
    describe('event queue management', () => {
        it('should handle event overflow gracefully', () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('QwenLogger: Dropped old event to prevent memory leak'));
        });
        it('should handle enqueue errors gracefully', () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('QwenLogger: Failed to enqueue log event.', expect.any(Error));
    ...
    describe('concurrent flush protection', () => {
        it('should handle concurrent flush requests', () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('QwenLogger: Flush already in progress, marking pending flush'));
    ...
    describe('failed event retry mechanism', () => {
        it('should requeue failed events with size limits', () => {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            logger.requeueFailedEvents(failedEvents);
            // Should have logged about dropping events due to retry limit
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('QwenLogger: Re-queued'));
        });
        it('should handle empty retry queue gracefully', () => {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            logger.requeueFailedEvents(failedEvents);
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('QwenLogger: No events re-queued'));
        });
    });
    ...
    describe('error handling', () => {
        it('should handle flush errors gracefully with debug mode', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('Error flushing to RUM:', expect.any(Error));
            // Restore original method
...
//# sourceMappingURL=qwen-logger.test.js.map