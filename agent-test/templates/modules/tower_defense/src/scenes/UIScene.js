...
export default class UIScene extends Phaser.Scene {
    ...
    pauseGame() {
        ...
        if (!gameScene || !this.scene.isActive(this.currentGameSceneKey))
            return;
    ...
    injectTowerIcons() {
        ...
        this.time.delayedCall(150, () => {
            for (const t of this.towerTypes) {
                const imgEl = document.querySelector(`img[data-tower-icon="${t.id}"]`);
                if (!imgEl)
                    continue;
                try {
                    ...
                    if (!ctx)
                        continue;
                    ctx.drawImage(frame.source.image, frame.cutX, frame.cutY, frame.cutWidth, frame.cutHeight, 0, 0, frame.width, frame.height);
    ...
    setupGameEventListeners() {
        ...
        if (!gameScene)
            return;
        gameScene.events.on('goldChanged', (_oldGold, newGold) => {
            this.gold = newGold;
            this.updateGoldDisplay();
        });
        gameScene.events.on('livesChanged', (_oldLives, newLives) => {
            this.lives = newLives;
            this.updateLivesDisplay();
        });
        gameScene.events.on('waveChanged', (current, total) => {
            this.currentWave = current;
            this.totalWaves = total;
            this.updateWaveDisplay();
            this.hideWaveTimer();
        });
        gameScene.events.on('waveComplete', (_waveNum, _totalWaves) => {
            this.showWaveTimer();
        });
        gameScene.events.on('towerTypesReady', (types) => {
            this.towerTypes = types;
        });
        gameScene.events.on('showCombo', (comboCount) => {
            this.showCombo(comboCount);
        });
        gameScene.events.on('showWaveBonus', (amount) => {
