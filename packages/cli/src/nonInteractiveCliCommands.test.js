...
describe('handleSlashCommand', () => {
    ...
    it('should return unsupported for /help when using default allowed list', async () => {
        ...
        expect(result.type).toBe('unsupported');
        if (result.type === 'unsupported') {
            expect(result.reason).toBe('The command "/help" is not supported in non-interactive mode.');
        }
    });
    it('should execute allowed built-in commands', async () => {
        ...
        const result = await handleSlashCommand('/init', abortController, mockConfig, mockSettings, ['init']);
