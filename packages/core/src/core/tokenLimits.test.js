...
import { normalize, tokenLimit, DEFAULT_TOKEN_LIMIT, DEFAULT_OUTPUT_TOKEN_LIMIT, } from './tokenLimits.js';
describe('normalize', () => {
    ...
    it('should remove date and version suffixes', () => {
        ...
        expect(normalize('gemini-2.0-flash-preview-20250520')).toBe('gemini-2.0-flash');
    });
    ...
    it('should handle a combination of normalization rules', () => {
        expect(normalize('  Google/GEMINI-2.5-PRO:gemini-2.5-pro-20250605  ')).toBe('gemini-2.5-pro');
    });
...
describe('tokenLimit with output type', () => {
    ...
    describe('Default output limits', () => {
        it('should return the default output limit for unknown models', () => {
            expect(tokenLimit('unknown-model', 'output')).toBe(DEFAULT_OUTPUT_TOKEN_LIMIT);
        ...
        it('should return the default output limit for models without specific output patterns', () => {
            expect(tokenLimit('qwen3-coder-7b', 'output')).toBe(DEFAULT_OUTPUT_TOKEN_LIMIT);
    ...
    describe('Input vs Output limits comparison', () => {
        ...
        it('should return same default limits for unknown models', () => {
            ...
            expect(tokenLimit('unknown-model', 'output')).toBe(DEFAULT_OUTPUT_TOKEN_LIMIT); // 4K output
        });
    });
    ...
    describe('Model normalization with output limits', () => {
        ...
        it('should handle complex model strings for output limits', () => {
            expect(tokenLimit('  a/b/c|QWEN3-CODER-PLUS:qwen3-coder-plus-2024-05-13  ', 'output')).toBe(65536);
            expect(tokenLimit('provider/qwen-vl-max-latest:qwen-vl-max-latest-v1', 'output')).toBe(8192);
        });
    });
});
