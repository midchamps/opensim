...
import { getCommandRoots, isCommandAllowed, isCommandNeedsPermission, stripShellWrapper, } from '../utils/shell-utils.js';
    ...
    async execute(signal, updateOutput, shellExecutionConfig, setPidCallback) {
        ...
        try {
            ...
            const { result: resultPromise, pid } = await ShellExecutionService.execute(commandToExecute, cwd, (event) => {
                ...
                if (shouldUpdate) {
                    updateOutput(typeof cumulativeOutput === 'string'
                        ? cumulativeOutput
