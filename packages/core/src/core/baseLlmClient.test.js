...
const createMockResponseWithFunctionCall = (args) => ({
    candidates: [
        {
            content: {
                role: 'model',
...
const createMockResponseWithoutFunctionCall = () => ({
    candidates: [
        {
            content: {
                role: 'model',
...
describe('BaseLlmClient', () => {
    ...
    describe('generateJson - Error Handling', () => {
        ...
        it('should not throw for empty response message check', async () => {
            const emptyResponseError = new Error('API returned an empty response for generateJson.');
            mockGenerateContent.mockRejectedValue(emptyResponseError);
            await expect(client.generateJson(defaultOptions)).rejects.toThrow('API returned an empty response for generateJson.');
