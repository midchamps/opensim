...
import { FatalConfigError, QWEN_DIR, getErrorMessage, Storage, } from '@opengame/opengame-core';
        ...
        try {
            if (fs.existsSync(filePath)) {
                ...
                if (needsMigration(settingsObject)) {
                    ...
                    if (MIGRATE_V2_OVERWRITE) {
                        try {
                            fs.writeFileSync(filePath, JSON.stringify(settingsObject, null, 2), 'utf-8');
                        }
                        catch (e) {
                            console.error(`Error adding version to settings file: ${getErrorMessage(e)}`);
                        }
                    }
                }
