...
`;
    describe('parseSkillContent', () => {
        it('should parse valid markdown content', () => {
            const config = manager.parseSkillContent(validMarkdown, validSkillConfig.filePath, 'project');
...
You are a helpful assistant with this skill.
`;
            const config = manager.parseSkillContent(markdownWithTools, validSkillConfig.filePath, 'project');
        ...
        it('should determine level from file path', () => {
            ...
            const projectConfig = manager.parseSkillContent(validMarkdown, projectPath, 'project');
            const userConfig = manager.parseSkillContent(validMarkdown, userPath, 'user');
...
Just content`;
            expect(() => manager.parseSkillContent(invalidMarkdown, validSkillConfig.filePath, 'project')).toThrow(SkillError);
        });
...
`;
            expect(() => manager.parseSkillContent(markdownWithoutName, validSkillConfig.filePath, 'project')).toThrow(SkillError);
        });
...
You are a helpful assistant.
`;
            expect(() => manager.parseSkillContent(markdownWithoutDescription, validSkillConfig.filePath, 'project')).toThrow(SkillError);
        });
    });
...
Skill 3 content`);
    ...
    describe('parse errors', () => {
        it('should track parse errors', async () => {
            ...
            vi.mocked(fs.access).mockResolvedValue(undefined);
            vi.mocked(fs.readFile).mockResolvedValue('invalid content without frontmatter');
...
//# sourceMappingURL=skill-manager.test.js.map