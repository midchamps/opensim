...
describe('bugCommand', () => {
    beforeEach(() => {
        vi.mocked(systemInfoUtils.getExtendedSystemInfo).mockResolvedValue({
            ...
            memoryUsage: '100 MB',
            gitCommit: GIT_COMMIT_INFO && !['N/A'].includes(GIT_COMMIT_INFO)
                ? GIT_COMMIT_INFO
                : undefined,
        });
    ...
    it('should generate the default GitHub issue URL', async () => {
        ...
        const gitCommitLine = GIT_COMMIT_INFO && !['N/A'].includes(GIT_COMMIT_INFO)
...
* **IDE Client:** VSCode
`;
        const expectedUrl = 'https://github.com/leigest519/OpenGame/issues/new?template=bug_report.yml&title=A%20test%20bug&info=' +
...
* **IDE Client:** VSCode
`;
        const expectedUrl = 'https://github.com/leigest519/OpenGame/issues/new?template=bug_report.yml&title=OpenAI%20bug&info=' +
            encodeURIComponent(expectedInfo);
        expect(open).toHaveBeenCalledWith(expectedUrl);
    });
});
