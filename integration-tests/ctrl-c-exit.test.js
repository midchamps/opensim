...
describe('Ctrl+C exit', () => {
    ...
    it.skip('should exit gracefully on second Ctrl+C', async () => {
        ...
        const showedExitPrompt = await rig.poll(() => output.includes('Press Ctrl+C again to exit'), 1500, 50);
        expect(showedExitPrompt, `Exit prompt not shown. Output: ${output}`).toBe(true);
        // Send second Ctrl+C
        ptyProcess.write(String.fromCharCode(3));
        ...
        const result = await Promise.race([
            promise,
            new Promise((_, reject) => setTimeout(() => reject(new Error(`Process did not exit within ${EXIT_TIMEOUT}ms. Output: ${output}`)), EXIT_TIMEOUT)),
        ]);
        // Expect a graceful exit (code 0)
        expect(result.exitCode, `Process exited with code ${result.exitCode}. Output: ${result.output}`).toBe(0);
...
//# sourceMappingURL=ctrl-c-exit.test.js.map