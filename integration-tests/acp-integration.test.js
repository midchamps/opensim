...
const IS_SANDBOX = process.env['GEMINI_SANDBOX'] &&
 ...
 */
function setupAcpTest(rig, options) {
    ...
    const agent = spawn('node', [rig.bundlePath, acpFlag, '--no-chat-recording'], {
        cwd: rig.testDir,
    ...
    const handleMessage = (msg) => {
        ...
        if (msg.method === 'fs/read_text_file' && typeof msg.id === 'number') {
            try {
                ...
                sendResponse(msg.id, { content: `ERROR: ${e.message}` });
            }
            return;
        }
        if (msg.method === 'fs/write_text_file' && typeof msg.id === 'number') {
            try {
                writeFileSync(msg.params?.path ?? '', msg.params?.content ?? '', 'utf8');
                sendResponse(msg.id, null);
            }
            catch (e) {
                sendResponse(msg.id, { message: e.message });
            }
        }
    };
    rl.on('line', (line) => {
...
(IS_SANDBOX ? describe.skip : describe)('acp integration', () => {
    ...
    it('receives available_commands_update with slash commands after session creation', async () => {
        ...
        try {
            ...
            const commandsUpdate = sessionUpdates.find((update) => update.update?.sessionUpdate === 'available_commands_update');
            expect(commandsUpdate).toBeDefined();
            expect(commandsUpdate?.update?.availableCommands).toBeDefined();
            expect(Array.isArray(commandsUpdate?.update?.availableCommands)).toBe(true);
            // Verify that the 'init' command is present (the only allowed built-in command for ACP)
            const initCommand = commandsUpdate?.update?.availableCommands?.find((cmd) => cmd.name === 'init');
    ...
    it('handles exit plan mode with permission request and mode update notification', async () => {
        ...
        const { sendRequest, cleanup, stderr, sessionUpdates, permissionRequests } = setupAcpTest(rig, {
        ...
        try {
            ...
            const switchModeRequests = permissionRequests.filter((req) => req.toolCall?.kind === 'switch_mode');
            // Check for current_mode_update notifications
            const modeUpdateNotifications = sessionUpdates.filter((update) => update.update?.sessionUpdate === 'current_mode_update');
    ...
    it('receives usage metadata in agent_message_chunk updates', async () => {
        ...
        try {
            ...
            const updatesWithUsage = sessionUpdates.filter((u) => u.update?.sessionUpdate === 'agent_message_chunk' &&
            ...
            expect(typeof usage?.promptTokens === 'number' ||
                typeof usage?.totalTokens === 'number').toBe(true);
        }
        catch (e) {
            if (stderr.length)
                console.error('Agent stderr:', stderr.join(''));
...
(IS_SANDBOX ? describe.skip : describe)('acp flag backward compatibility', () => {
...
//# sourceMappingURL=acp-integration.test.js.map