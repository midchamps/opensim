...
async function main() {
    ...
    if (reactive.length > 0) {
        ...
        for (const entry of reactive) {
            const hitLabel = entry.occurrences > 0
                ...
                : 'never matched';
            console.log(`  [${entry.signature.stage}] ${entry.signature.errorCode.padEnd(20)} ` +
    ...
    if (proactive.length > 0) {
        ...
        for (const entry of proactive) {
            console.log(`  ${entry.signature.errorCode.padEnd(30)} | ${entry.rootCause.slice(0, 60)}`);
        }
    }
    // Rules summary
    if (protocol.rules.length > 0) {
        ...
        for (const rule of protocol.rules) {
            console.log(`  ${rule.name.padEnd(40)} | action: ${rule.action} | ` +
