...
describe('MCPOAuthTokenStorage', () => {
    ...
    describe('with encrypted flag false', () => {
        ...
        describe('getAllCredentials', () => {
            ...
            it('should load tokens from file successfully', async () => {
                ...
                expect(fs.readFile).toHaveBeenCalledWith(path.join('/mock/home', QWEN_DIR, 'mcp-oauth-tokens.json'), 'utf-8');
            });
        ...
        describe('saveToken', () => {
            it('should save token with restricted permissions', async () => {
                ...
                expect(fs.mkdir).toHaveBeenCalledWith(path.join('/mock/home', QWEN_DIR), { recursive: true });
                expect(fs.writeFile).toHaveBeenCalledWith(path.join('/mock/home', QWEN_DIR, 'mcp-oauth-tokens.json'), expect.stringContaining('test-server'), { mode: 0o600 });
            });
        ...
        describe('deleteCredentials', () => {
            ...
            it('should remove token file when no tokens remain', async () => {
                ...
                expect(fs.unlink).toHaveBeenCalledWith(path.join('/mock/home', QWEN_DIR, 'mcp-oauth-tokens.json'));
    ...
    describe('with encrypted flag true', () => {
        ...
        it('should use HybridTokenStorage to set credentials', async () => {
            ...
            expect(mockHybridTokenStorage.setCredentials).toHaveBeenCalledWith(mockCredentials);
        });
        it('should use HybridTokenStorage to save a token', async () => {
            const serverName = 'server1';
            const now = Date.now();
            vi.spyOn(Date, 'now').mockReturnValue(now);
            await tokenStorage.saveToken(serverName, mockToken, 'clientId', 'tokenUrl', 'mcpUrl');
            ...
            expect(mockHybridTokenStorage.setCredentials).toHaveBeenCalledWith(expectedCredential);
