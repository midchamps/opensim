...
import { query, isSDKAssistantMessage, isSDKSystemMessage, } from '@opengame/sdk';
...
describe('System Control (E2E)', () => {
    ...
    describe('setModel API', () => {
        it('should change model dynamically during streaming input', async () => {
            const { generator, resume } = createStreamingInputWithControlPoint('Tell me the model name.', 'Tell me the model name now again.');
            const q = query({
                prompt: generator,
                options: {
                    ...SHARED_TEST_OPTIONS,
        ...
        it('should handle multiple model changes in sequence', async () => {
            const sessionId = crypto.randomUUID();
            let resumeResolve1 = null;
            let resumeResolve2 = null;
            const resumePromise1 = new Promise((resolve) => {
                resumeResolve1 = resolve;
            });
            const resumePromise2 = new Promise((resolve) => {
            ...
            try {
                ...
                await Promise.race([
                    responsePromises[0],
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout 1')), 10000)),
                ]);
                // First model change
                ...
                await Promise.race([
                    responsePromises[1],
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout 2')), 10000)),
                ]);
                // Second model change
                await q.setModel('qwen3-vl-plus');
                resumeResolve2();
                ...
                await Promise.race([
                    responsePromises[2],
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout 3')), 10000)),
                ]);
        ...
        it('should throw error when setModel is called on closed query', async () => {
            ...
            await q.close();
            await expect(q.setModel('qwen3-turbo')).rejects.toThrow('Query is closed');
        });
    });
...
//# sourceMappingURL=system-control.test.js.map