...
class GenerateGDDInvocation extends BaseToolInvocation {
    ...
    resolvedModelConfig;
    constructor(config, params, overrideModelConfig) {
        super(params);
        this.config = config;
        this.overrideModelConfig = overrideModelConfig;
    }
    get modelConfig() {
        if (this.overrideModelConfig)
            return this.overrideModelConfig;
        if (!this.resolvedModelConfig) {
            this.resolvedModelConfig = GenerateGDDTool.resolveModelConfig(this.config);
        }
...
DO NOT STOP. CONTINUE TO PHASE 3 NOW.
    ...
    async buildSystemPrompt() {
        ...
        if (docsDir) {
            try {
                coreRules = await fs.readFile(path.join(docsDir, 'gdd', 'core.md'), 'utf-8');
        ...
        if (docsDir) {
            try {
                designRules = await fs.readFile(path.join(docsDir, 'modules', archetype, 'design_rules.md'), 'utf-8');
        ...
        if (docsDir) {
            try {
                templateApi = await fs.readFile(path.join(docsDir, 'modules', archetype, 'template_api.md'), 'utf-8');
...
`;
    ...
    getBuiltinArchetypeRules(archetype) {
        const rules = {
            platformer: `---
...
- Wave clear bonuses: 10-100g per wave`,
    ...
    async callModel(systemPrompt, userPrompt, signal) {
        const payload = {
...
export class GenerateGDDTool extends BaseDeclarativeTool {
    config;
    modelConfig;
    static Name = ToolNames.GENERATE_GDD;
    /**
     ...
     * tool registration.
     */
    static resolveModelConfig(config) {
    ...
    constructor(config, modelConfig) {
        super(GenerateGDDTool.Name, ToolDisplayNames.GENERATE_GDD, `Generates a Game Design Document (GDD) tailored to a specific game archetype. Call this AFTER classify-game-type and scaffold. The tool dynamically loads archetype-specific design rules.`, Kind.Think, {
            type: 'object',
            properties: {
                ...
                config_summary: {
                    type: 'string',
                    description: 'Optional: Summary of gameConfig.json if already read.',
                },
            },
    ...
    validateToolParamValues(params) {
        if (!params.raw_user_requirement ||
...
//# sourceMappingURL=generate-gdd.js.map