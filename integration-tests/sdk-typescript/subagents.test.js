...
import { SDKTestHelper, extractText, createSharedTestOptions, findToolUseBlocks, assertSuccessfulCompletion, findSystemMessage, } from './test-helper.js';
...
describe('Subagents (E2E)', () => {
    ...
    describe('Subagent Configuration', () => {
        it('should accept session-level subagent configuration', async () => {
            const simpleSubagent = {
                ...
                systemPrompt: 'You are a friendly greeter. When given a task, respond with a cheerful greeting.',
                level: 'session',
            };
            const q = query({
        ...
        it('should handle subagent with specific tools', async () => {
            const toolRestrictedAgent = {
                name: 'read-only-agent',
                description: 'Agent that can only read files',
                systemPrompt: 'You are a file reading assistant. Read files when asked.',
                level: 'session',
    ...
    describe('Subagent Execution', () => {
        ...
        it('should complete simple task with subagent', async () => {
            const simpleTaskAgent = {
                name: 'simple-calculator',
                description: 'Performs simple arithmetic calculations',
                systemPrompt: 'You are a calculator. When given a math problem, solve it and provide just the answer.',
                level: 'session',
            };
            const q = query({
            ...
            try {
                for await (const message of q) {
                    ...
                    if (isSDKAssistantMessage(message)) {
                        ...
                        const toolUseBlock = message.message.content.find((block) => block.type === 'tool_use' && block.name === 'task');
                        if (toolUseBlock) {
                            foundTaskTool = true;
                        }
                        assistantText += extractText(message.message.content);
        ...
        it('should verify subagent execution with comprehensive parent_tool_use_id checks', async () => {
            ...
            try {
                ...
                const subagentMessages = messages.filter((msg) => isSDKAssistantMessage(msg) && msg.parent_tool_use_id !== null);
                ...
                const mainAgentMessages = messages.filter((msg) => isSDKAssistantMessage(msg) && msg.parent_tool_use_id === null);
