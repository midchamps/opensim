...
describe('FileCommandLoader', () => {
    ...
    describe('Extension Command Loading', () => {
        it('loads commands from active extensions', async () => {
            const userCommandsDir = Storage.getUserCommandsDir();
            const projectCommandsDir = new Storage(process.cwd()).getProjectCommandsDir();
            const extensionDir = path.join(process.cwd(), '.qwen/extensions/test-ext');
            mock({
                [userCommandsDir]: {
        ...
        it('only loads commands from active extensions', async () => {
            const extensionDir1 = path.join(process.cwd(), '.qwen/extensions/active-ext');
            const extensionDir2 = path.join(process.cwd(), '.qwen/extensions/inactive-ext');
            mock({
                [extensionDir1]: {
                    'qwen-extension.json': JSON.stringify({
                        name: 'active-ext',
        ...
        it('handles missing extension commands directory gracefully', async () => {
            const extensionDir = path.join(process.cwd(), '.qwen/extensions/no-commands');
            mock({
                [extensionDir]: {
                    'qwen-extension.json': JSON.stringify({
                        name: 'no-commands',
