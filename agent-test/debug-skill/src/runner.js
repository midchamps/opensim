 ...
 */
export async function runStage(projectDir, stage) {
 ...
 */
function parseErrors(output, stage) {
    const errors = [];
...
function parseTSError(line) {
    const match = line.match(/^(.+?)\((\d+),(\d+)\):\s*error\s+(TS\d+):\s*(.+)$/);
    if (!match) {
        // Alternative format: "src/foo.ts:10:5 - error TS2339: ..."
        const alt = line.match(/^(.+?):(\d+):(\d+)\s*-\s*error\s+(TS\d+):\s*(.+)$/);
        if (!alt)
            return null;
        return {
            code: alt[4],
...
function parseRuntimeError(line, stage) {
    if (stage === 'build')
        return null;
    // ReferenceError, TypeError, RangeError, etc.
    const runtimeMatch = line.match(/^\s*(ReferenceError|TypeError|RangeError|SyntaxError|Error):\s*(.+)$/);
    if (runtimeMatch) {
        return {
            code: runtimeMatch[1],
    ...
    const phaserMatch = line.match(/Texture\s+'(.+)'\s+not found/i);
    if (phaserMatch) {
        return {
    ...
    const animMatch = line.match(/Animation\s+'(.+)'\s+(doesn't exist|not found)/i);
    if (animMatch) {
        return {
    ...
    const sceneMatch = line.match(/Scene\s+'(.+)'\s+not found/i);
    if (sceneMatch) {
        return {
            code: 'SceneNotFound',
