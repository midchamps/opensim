...
import { countOccurrences, maybeAugmentOldStringForDeletion, normalizeEditStrings, } from './editHelper.js';
...
`;
    it('returns literal matches unchanged and trims new_string trailing whitespace', () => {
        const result = normalizeEditStrings(file, 'const two = 2;', '  const two = 42;  ');
    ...
    it('normalizes smart quotes to match on-disk text', () => {
        const result = normalizeEditStrings("const greeting = 'Don't';\n", 'const greeting = ‘Don’t’;', 'const greeting = “Hello”;   ');
        expect(result).toEqual({
    ...
    it('matches unicode dash variants', () => {
        const result = normalizeEditStrings('const range = "1-2";\n', 'const range = "1\u20132";', 'const range = "3\u20135";   ');
        expect(result).toEqual({
    ...
    it('matches when trailing whitespace differs only at line ends', () => {
        const result = normalizeEditStrings('value = 1;\n', 'value = 1;   \n', 'value = 2;   \n');
        expect(result).toEqual({
            oldString: 'value = 1;\n',
            newString: 'value = 2;\n',
        });
    ...
    it('treats non-breaking spaces as regular spaces', () => {
        const result = normalizeEditStrings('const label = "hello world";\n', 'const label = "hello\u00a0world";', 'const label = "hi\u00a0world";');
        expect(result).toEqual({
    ...
    it('drops trailing newline from new content when the file lacks it', () => {
        const result = normalizeEditStrings('console.log("hi")', 'console.log("hi")\n', 'console.log("bye")\n');
        expect(result).toEqual({
...
describe('maybeAugmentOldStringForDeletion', () => {
    ...
    it('appends newline when deleting text followed by newline', () => {
        expect(maybeAugmentOldStringForDeletion(file, 'console.log("hi")', '')).toBe('console.log("hi")\n');
    });
    it('leaves strings untouched when not deleting', () => {
        expect(maybeAugmentOldStringForDeletion(file, 'console.log("hi")', 'replacement')).toBe('console.log("hi")');
    });
    it('does not append newline when file lacks the variant', () => {
        expect(maybeAugmentOldStringForDeletion('console.log("hi")', 'console.log("hi")', '')).toBe('console.log("hi")');
    });
    it('no-ops when the old string already ends with a newline', () => {
        expect(maybeAugmentOldStringForDeletion(file, 'console.log("bye")\n', '')).toBe('console.log("bye")\n');
    });
});
//# sourceMappingURL=editHelper.test.js.map