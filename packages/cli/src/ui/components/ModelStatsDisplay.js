...
export const ModelStatsDisplay = () => {
    ...
    if (activeModels.length === 0) {
        return (_jsx(Box, { borderStyle: "round", borderColor: theme.border.default, paddingY: 1, paddingX: 2, children: _jsx(Text, { color: theme.text.primary, children: t('No API calls have been made in this session.') }) }));
    }
    ...
    return (_jsxs(Box, { borderStyle: "round", borderColor: theme.border.default, flexDirection: "column", paddingY: 1, paddingX: 2, children: [_jsx(Text, { bold: true, color: theme.text.accent, children: t('Model Stats For Nerds') }), _jsx(Box, { height: 1 }), _jsxs(Box, { children: [_jsx(Box, { width: METRIC_COL_WIDTH, children: _jsx(Text, { bold: true, color: theme.text.primary, children: t('Metric') }) }), modelNames.map((name) => (_jsx(Box, { width: MODEL_COL_WIDTH, children: _jsx(Text, { bold: true, color: theme.text.primary, children: name }) }, name)))] }), _jsx(Box, { borderStyle: "single", borderBottom: true, borderTop: false, borderLeft: false, borderRight: false, borderColor: theme.border.default }), _jsx(StatRow, { title: t('API'), values: [], isSection: true }), _jsx(StatRow, { title: t('Requests'), values: getModelValues((m) => m.api.totalRequests.toLocaleString()) }), _jsx(StatRow, { title: t('Errors'), values: getModelValues((m) => {
                ...
                }) }), _jsx(Box, { height: 1 }), _jsx(StatRow, { title: t('Tokens'), values: [], isSection: true }), _jsx(StatRow, { title: t('Total'), values: getModelValues((m) => (_jsx(Text, { color: theme.status.warning, children: m.tokens.total.toLocaleString() }))) }), _jsx(StatRow, { title: t('Prompt'), isSubtle: true, values: getModelValues((m) => m.tokens.prompt.toLocaleString()) }), hasCached && (_jsx(StatRow, { title: t('Cached'), isSubtle: true, values: getModelValues((m) => {
                    const cacheHitRate = calculateCacheHitRate(m);
                    return (_jsxs(Text, { color: theme.status.success, children: [m.tokens.cached.toLocaleString(), " (", cacheHitRate.toFixed(1), "%)"] }));
                }) })), hasThoughts && (_jsx(StatRow, { title: t('Thoughts'), isSubtle: true, values: getModelValues((m) => m.tokens.thoughts.toLocaleString()) })), hasTool && (_jsx(StatRow, { title: t('Tool'), isSubtle: true, values: getModelValues((m) => m.tokens.tool.toLocaleString()) })), _jsx(StatRow, { title: t('Output'), isSubtle: true, values: getModelValues((m) => m.tokens.candidates.toLocaleString()) })] }));
};
