...
export class DualPlayerSystem extends Phaser.Events.EventEmitter {
    ...
    setupPlayers() {
        [this.config.player1, this.config.player2].forEach((pc) => {
            ...
            if (this.scene.input.keyboard) {
                ...
                state.answerKeys = pc.keys.answers.map((k) => this.scene.input.keyboard.addKey(k));
            }
    ...
    startBuzzerRound(question) {
        ...
        if (this.config.buzzerTimeLimit && this.config.buzzerTimeLimit > 0) {
            this.buzzerTimer = this.scene.time.delayedCall(this.config.buzzerTimeLimit, () => {
                if (this.isRoundActive && !this.buzzedPlayerId) {
     ...
     */
    waitForBuzzerAnswer(playerId) {
        const state = this.players.get(playerId);
        if (!state || !this.currentQuestion)
            return;
        ...
        if (this.config.buzzerTimeLimit && this.config.buzzerTimeLimit > 0) {
            this.answerTimer = this.scene.time.delayedCall(this.config.buzzerTimeLimit, () => {
                if (!this.isRoundActive)
                    return;
                this.isRoundActive = false;
    ...
    handleBuzzerAnswer(playerId, answerIndex, timeMs) {
     ...
     * Call this from onQuizPhaseStart() or similar hook.
     */
    startSimultaneousRound(question) {
        this.roundNumber++;
        this.isRoundActive = true;
        this.currentQuestion = question;
        ...
        this.players.forEach((state, playerId) => {
            state.answerKeys.forEach((key, answerIndex) => {
                key.once('down', () => {
                    if (!this.isRoundActive || this.playerAnswers.has(playerId))
                        return;
                    const timeMs = Date.now() - this.roundStartTime;
    ...
    resolveSimultaneousRound() {
        if (!this.isRoundActive || !this.currentQuestion)
            return;
        this.isRoundActive = false;
        if (this.buzzerTimer) {
            this.buzzerTimer.remove();
        ...
        this.players.forEach((state, playerId) => {
            ...
            const correct = answered && answer.index === this.currentQuestion.correctIndex;
        ...
        if (winnerId && (this.config.speedBonus ?? 0) > 0) {
            ...
            if (winnerDetail)
                winnerDetail.pointsEarned += this.config.speedBonus;
        }
     ...
     */
    recordTurnAnswer(playerId, correct) {
        const state = this.players.get(playerId);
        if (!state)
            return;
        if (correct) {
    ...
    buildRoundResult(winnerId, details) {
        return {
    ...
    checkWinCondition() {
        if (!this.config.scoreToWin || this.config.scoreToWin <= 0)
            return;
        ...
        let highestScore = -1;
        this.players.forEach((state, id) => {
            if (state.score >= this.config.scoreToWin &&
...
//# sourceMappingURL=DualPlayerSystem.js.map