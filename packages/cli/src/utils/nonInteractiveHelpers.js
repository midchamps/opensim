 ...
 */
import { OutputFormat, ToolErrorType, getMCPServerStatus, } from '@opengame/opengame-core';
 ...
 */
async function loadSlashCommandNames(config, allowedBuiltinCommandNames) {
    const controller = new AbortController();
    try {
        const commands = await getAvailableCommands(config, controller.signal, allowedBuiltinCommandNames);
 ...
 */
export async function buildSystemMessage(config, sessionId, permissionMode, allowedBuiltinCommandNames) {
    ...
    const slashCommands = await loadSlashCommandNames(config, allowedBuiltinCommandNames);
