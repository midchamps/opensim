...
export class GameOverUIScene extends Phaser.Scene {
    ...
    restartGame() {
        ...
        let keyToRestart = this.currentLevelKey ?? LevelManager.getFirstLevelScene();
        if (!keyToRestart) {
    ...
    safeReturnToTitleOrFirstLevel() {
        ...
        const target = this.scene.get('TitleScreen')
            ...
            : firstLevel && this.scene.get(firstLevel)
                ...
                : null;
        if (!target) {
            console.error('GameOverUIScene: No TitleScreen or level scene found. Cannot restart.');
            this.isRestarting = false;
            return;
        }
        this.input.off('pointerdown');
...
//# sourceMappingURL=GameOverUIScene.js.map