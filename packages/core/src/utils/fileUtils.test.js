...
describe('fileUtils', () => {
    ...
    describe('processSingleFileContent', () => {
        ...
        it('should read a text file successfully', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, mockConfig);
            expect(result.llmContent).toBe(content);
            expect(result.returnDisplay).toBe('');
            expect(result.error).toBeUndefined();
        });
        it('should handle file not found', async () => {
            const result = await processSingleFileContent(nonexistentFilePath, mockConfig);
        ...
        it('should handle read errors for image/pdf files', async () => {
            ...
            vi.spyOn(fsPromises, 'readFile').mockRejectedValueOnce(readError);
            const result = await processSingleFileContent(testImageFilePath, mockConfig);
        ...
        it('should process a PDF file', async () => {
            ...
            const result = await processSingleFileContent(testPdfFilePath, mockConfig);
            expect(result.llmContent.inlineData).toBeDefined();
            expect(result.llmContent.inlineData
  ...
  `;
            ...
            const result = await processSingleFileContent(testSvgFilePath, mockConfig);
        ...
        it('should skip binary files', async () => {
            ...
            const result = await processSingleFileContent(testBinaryFilePath, mockConfig);
        ...
        it('should paginate text files correctly (offset and limit)', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, mockConfig, 5, 5); // Read lines 6-10
        ...
        it('should identify truncation when reading the end of a file', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, mockConfig, 10, 10);
        ...
        it('should handle limit exceeding file length', async () => {
            ...
            actualNodeFs.writeFileSync(testTextFilePath, lines.join('\n'));
            const result = await processSingleFileContent(testTextFilePath, mockConfig, 0, 10);
        ...
        it('should truncate long lines in text files', async () => {
            ...
            expect(result.returnDisplay).toBe('Read lines 1-2 of 3 from test.txt (truncated)');
            expect(result.isTruncated).toBe(true);
        });
        it('should truncate when line count exceeds the limit', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, mockConfig, 0, 5);
            expect(result.isTruncated).toBe(true);
        ...
        it('should truncate when a line length exceeds the character limit', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, mockConfig, 0, 11);
            expect(result.isTruncated).toBe(true);
            expect(result.returnDisplay).toBe('Read lines 1-11 of 11 from test.txt (truncated)');
        });
        it('should truncate both line count and line length when both exceed limits', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, mockConfig, 0, 10);
            expect(result.isTruncated).toBe(true);
            expect(result.returnDisplay).toBe('Read lines 1-5 of 20 from test.txt (truncated)');
        });
