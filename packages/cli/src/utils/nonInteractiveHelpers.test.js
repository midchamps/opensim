...
import { ToolErrorType, MCPServerStatus, getMCPServerStatus, OutputFormat, } from '@opengame/opengame-core';
...
vi.mock('../nonInteractiveCliCommands.js', () => ({
    getAvailableCommands: vi
        .fn()
        .mockImplementation(async (_config, _signal, allowedBuiltinCommandNames) => {
        ...
        return allCommands.filter((cmd) => cmd.kind === 'file' ||
...
describe('buildSystemMessage', () => {
    ...
    it('should build system message with all fields', async () => {
        ...
        const result = await buildSystemMessage(mockConfig, 'test-session-id', 'auto', allowedBuiltinCommands);
        expect(result).toEqual({
            type: 'system',
            subtype: 'init',
    ...
    it('should handle empty tool registry', async () => {
        ...
        const result = await buildSystemMessage(config, 'test-session-id', 'auto', ['init', 'summary']);
