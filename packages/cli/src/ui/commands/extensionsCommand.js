...
import { updateAllUpdatableExtensions, updateExtension, checkForAllExtensionUpdates, } from '../../config/extensions/update.js';
...
async function updateAction(context, args) {
    ...
    try {
        ...
        if (all) {
            updateInfos = await updateAllUpdatableExtensions(context.services.config.getWorkingDir(), 
            // We don't have the ability to prompt for consent yet in this flow.
            (description) => requestConsentInteractive(description, context.ui.addConfirmUpdateExtensionRequest), context.services.config.getExtensions(), context.ui.extensionsUpdateState, context.ui.dispatchExtensionStateUpdate);
        }
            ...
            for (const name of names) {
                ...
                const updateInfo = await updateExtension(extension, workingDir, (description) => requestConsentInteractive(description, context.ui.addConfirmUpdateExtensionRequest), context.ui.extensionsUpdateState.get(extension.name)?.status ??
                    ExtensionUpdateState.UNKNOWN, context.ui.dispatchExtensionStateUpdate);
                if (updateInfo)
