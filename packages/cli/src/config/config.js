...
export async function parseArguments(settings) {
    ...
    const yargsInstance = yargs(rawArgv)
        .locale('en')
        .scriptName('opengame')
        .usage('Usage: opengame [options] [command]\n\nOpenGame - Launch an interactive CLI for end-to-end web game creation; use -p/--prompt for non-interactive mode')
        .option('telemetry', {
        type: 'boolean',
        ...
        .option('proxy', {
        type: 'string',
        description: 'Proxy for OpenGame, like schema://user:password@host:port',
    })
        ...
        .command('$0 [query..]', 'Launch OpenGame CLI', (yargsInstance) => yargsInstance
        .positional('query', {
    ...
    if (result['experimentalAcp']) {
        console.warn('\x1b[33m⚠ Warning: --experimental-acp is deprecated and will be removed in a future release. Please use --acp instead.\x1b[0m');
...
export async function loadCliConfig(settings, extensions, extensionEnablementManager, argv, cwd = process.cwd()) {
    ...
    const outputLanguageFilePath = path.join(Storage.getGlobalQwenDir(), 'output-language.md');
    if (fs.existsSync(outputLanguageFilePath)) {
        extensionContextFilePaths.push(outputLanguageFilePath);
        if (debugMode) {
            logger.debug(`Found output-language.md, adding to context files: ${outputLanguageFilePath}`);
        }
    }
    ...
    // Headless / non-interactive mode (e.g. `opengame -p "..."`) defaults to
    // AUTO_EDIT instead of DEFAULT so the model can actually create files —
    // otherwise WriteFile/Edit/Shell would be silently excluded and the model
    // would claim it "has no permission". Shell stays gated until the user
    // explicitly opts in with --yolo. Interactive mode keeps the safer DEFAULT.
    let approvalMode;
    let approvalModeAutoElevated = false;
    ...
    if (!trustedFolder &&
        ...
        approvalMode = ApprovalMode.DEFAULT;
        approvalModeAutoElevated = false;
    }
    ...
    const extraExcludes = [];
    const nonInteractiveToolGating = !interactive &&
        !argv.experimentalAcp &&
        inputFormat !== InputFormat.STREAM_JSON;
    if (nonInteractiveToolGating) {
        switch (approvalMode) {
    ...
    // Surface why tools are/aren't available in headless mode so users don't get
    // confused by the model saying "I can't write files".
    if (nonInteractiveToolGating) {
        if (approvalModeAutoElevated) {
            logger.warn('Headless mode: defaulting approval mode to "auto-edit" so the model can write/edit files. ' +
                'Shell commands are still disabled — pass --yolo (or --approval-mode yolo) to enable them, ' +
                'or --approval-mode default to fully restrict tools.');
        }
        else if (approvalMode === ApprovalMode.DEFAULT ||
            approvalMode === ApprovalMode.PLAN) {
            logger.warn(`Headless mode with approval mode "${approvalMode === ApprovalMode.PLAN ? 'plan' : 'default'}": write_file, replace, and run_shell_command are disabled, so the model cannot create or modify files. ` +
                'Pass --approval-mode auto-edit to allow file edits, or --yolo / --approval-mode yolo to also allow shell commands.');
        }
    }
    const excludeTools = mergeExcludeTools(settings, activeExtensions, extraExcludes.length > 0 ? extraExcludes : undefined, argv.excludeTools);
    ...
    const apiKey = (selectedAuthType === AuthType.USE_OPENAI
        ? argv.openaiApiKey ||
            process.env['OPENAI_API_KEY'] ||
            settings.security?.auth?.apiKey
        : '') || '';
    const baseUrl = (selectedAuthType === AuthType.USE_OPENAI
        ? argv.openaiBaseUrl ||
    ...
    return new Config({
        ...
        generationConfig: {
            ...
            openAILoggingDir: argv.openaiLoggingDir || settings.model?.openAILoggingDir,
        },
        cliVersion: await getCliVersion(),
        webSearch: buildWebSearchConfig(argv, settings, settings.security?.auth?.selectedType),
        // Per-modality OpenGame provider configuration. Read from settings.json;
        // env vars take precedence inside the resolver in
        // packages/core/src/services/providerConfig.ts.
        openGameProviders: settings.openGame?.providers,
