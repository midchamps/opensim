...
const createWrappedGenerator = (generateContent, generateContentStream) => ({
    generateContent,
...
const createResponse = (responseId, modelVersion, parts, usageMetadata, finishReason) => {
...
describe('LoggingContentGenerator', () => {
    ...
    it('logs request/response, normalizes thought parts, and logs OpenAI interaction', async () => {
        const wrapped = createWrappedGenerator(vi.fn().mockResolvedValue(createResponse('resp-1', 'model-v2', [{ text: 'ok' }], {
            promptTokenCount: 3,
        ...
        const [openaiRequest, openaiResponse, openaiError] = openaiLoggerInstance.logInteraction.mock.calls[0];
    ...
    it('logs errors with status code and request id, then rethrows', async () => {
        ...
        const wrapped = createWrappedGenerator(vi.fn().mockRejectedValue(error), vi.fn());
        const generator = new LoggingContentGenerator(wrapped, createConfig({ enableOpenAILogging: true }));
        const request = {
            model: 'test-model',
            contents: 'Hello',
        };
        await expect(generator.generateContent(request, 'prompt-2')).rejects.toThrow('boom');
    ...
    it('logs streaming responses and consolidates tool calls', async () => {
        ...
        const wrapped = createWrappedGenerator(vi.fn(), vi.fn().mockResolvedValue((async function* () {
            yield response1;
            yield response2;
        })()));
        const generator = new LoggingContentGenerator(wrapped, createConfig({ enableOpenAILogging: true }));
        ...
        expect(convertGeminiResponseToOpenAISpy).toHaveBeenCalledTimes(1);
        const [consolidatedResponse] = convertGeminiResponseToOpenAISpy.mock.calls[0];
        const consolidatedParts = consolidatedResponse.candidates?.[0]?.content?.parts || [];
