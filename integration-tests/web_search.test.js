...
describe('web_search', () => {
    it('should be able to search the web', async () => {
        ...
        if (!hasTavilyKey && !hasGoogleKey) {
            console.warn('Skipping web search test: No web search provider configured. ' +
        ...
        if (!foundToolCall) {
            ...
            const failedSearchCalls = allTools.filter((t) => t.toolRequest.name === 'web_search' && !t.toolRequest.success);
            if (failedSearchCalls.length > 0) {
                console.warn('web_search tool was called but failed, possibly due to network issues');
                console.warn('Failed calls:', failedSearchCalls.map((t) => t.toolRequest.args));
        ...
        const hasExpectedContent = validateModelOutput(result, ['weather', 'london'], 'Web search test');
        ...
        if (!hasExpectedContent) {
            ...
            if (searchCalls.length > 0) {
                console.warn('Search queries used:', searchCalls.map((t) => t.toolRequest.args));
            }
        }
    });
});
//# sourceMappingURL=web_search.test.js.map