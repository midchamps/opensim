...
import { AuthType, OutputFormat } from '@opengame/opengame-core';
...
describe('validateNonInterActiveAuth', () => {
    ...
    let originalEnvGcp;
    let originalEnvOpenAiApiKey;
    let originalEnvQwenOauth;
    let originalEnvGoogleApiKey;
    let originalEnvAnthropicApiKey;
    let consoleErrorSpy;
    let processExitSpy;
    let refreshAuthMock;
    let mockSettings;
    ...
    it('uses configured QWEN_OAUTH if provided', async () => {
        ...
        await validateNonInteractiveAuth(AuthType.QWEN_OAUTH, undefined, nonInteractiveConfig, mockSettings);
    ...
    it('uses enforcedAuthType if provided', async () => {
        ...
        await validateNonInteractiveAuth(AuthType.USE_OPENAI, undefined, nonInteractiveConfig, mockSettings);
        expect(refreshAuthMock).toHaveBeenCalledWith(AuthType.USE_OPENAI);
    });
    it('exits if currentAuthType does not match enforcedAuthType', async () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('The configured auth type is qwen-oauth, but the current auth type is openai. Please re-authenticate with the correct type.');
        expect(processExitSpy).toHaveBeenCalledWith(1);
    });
    describe('JSON output mode', () => {
        let emitResultMock;
        let runExitCleanupMock;
        beforeEach(() => {
            ...
            vi.spyOn(cleanupModule, 'runExitCleanup').mockImplementation(runExitCleanupMock);
        });
        it('emits error result and exits when no auth is configured', async () => {
            ...
            expect(emitResultMock).toHaveBeenCalledWith({
                isError: true,
                errorMessage: expect.stringContaining('Please set an Auth method in your'),
                durationMs: 0,
                apiDurationMs: 0,
                numTurns: 0,
                usage: undefined,
        ...
        it('emits error result and exits when enforced auth mismatches current auth', async () => {
            ...
            expect(emitResultMock).toHaveBeenCalledWith({
                isError: true,
                errorMessage: expect.stringContaining('The configured auth type is qwen-oauth, but the current auth type is openai.'),
                durationMs: 0,
                apiDurationMs: 0,
                numTurns: 0,
                usage: undefined,
    ...
    describe('STREAM_JSON output mode', () => {
        ...
        beforeEach(() => {
            emitResultMock = vi.fn();
            runExitCleanupMock = vi.fn().mockResolvedValue(undefined);
            vi.spyOn(StreamJsonOutputAdapterModule, 'StreamJsonOutputAdapter').mockImplementation(() => ({
                emitResult: emitResultMock,
            }));
            vi.spyOn(cleanupModule, 'runExitCleanup').mockImplementation(runExitCleanupMock);
        });
