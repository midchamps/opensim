...
describe('JSON output', () => {
    ...
    it('should return a valid JSON array with result message containing response and stats', async () => {
        const result = await rig.run('What is the capital of France?', '--output-format', 'json');
    ...
    it('should return line-delimited JSON messages for stream-json output format', async () => {
        const result = await rig.run('What is the capital of France?', '--output-format', 'stream-json');
        ...
        for (const line of lines) {
            try {
                ...
                throw new Error(`Failed to parse JSON line: ${line}. Error: ${parseError}`);
            }
        }
        // Should have at least system, assistant, and result messages
        expect(messages.length).toBeGreaterThanOrEqual(3);
        // Find system message
        const systemMessage = messages.find((msg) => typeof msg === 'object' &&
            msg !== null &&
            'type' in msg &&
            msg.type === 'system');
        ...
        const assistantMessage = messages.find((msg) => typeof msg === 'object' &&
            msg !== null &&
            'type' in msg &&
            msg.type === 'assistant');
        ...
        expect(resultMessage).toBeDefined();
        expect(typeof resultMessage === 'object' &&
            resultMessage !== null &&
            'type' in resultMessage &&
            resultMessage.type === 'result').toBe(true);
        expect(resultMessage).toHaveProperty('is_error');
        expect(resultMessage.is_error).toBe(false);
        expect(resultMessage).toHaveProperty('result');
        expect(typeof resultMessage.result).toBe('string');
    ...
    it('should include stream events when using stream-json with include-partial-messages', async () => {
        const result = await rig.run('What is the capital of France?', '--output-format', 'stream-json', '--include-partial-messages');
        // Stream-json output is line-delimited JSON (one JSON object per line)
        const lines = result
            .trim()
            .split('\n')
        ...
        const streamEvents = messages.filter((msg) => typeof msg === 'object' &&
            msg !== null &&
            'type' in msg &&
        ...
        const eventTypes = streamEvents.map((event) => typeof event === 'object' &&
            event !== null &&
            'event' in event &&
    ...
    it('should return a JSON error for enforced auth mismatch before running', async () => {
        ...
        expect(stdoutMatch, 'Expected to find stdout in the error message').toBeTruthy();
        ...
        try {
            ...
            throw new Error(`Test failed: Could not parse JSON from stdout. Details: ${parseError}`);
        }
        // The output should be an array of messages
        expect(Array.isArray(parsed)).toBe(true);
        expect(parsed.length).toBeGreaterThan(0);
        ...
        expect(resultMessage.error?.message).toContain('configured auth type is qwen-oauth');
        expect(resultMessage.error?.message).toContain('current auth type is openai');
    });
});
