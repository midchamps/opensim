...
function buildSystemPrompt(library) {
    const familySection = library.families.length > 0
...
function buildExistingFamiliesSection(families) {
    const lines = ['## Existing Families in the Library\n'];
    for (const f of families) {
        const pp = f.physicsProfile;
        lines.push(`### "${f.archetype}" (stability: ${(f.stability * 100).toFixed(0)}%, projects: ${f.contributingProjects.length})`);
        lines.push(`- gravity: ${pp.hasGravity}, perspective: ${pp.perspective}, movement: ${pp.movementType}`);
        lines.push(`- summary: ${f.summary}`);
        lines.push('');
    }
    lines.push('If the new project clearly fits one of these, reuse its archetype name.');
    lines.push('Only create a new archetype if the physics regime is fundamentally different.\n');
    return lines.join('\n');
}
 ...
 */
export function classifyByPhysicsHeuristics(code, existingFamilies) {
    ...
    const detectedProfile = {
        ...
        perspective: best.signal.profileHint.perspective,
        movementType: best.signal.profileHint.movementType,
    };
    ...
    for (const family of existingFamilies) {
        ...
        if (fp.hasGravity === detectedProfile.hasGravity &&
            fp.perspective === detectedProfile.perspective &&
            fp.movementType === detectedProfile.movementType) {
            return {
                archetype: family.archetype,
                reasoning: `Heuristic match to existing family "${family.archetype}" ` +
    ...
    return {
        ...
        reasoning: `New regime detected via heuristic signal "${best.signal.name}" ` +
 ...
 */
export async function classifyProject(snapshot, library) {
    ...
    if (config.apiKey) {
        try {
            console.log(`[Classifier] Calling LLM (${config.modelName}), ` +
            ...
            console.log(`[Classifier] LLM result: "${result.archetype}" (confidence: ${result.confidence})`);
            return result;
        }
        catch (e) {
        ...
        console.log('[Classifier] No API key configured, using heuristic classification');
    }
    ...
    console.log(`[Classifier] Heuristic result: "${result.archetype}" (confidence: ${result.confidence})`);
    return result;
}
