...
import { parseExecutableSpec, prepareSpawnInfo, findNativeCliPath, } from '../../src/utils/cliPath.js';
// Mock fs module
vi.mock('node:fs');
...
describe('CLI Path Utilities', () => {
    ...
    describe('parseExecutableSpec', () => {
        describe('auto-detection (no spec provided)', () => {
            it('should auto-detect bundled CLI when no spec provided', () => {
                ...
                mockFs.existsSync.mockImplementation((p) => {
                    ...
                    return (pathStr.includes('cli/cli.js') || pathStr.includes('cli\\cli.js'));
                });
            ...
            it('should throw when bundled CLI not found', () => {
                ...
                expect(() => parseExecutableSpec()).toThrow('Bundled OpenGame CLI not found');
            });
        });
        describe('runtime prefix parsing', () => {
            ...
            it('should treat non-whitelisted runtime prefixes as command names', () => {
                ...
                expect(() => parseExecutableSpec('invalid:format')).toThrow('Invalid command name');
            });
            ...
            it('should throw when runtime-prefixed file does not exist', () => {
                mockFs.existsSync.mockReturnValue(false);
                expect(() => parseExecutableSpec('node:/nonexistent/cli.js')).toThrow('Executable file not found at');
            });
        });
        ...
        describe('file path resolution', () => {
            ...
            it('should throw when file path does not exist', () => {
                mockFs.existsSync.mockReturnValue(false);
                expect(() => parseExecutableSpec('/nonexistent/path')).toThrow('Executable file not found at');
            });
        });
    });
    describe('prepareSpawnInfo', () => {
        ...
        describe('TypeScript files', () => {
            ...
            it('should throw helpful error when tsx is not available', () => {
                ...
                expect(() => prepareSpawnInfo('/path/to/index.ts')).toThrow(`TypeScript file '${resolvedPath}' requires 'tsx' runtime, but it's not available`);
                expect(() => prepareSpawnInfo('/path/to/index.ts')).toThrow('Please install tsx: npm install -g tsx');
            });
        });
    ...
    describe('findNativeCliPath', () => {
        ...
        it('should throw descriptive error when bundled CLI not found', () => {
            mockFs.existsSync.mockReturnValue(false);
            expect(() => findNativeCliPath()).toThrow('Bundled OpenGame CLI not found');
        });
    });
    ...
    describe('error cases', () => {
        it('should provide helpful error for missing TypeScript file', () => {
            mockFs.existsSync.mockReturnValue(false);
            expect(() => prepareSpawnInfo('/missing/index.ts')).toThrow('Executable file not found at');
        });
        it('should provide helpful error for missing JavaScript file', () => {
            mockFs.existsSync.mockReturnValue(false);
            expect(() => prepareSpawnInfo('/missing/cli.js')).toThrow('Executable file not found at');
        });
        it('should treat non-whitelisted runtime prefixes as command names', () => {
            // With whitelist approach, 'invalid:spec' is not recognized as a runtime spec
            // so it's treated as a command name, which fails validation due to the colon
            expect(() => prepareSpawnInfo('invalid:spec')).toThrow('Invalid command name');
        });
        it('should handle Windows paths correctly even when file is missing', () => {
            mockFs.existsSync.mockReturnValue(false);
            expect(() => prepareSpawnInfo('D:\\missing\\cli.js')).toThrow('Executable file not found at');
            // Should not throw 'Invalid command name' error (which would happen if 'D:' was treated as invalid command)
            expect(() => prepareSpawnInfo('D:\\missing\\cli.js')).not.toThrow('Invalid command name');
        });
    });
    describe('comprehensive validation', () => {
        describe('runtime validation', () => {
            ...
            it('should validate runtime availability for explicit runtime specs', () => {
                ...
                expect(() => parseExecutableSpec('bun:/path/to/cli.js')).toThrow("Runtime 'bun' is not available on this system. Please install it first.");
            });
            ...
            it('should validate file extension matches runtime', () => {
                mockFs.existsSync.mockReturnValue(true);
                expect(() => parseExecutableSpec('tsx:/path/to/file.js')).toThrow("File extension '.js' is not compatible with runtime 'tsx'");
            });
            it('should validate node runtime with JavaScript files', () => {
                mockFs.existsSync.mockReturnValue(true);
                expect(() => parseExecutableSpec('node:/path/to/file.ts')).toThrow("File extension '.ts' is not compatible with runtime 'node'");
            });
            it('should accept valid runtime-file combinations', () => {
                mockFs.existsSync.mockReturnValue(true);
                expect(() => parseExecutableSpec('tsx:/path/to/file.ts')).not.toThrow();
                expect(() => parseExecutableSpec('node:/path/to/file.js')).not.toThrow();
                expect(() => parseExecutableSpec('bun:/path/to/file.mjs')).not.toThrow();
            });
        });
        describe('command name validation', () => {
            it('should reject empty command names', () => {
                expect(() => parseExecutableSpec('')).toThrow('Command name cannot be empty');
                expect(() => parseExecutableSpec('   ')).toThrow('Command name cannot be empty');
            });
            it('should reject invalid command name characters', () => {
                expect(() => parseExecutableSpec('qwen@invalid')).toThrow("Invalid command name 'qwen@invalid'. Command names should only contain letters, numbers, dots, hyphens, and underscores.");
        ...
        describe('file path validation', () => {
            ...
            it('should validate path points to a file, not directory', () => {
                ...
                expect(() => parseExecutableSpec('/path/to/directory')).toThrow('exists but is not a file');
            });
        ...
        describe('error message quality', () => {
            it('should provide helpful error for missing runtime-prefixed file', () => {
                mockFs.existsSync.mockReturnValue(false);
                expect(() => parseExecutableSpec('tsx:/missing/file.ts')).toThrow('Executable file not found at');
                expect(() => parseExecutableSpec('tsx:/missing/file.ts')).toThrow('Please check the file path and ensure the file exists');
            });
            it('should provide helpful error for missing regular file', () => {
                mockFs.existsSync.mockReturnValue(false);
                expect(() => parseExecutableSpec('/missing/file')).toThrow('Executable file not found at');
                expect(() => parseExecutableSpec('/missing/file')).toThrow('Please check the file path and ensure the file exists');
            });
        });
    });
});
//# sourceMappingURL=cliPath.test.js.map