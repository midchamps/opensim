...
describe('relaunchOnExitCode', () => {
    ...
    it('should handle runner errors', async () => {
        ...
        await expect(relaunchOnExitCode(runner)).rejects.toThrow('PROCESS_EXIT_CALLED');
        expect(runner).toHaveBeenCalledTimes(1);
        expect(consoleErrorSpy).toHaveBeenCalledWith('Fatal error: Failed to relaunch the CLI process.', error);
 ...
 */
function createMockChildProcess(exitCode = 0, autoClose = false) {
