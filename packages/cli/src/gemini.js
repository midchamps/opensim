...
import { InputFormat, logUserPrompt } from '@opengame/opengame-core';
...
export async function startInteractiveUI(config, settings, startupWarnings, workspaceRoot = process.cwd(), initializationResult) {
    ...
    const AppWrapper = () => {
        ...
        return (_jsx(SettingsContext.Provider, { value: settings, children: _jsx(KeypressProvider, { kittyProtocolEnabled: kittyProtocolStatus.enabled, config: config, debugKeystrokeLogging: settings.merged.general?.debugKeystrokeLogging, pasteWorkaround: process.platform === 'win32' || nodeMajorVersion < 20, children: _jsx(SessionStatsProvider, { sessionId: config.getSessionId(), children: _jsx(VimModeProvider, { settings: settings, children: _jsx(AppContainer, { config: config, settings: settings, startupWarnings: startupWarnings, version: version, initializationResult: initializationResult }) }) }) }) }));
    };
...
export async function main() {
    ...
    if (!process.env['SANDBOX']) {
        ...
        if (sandboxConfig) {
            const partialConfig = await loadCliConfig(settings.merged, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir()), argv);
    ...
    {
        const extensionEnablementManager = new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions);
        const extensions = loadExtensions(extensionEnablementManager);
        const config = await loadCliConfig(settings.merged, extensions, extensionEnablementManager, argv);
        ...
        const nonInteractiveConfig = await validateNonInteractiveAuth(argv.authType ||
            settings.merged.security?.auth?.selectedType, settings.merged.security?.auth?.useExternal, config, settings);
        const prompt_id = Math.random().toString(16).slice(2);
        if (inputFormat === InputFormat.STREAM_JSON) {
            const trimmedInput = (input ?? '').trim();
            await runNonInteractiveStreamJson(nonInteractiveConfig, trimmedInput.length > 0 ? trimmedInput : '');
