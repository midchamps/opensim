...
vi.mock('@opengame/opengame-core', async (importOriginal) => {
    ...
    return {
        ...
        getUnsupportedImageFormatWarning: vi.fn(() => 'Only the following image formats are supported: BMP, JPEG, JPG, PNG, TIFF, WEBP, HEIC. Other formats may not work as expected.'),
    };
});
...
describe('useVisionAutoSwitch helpers', () => {
    describe('shouldOfferVisionSwitch', () => {
        ...
        it('detects image when provided as a single Part object (non-array)', () => {
            ...
            const result = shouldOfferVisionSwitch(singleImagePart, AuthType.QWEN_OAUTH, 'qwen3-coder-plus', true);
            expect(result).toBe(true);
        });
    ...
    describe('getVisionSwitchGuidanceMessage', () => {
        it('returns the expected guidance message', () => {
            const vl = getDefaultVisionModel();
            const expected = 'To use images with your query, you can:\n' +
...
describe('useVisionAutoSwitch hook', () => {
    ...
    it('returns shouldProceed=true immediately for continuations', async () => {
        ...
        const { result } = renderHook(() => useVisionAutoSwitch(config, addItem, true, vi.fn()));
        const parts = [
            { inlineData: { mimeType: 'image/png', data: '...' } },
        ];
        let res;
