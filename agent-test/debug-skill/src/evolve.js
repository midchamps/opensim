 ...
 */
export async function evolveFromTrace(tracePath) {
    ...
    try {
        ...
        console.log(`  Project: ${trace.projectPath} | Iterations: ${trace.totalIterations} | ` +
...
export async function evolveBatch() {
    ...
    console.log(`Protocol v${protocol.version} | ` +
        `${protocol.entries.length} entries | ${protocol.rules.length} rules`);
    console.log(`New entries accumulated: ${totalNewEntries} | New rules: ${totalNewRules + newRules.length}`);
    console.log(`${'='.repeat(60)}\n`);
    return results;
}
/**
 ...
 */
export async function evolveInline(trace, protocol) {
