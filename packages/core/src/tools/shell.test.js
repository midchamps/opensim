...
describe('ShellTool', () => {
    ...
    describe('build', () => {
        ...
        it('should throw an error for a directory outside the workspace', () => {
            mockConfig.getWorkspaceContext.mockReturnValue(createMockWorkspaceContext('/test/dir', ['/another/workspace']));
            expect(() => shellTool.build({
                command: 'ls',
                directory: '/not/in/workspace',
                is_background: false,
            })).toThrow("Directory '/not/in/workspace' is not within any of the registered workspace directories.");
        });
    ...
    describe('execute', () => {
        ...
        it('should wrap command on linux and parse pgrep output', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith(wrappedCommand, '/test/dir', expect.any(Function), mockAbortSignal, false, {});
        ...
        it('should add ampersand to command when is_background is true and command does not end with &', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith(wrappedCommand, expect.any(String), expect.any(Function), mockAbortSignal, false, {});
        });
        ...
        it('should use the provided directory as cwd', async () => {
            mockConfig.getWorkspaceContext.mockReturnValue(createMockWorkspaceContext('/test/dir'));
            const invocation = shellTool.build({
                command: 'ls',
                directory: '/test/dir/subdir',
                is_background: false,
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith(wrappedCommand, '/test/dir/subdir', expect.any(Function), mockAbortSignal, false, {});
        });
        it('should not wrap command on windows', async () => {
            ...
            await promise;
            expect(mockShellExecutionService).toHaveBeenCalledWith('dir', '/test/dir', expect.any(Function), mockAbortSignal, false, {});
        });
        ...
        describe('addCoAuthorToGitCommit', () => {
            it('should add co-author to git commit with double quotes', async () => {
                ...
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('Co-authored-by: Qwen-Coder <qwen-coder@alibabacloud.com>'), expect.any(String), expect.any(Function), mockAbortSignal, false, {});
            });
            ...
            it('should not modify non-git commands', async () => {
                ...
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('npm install'), expect.any(String), expect.any(Function), mockAbortSignal, false, {});
            });
            it('should not modify git commands without -m flag', async () => {
                ...
                await promise;
                // On Linux, commands are wrapped with pgrep functionality
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('git commit'), expect.any(String), expect.any(Function), mockAbortSignal, false, {});
            });
            ...
            it('should not add co-author when disabled in config', async () => {
                ...
                await promise;
                // On Linux, commands are wrapped with pgrep functionality
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('git commit -m "Initial commit"'), expect.any(String), expect.any(Function), mockAbortSignal, false, {});
            });
            it('should use custom name and email from config', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('Co-authored-by: Custom Bot <custom@example.com>'), expect.any(String), expect.any(Function), mockAbortSignal, false, {});
            });
