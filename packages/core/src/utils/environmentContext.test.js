...
import { getEnvironmentContext, getDirectoryContextString, getInitialChatHistory, } from './environmentContext.js';
...
describe('getInitialChatHistory', () => {
    ...
    it('includes startup context when skipStartupContext is false', async () => {
        ...
        expect(history).toEqual([
            expect.objectContaining({
                ...
                parts: [
                    expect.objectContaining({
                        text: expect.stringContaining("I'm currently working in the directory"),
                    }),
                ],
            }),
            {
    ...
    it('returns only extra history when skipStartupContext is true', async () => {
        ...
        mockConfig.getWorkspaceContext = vi.fn(() => {
            throw new Error('getWorkspaceContext should not be called when skipping startup context');
        });
        mockConfig.getFullContext = vi.fn(() => {
            throw new Error('getFullContext should not be called when skipping startup context');
        });
        mockConfig.getToolRegistry = vi.fn(() => {
            throw new Error('getToolRegistry should not be called when skipping startup context');
        });
        const extraHistory = [
            { role: 'user', parts: [{ text: 'custom context' }] },
        ];
        const history = await getInitialChatHistory(mockConfig, extraHistory);
        expect(mockConfig.getSkipStartupContext).toHaveBeenCalled();
