...
describe('OpenAIContentConverter', () => {
    ...
    describe('convertGeminiRequestToOpenAI', () => {
        const createRequestWithFunctionResponse = (response) => {
            const contents = [
                {
                    role: 'model',
                    parts: [
    ...
    describe('OpenAI -> Gemini reasoning content', () => {
        it('should convert reasoning_content to a thought part for non-streaming responses', () => {
            ...
            expect(parts?.[0]).toEqual(expect.objectContaining({ thought: true, text: 'chain-of-thought' }));
            expect(parts?.[1]).toEqual(expect.objectContaining({ text: 'final answer' }));
        });
        it('should convert streaming reasoning_content delta to a thought part', () => {
            ...
            expect(parts?.[0]).toEqual(expect.objectContaining({ thought: true, text: 'thinking...' }));
            expect(parts?.[1]).toEqual(expect.objectContaining({ text: 'visible text' }));
        });
    });
    ...
    describe('convertGeminiToolParametersToOpenAI', () => {
        ...
        it('should return undefined for null or non-object input', () => {
            expect(converter.convertGeminiToolParametersToOpenAI(null)).toBeNull();
            expect(converter.convertGeminiToolParametersToOpenAI(undefined)).toBeUndefined();
        });
