...
export function getBuiltinRipgrep() {
    ...
    const vendorPath = isBundled
        ? path.join(__dirname, 'vendor', 'ripgrep', `${arch}-${platform}`, binaryName)
        : path.join(__dirname, '..', '..', '..', 'vendor', 'ripgrep', `${arch}-${platform}`, binaryName);
    return vendorPath;
}
/**
 ...
 */
export async function runRipgrep(args, signal) {
    ...
    return new Promise((resolve) => {
        const child = execFile(selection.command, args, {
        ...
        // Handle spawn errors
        child.on('error', (err) => resolve({ stdout: '', truncated: false, error: err }));
    });
}
