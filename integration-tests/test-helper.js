...
export function validateModelOutput(result, expectedContent = null, testName = '') {
    ...
    if (expectedContent) {
        ...
        if (missingContent.length > 0) {
            console.warn(`Warning: LLM did not include expected content in response: ${missingContent.join(', ')}.`, 'This is not ideal but not a test failure.');
...
export async function type(ptyProcess, text) {
    const delay = 5;
...
export class TestRig {
    ...
    getDefaultTimeout() {
        if (env['CI'])
            return 60000; // 1 minute in CI
        if (env['GEMINI_SANDBOX'])
    ...
    setup(testName, options = {}) {
        ...
        writeFileSync(join(qwenDir, 'settings.json'), JSON.stringify(settings, null, 2));
    }
    ...
    _getCommandAndArgs(extraInitialArgs = []) {
        const isNpmReleaseTest = process.env.INTEGRATION_TEST_USE_INSTALLED_GEMINI === 'true';
        ...
        const promise = new Promise((resolve, reject) => {
            child.on('close', (code) => {
                if (code === 0) {
                    ...
                    const isJsonOutput = commandArgs.includes('--output-format') &&
                    ...
                    const isJsonOutputOnError = commandArgs.includes('--output-format') &&
                        (commandArgs.includes('json') ||
                            commandArgs.includes('stream-json'));
    ...
    async waitForTelemetryEvent(eventName, timeout) {
        ...
        return this.poll(() => {
            ...
            return logs.some((logData) => logData.attributes &&
    ...
    async waitForText(text, timeout) {
        ...
        return this.poll(() => stripAnsi(this._interactiveOutput)
            .toLowerCase()
            .includes(text.toLowerCase()), timeout, 200);
    }
    runInteractive(...args) {
        const { command, initialArgs } = this._getCommandAndArgs(['--yolo']);
        const commandArgs = [...initialArgs, ...args];
