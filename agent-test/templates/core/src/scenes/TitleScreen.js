...
export class TitleScreen extends Phaser.Scene {
    ...
    createBackground() {
        ...
        if (this.textures.exists(bgKey)) {
            ...
            const overlay = this.add.rectangle(cam.width / 2, cam.height / 2, cam.width, cam.height, 0x000000, 0.3);
            overlay.setDepth(1);
        }
        else {
            // Fallback: solid dark background when no title_bg asset is available
            this.add
                .rectangle(cam.width / 2, cam.height / 2, cam.width, cam.height, 0x1a1a2e, 1)
                .setDepth(0);
        }
    }
    /**
