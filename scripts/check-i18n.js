 ...
 */
async function loadTranslationsFile(filePath) {
    try {
        ...
        try {
            const content = fs.readFileSync(filePath.replace('.js', '.json'), 'utf-8');
 ...
 */
function extractStringLiteral(content, startPos, quote) {
 ...
 */
async function extractUsedKeys(sourceDir) {
 ...
 */
function checkKeyValueConsistency(enTranslations) {
    const errors = [];
 ...
 */
function checkKeyMatching(enTranslations, zhTranslations) {
    const errors = [];
 ...
 */
function saveKeysOnlyInLocalesToJson(keysOnlyInLocales, outputPath) {
    try {
        const data = {
 ...
 */
async function findKeysOnlyInLocales(unusedKeys, sourceDir, localesDir) {
    if (unusedKeys.length === 0) {
        return [];
    }
    const keysOnlyInLocales = [];
...
async function checkI18n() {
    ...
    try {
        ...
        errors.push(`Failed to load en.js: ${error instanceof Error ? error.message : String(error)}`);
        return {
            success: false,
            errors,
            warnings,
    ...
    try {
        ...
        errors.push(`Failed to load zh.js: ${error instanceof Error ? error.message : String(error)}`);
        return {
            success: false,
            errors,
            warnings,
    ...
    const unusedKeysOnlyInLocales = unusedKeys.length > 0
...
async function main() {
    ...
    const coverage = result.stats.totalKeys > 0
        ? ((result.stats.translatedKeys / result.stats.totalKeys) * 100).toFixed(1)
        : '0.0';
    ...
    if (result.warnings.length > 0) {
        ...
        // Show unused keys
        if (result.stats.unusedKeys.length > 0 &&
        ...
        if (result.stats.unusedKeysOnlyInLocales &&
            ...
            console.log('   Please review these keys - they might be safe to remove.');
            result.stats.unusedKeysOnlyInLocales.forEach((key) => console.log(`  - "${key}"`));
            // Save these keys to a JSON file
            const outputPath = path.join(__dirname, 'unused-keys-only-in-locales.json');
            saveKeysOnlyInLocalesToJson(result.stats.unusedKeysOnlyInLocales, outputPath);
        }
        console.log();
    }
...
//# sourceMappingURL=check-i18n.js.map