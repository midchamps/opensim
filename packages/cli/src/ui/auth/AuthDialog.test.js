...
const renderAuthDialog = (settings, uiStateOverrides = {}, uiActionsOverrides = {}) => {
    const uiState = createMockUIState(uiStateOverrides);
    const uiActions = createMockUIActions(uiActionsOverrides);
    return renderWithProviders(_jsx(UIStateContext.Provider, { value: uiState, children: _jsx(UIActionsContext.Provider, { value: uiActions, children: _jsx(AuthDialog, {}) }) }), { settings });
};
describe('AuthDialog', () => {
    ...
    it('should prevent exiting when no auth method is selected and show error message', async () => {
        ...
        const { lastFrame, stdin, unmount } = renderAuthDialog(settings, {}, { handleAuthSelect });
        await wait();
        // Simulate pressing escape key
        stdin.write('\u001b'); // ESC key
        await wait();
        // Should show error message instead of calling handleAuthSelect
        expect(lastFrame()).toContain('You must select an auth method to proceed. Press Ctrl+C again to exit.');
    ...
    it('should not exit if there is already an error message', async () => {
        ...
        const { lastFrame, stdin, unmount } = renderAuthDialog(settings, { authError: 'Initial error' }, { handleAuthSelect });
        await wait();
    ...
    it('should allow exiting when auth method is already selected', async () => {
        ...
        const { stdin, unmount } = renderAuthDialog(settings, {}, { handleAuthSelect });
        await wait();
        // Simulate pressing escape key
        stdin.write('\u001b'); // ESC key
        await wait();
