...
export const M0_GAME_CONFIG_PATH = path.join(META_TEMPLATE_PATH, 'core', 'src', 'gameConfig.json');
...
export function getClassifierConfig() {
    return {
        ...
        modelName: env('CLASSIFIER_MODEL_NAME') ||
...
export function getAbstractorConfig() {
    return {
        apiKey: env('REASONING_MODEL_API_KEY') ||
            env('DASHSCOPE_API_KEY') ||
            env('OPENAI_API_KEY'),
        baseUrl: env('REASONING_MODEL_BASE_URL') ||
            env('OPENAI_BASE_URL') ||
            'https://dashscope-intl.aliyuncs.com/compatible-mode/v1',
        modelName: env('ABSTRACTOR_MODEL_NAME') ||
            env('REASONING_MODEL_NAME') ||
            'qwen-plus',
        temperature: 0.5,
        maxTokens: 8000,
