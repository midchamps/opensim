 ...
 */
function generateFamilyId(archetype) {
 ...
 */
function mergeConfigFields(existing, incoming) {
    const map = new Map();
 ...
 */
function mergeTemplateFiles(existing, incoming) {
    const map = new Map();
 ...
 */
function computeStability(contributingProjects) {
    return Math.min(1.0, contributingProjects.length / 5);
}
// -----------------------------------------------------------------------------
// Create a new family from scratch
// -----------------------------------------------------------------------------
function createNewFamily(abstracted, patterns, taskNumber) {
    ...
    return {
        ...
        baseClasses: patterns.classes.filter((c) => c.isAbstract || c.name.startsWith('Base')),
...
// -----------------------------------------------------------------------------
function mergeIntoFamily(family, abstracted, patterns) {
    const contributingProjects = family.contributingProjects.includes(patterns.projectPath)
    ...
    for (const [dir, files] of Object.entries(patterns.fileStructure.filesByDirectory)) {
    ...
    return {
        ...
        configExtensions: mergeConfigFields(family.configExtensions, abstracted.configSchema),
        templateFiles: mergeTemplateFiles(family.templateFiles, abstracted.templateFiles),
 ...
 */
export function mergeIntoLibrary(library, abstracted, patterns) {
    const taskNumber = library.evolutionLog.length + 1;
    const existingFamily = library.families.find((f) => f.archetype === patterns.archetype);
    let action;
    let familyId;
    if (existingFamily) {
        // Merge into existing family
        console.log(`[Merger] Merging into existing family: ${existingFamily.id} (${existingFamily.archetype})`);
        ...
        // Create new family
        console.log(`[Merger] Creating new family for archetype: ${patterns.archetype}`);
    ...
    console.log(`[Merger] ${action}: ${familyId} | Library v${library.version} | ` +
