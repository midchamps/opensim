...
import { isCommandAvailable, execCommand } from '@opengame/opengame-core';
// Mock @opengame/opengame-core
vi.mock('@opengame/opengame-core', async () => {
    const original = await vi.importActual('@opengame/opengame-core');
    return {
        ...original,
        execCommand: vi.fn(),
...
describe('useGitBranchName', () => {
    ...
    it('should return undefined if git command fails', async () => {
        execCommand.mockRejectedValue(new Error('Git error'));
        const { result, rerender } = renderHook(() => useGitBranchName(CWD));
    ...
    it('should return short commit hash if branch is HEAD (detached state)', async () => {
        execCommand.mockImplementation(async (_command, args) => {
