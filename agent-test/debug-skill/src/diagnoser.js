...
import { getDiagnoserConfig, SIGNATURE_MATCH_THRESHOLD } from './config.js';
/**
 * Diagnose a set of parsed errors using the protocol.
 * Returns diagnoses in the same order as input errors.
 */
export async function diagnoseErrors(errors, protocol, projectDir) {
    const results = [];
 ...
 */
async function diagnoseSingleError(error, protocol, projectDir) {
    ...
    try {
        ...
        console.warn(`[Diagnoser] LLM fallback failed: ${e instanceof Error ? e.message : String(e)}`);
    }
...
// -----------------------------------------------------------------------------
function matchAgainstProtocol(error, protocol) {
 ...
 */
function computeMatchScore(error, signature) {
    let score = 0;
    let weights = 0;
...
async function llmDiagnose(error, protocol, projectDir) {
...
function buildDiagnoserSystemPrompt(protocol) {
...
function buildDiagnoserUserPrompt(error, projectDir) {
...
function parseLLMDiagnosis(content, error) {
    try {
        ...
        const stage = (['build', 'test', 'runtime'].includes(parsed['stage'])
            ? parsed['stage']
            : 'build');
        return {
            entry: {
                ...
                signature: {
                    stage,
                    errorCode: parsed['errorCode'] || error.code,
                    messagePattern: parsed['messagePattern'] || error.message,
                    fileContext: error.file,
                },
                rootCause: parsed['rootCause'] || 'Unknown',
                tags: parsed['tags'] || [],
                fix: {
                    type: (['edit', 'shell', 'config', 'delete', 'create'].includes(parsed['fixType'])
                        ? parsed['fixType']
                        : 'edit'),
                    description: parsed['fixDescription'] || '',
                    patch: parsed['fixPatch'] || '',
                },
