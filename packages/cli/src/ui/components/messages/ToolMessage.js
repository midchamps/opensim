...
export const ToolMessage = ({ name, description, resultDisplay, status, availableTerminalHeight, terminalWidth, emphasis = 'medium', renderOutputAsMarkdown = true, activeShellPtyId, embeddedShellFocused, ptyId, config, }) => {
    ...
    return (_jsxs(Box, { paddingX: 1, paddingY: 0, flexDirection: "column", children: [_jsxs(Box, { minHeight: 1, children: [_jsx(ToolStatusIndicator, { status: status, name: name }), _jsx(ToolInfo, { name: name, status: status, description: description, emphasis: emphasis }), shouldShowFocusHint && (_jsx(Box, { marginLeft: 1, flexShrink: 0, children: _jsx(Text, { color: theme.text.accent, children: isThisShellFocused ? '(Focused)' : '(ctrl+f to focus)' }) })), emphasis === 'high' && _jsx(TrailingIndicator, {})] }), displayRenderer.type !== 'none' && (_jsx(Box, { paddingLeft: STATUS_INDICATOR_WIDTH, width: "100%", marginTop: 1, children: _jsxs(Box, { flexDirection: "column", children: [displayRenderer.type === 'todo' && (_jsx(TodoResultRenderer, { data: displayRenderer.data })), displayRenderer.type === 'plan' && (_jsx(PlanResultRenderer, { data: displayRenderer.data, availableHeight: availableHeight, childWidth: childWidth })), displayRenderer.type === 'task' && config && (_jsx(SubagentExecutionRenderer, { data: displayRenderer.data, availableHeight: availableHeight, childWidth: childWidth, config: config })), displayRenderer.type === 'diff' && (_jsx(DiffResultRenderer, { data: displayRenderer.data, availableHeight: availableHeight, childWidth: childWidth })), displayRenderer.type === 'ansi' && (_jsx(AnsiOutputText, { data: displayRenderer.data, availableTerminalHeight: availableHeight })), displayRenderer.type === 'string' && (_jsx(StringResultRenderer, { data: displayRenderer.data, renderAsMarkdown: renderOutputAsMarkdown, availableHeight: availableHeight, childWidth: childWidth }))] }) })), isThisShellFocused && config && (_jsx(Box, { paddingLeft: STATUS_INDICATOR_WIDTH, marginTop: 1, children: _jsx(ShellInputPrompt, { activeShellPtyId: activeShellPtyId ?? null, focus: embeddedShellFocused }) }))] }));
};
