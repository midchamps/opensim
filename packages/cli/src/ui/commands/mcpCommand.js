...
import { DiscoveredMCPTool, getMCPDiscoveryState, getMCPServerStatus, MCPDiscoveryState, MCPServerStatus, getErrorMessage, MCPOAuthTokenStorage, MCPOAuthProvider, } from '@opengame/opengame-core';
...
const authCommand = {
    ...
    action: async (context, args) => {
        ...
        try {
            ...
            context.ui.addItem({
                type: 'info',
                text: t("Successfully authenticated and refreshed tools for '{{name}}'.", {
                    name: serverName,
                }),
            }, Date.now());
            ...
            return {
                type: 'message',
                messageType: 'info',
                content: t("Successfully authenticated and refreshed tools for '{{name}}'.", {
                    name: serverName,
                }),
            };
        }
            ...
            return {
                type: 'message',
                messageType: 'error',
                content: t("Failed to authenticate with MCP server '{{name}}': {{error}}", {
                    name: serverName,
...
export const mcpCommand = {
    name: 'mcp',
    get description() {
        return t('list configured MCP servers and tools, or authenticate with OAuth-enabled servers');
    },
    kind: CommandKind.BUILT_IN,
