...
export class OpenAIContentConverter {
  ...
  /**
   * Gemini 3 multi-turn requires the model's returned `thought_signature` on
   * each tool_call to be echoed back on the assistant message in subsequent
   * turns. The OpenAI SDK deserializes responses into a typed shape but keeps
   * unknown fields (like `extra_content`) as excess properties. This map
   * remembers signatures keyed by tool_call id so they can be reattached on
   * request re-serialization.
   */
  private static toolCallThoughtSignatures: Map<string, string> = new Map();
  ...
  private static rememberToolCallSignature(
    id: string | undefined,
    signature: string | undefined,
  ): void {
    if (!id || !signature) return;
    OpenAIContentConverter.toolCallThoughtSignatures.set(id, signature);
  }

  private static recallToolCallSignature(
    id: string | undefined,
  ): string | undefined {
    if (!id) return undefined;
    return OpenAIContentConverter.toolCallThoughtSignatures.get(id);
  }
  ...
  private processContent(
    ...
    if (content.role === 'model' && parsedParts.functionCalls.length > 0) {
      const toolCalls = parsedParts.functionCalls.map((fc, index) => {
        ...
        // Gemini 3 requires echoing the `thought_signature` the model
        // returned on the previous turn. Pull it from our signature map and
        // attach it via `extra_content`, which the OpenAI-compat shim
        // forwards to Gemini as a native functionCall part field.
        const signature =
          OpenAIContentConverter.recallToolCallSignature(id);
        const base: {
          id: string;
          type: 'function';
    ...
    if (choice.message.tool_calls) {
      for (const toolCall of choice.message.tool_calls) {
        if (toolCall.function) {
          ...
          // Gemini 3 returns a `thought_signature` inside
          // `extra_content.google.thought_signature` on each tool_call.
          // We must preserve this across turns — Gemini rejects follow-up
          // requests (400) unless the same signature is echoed back on the
          // assistant's function call. Cast to access the excess field the
          // OpenAI SDK keeps on the parsed object.
          const extraContent = (
            toolCall as {
              extra_content?: { google?: { thought_signature?: string } };
            }
          ...
          const thoughtSignature =
            extraContent?.google?.thought_signature ?? undefined;
          OpenAIContentConverter.rememberToolCallSignature(
            toolCall.id,
            thoughtSignature,
          );
    ...
    if (choice) {
      ...
      if (choice.delta?.tool_calls) {
        for (const toolCall of choice.delta.tool_calls) {
          ...
          // Gemini 3 sometimes places `extra_content.google.thought_signature`
          // on a specific delta chunk; capture it whenever it appears so we
          // can echo it back on the next turn.
          const extraContent = (
            toolCall as {
              extra_content?: { google?: { thought_signature?: string } };
            }
      ...
      if (choice.finish_reason) {
        ...
        for (const toolCall of completedToolCalls) {
          if (toolCall.name) {
            ...
            const sig = OpenAIContentConverter.recallToolCallSignature(id);
            parts.push({
              functionCall: {
                id,
                name: toolCall.name,
