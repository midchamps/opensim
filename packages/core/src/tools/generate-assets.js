...
class GenerateAssetsInvocation extends BaseToolInvocation {
    ...
    constructor(config, params) {
        ...
        const bgBackend = process.env.BACKGROUND_REMOVAL_BACKEND || 'imgly';
    ...
    async execute(signal) {
        ...
        const targetDirName = this.params.output_dir_name || path.join('public', 'assets');
            ...
            const task = (async () => {
                try {
                    switch (assetReq.type) {
                        case 'background':
                            await this.handleBackground(assetReq, assetPack, absoluteAssetsDir, signal);
                            break;
                        case 'image':
                            await this.handleImage(assetReq, assetPack, absoluteAssetsDir, signal);
                            break;
                        case 'animation':
                            await this.handleAnimation(assetReq, assetPack, absoluteAssetsDir, signal);
                            break;
                        case 'audio':
                            await this.handleAudio(assetReq, assetPack, absoluteAssetsDir, signal);
                            break;
                        case 'tileset':
                            await this.handleTileset(assetReq, assetPack, absoluteAssetsDir, signal);
                            break;
                        default:
                            console.warn(`[GenerateAssets] Unknown asset type: ${assetReq.type}`);
                            throw new Error(`Unsupported asset type: ${assetReq.type}. Supported types: background, image, animation, audio, tileset`);
                    }
...
`;
    ...
    // ============== Asset Handlers ==============
    async handleBackground(req, assetPack, assetsDir, signal) {
        if (signal.aborted)
    ...
    `;
        const imageUrl = await this.modelRouter.generateImage(prompt, req.resolution || '1024*1024');
    ...
    async handleImage(req, assetPack, assetsDir, signal) {
        if (signal.aborted)
            throw new Error('Aborted');
    ...
    `;
        const imageUrl = await this.modelRouter.generateImage(prompt, req.size || '1024*1024');
        // Remove background
    ...
    async handleAnimation(req, assetPack, assetsDir, signal) {
        if (signal.aborted)
            throw new Error('Aborted');
    ...
    `;
        const baseImageUrl = await this.modelRouter.generateImage(basePrompt, '1024*1024');
        // Save base/idle frame
        const baseKey = `${req.key}_idle_01`;
        const baseBuffer = await this.bgRemovalService.removeBackgroundSafe(baseImageUrl);
        ...
        if (useI2V) {
            ...
            await this.generateAnimationViaI2V(req, baseImageUrl, assetPack, assetsDir, signal);
        }
        else {
            // Use I2I (Image-to-Image) approach - may have OSS accessibility issues
            console.warn(`[GenerateAssets] Using I2I mode - may have OSS network issues`);
            await this.generateAnimationViaI2I(req, baseImageUrl, assetPack, assetsDir, signal);
        }
    }
    async generateAnimationViaI2V(req, baseImageUrl, assetPack, assetsDir, signal) {
        // Check if ffmpeg is available for local frame extraction
        const ffmpegAvailable = await this.frameExtractionService.isFFmpegAvailable();
        if (!ffmpegAvailable) {
            console.warn(`[GenerateAssets] FFmpeg not available, falling back to I2I mode`);
            await this.generateAnimationViaI2I(req, baseImageUrl, assetPack, assetsDir, signal);
            return;
        }
        ...
        for (const animation of req.animations) {
            if (signal.aborted)
                break;
            if (animation.name === 'idle' && animation.frameCount <= 1)
                continue;
            console.log(`[GenerateAssets] Generating I2V animation: ${animation.name}`);
            try {
        ...
        `;
                ...
                const { videoUrl } = await this.modelRouter.generateVideo(baseImageUrl, videoPrompt, '480P');
                console.log(`[GenerateAssets] Video generated: ${videoUrl.substring(0, 50)}...`);
                ...
                console.log(`[GenerateAssets] Step 2: Extracting frames locally via ffmpeg (firstLastOnly=${firstLastOnly})...`);
                const result = await this.frameExtractionService.extractFramesLocal(videoUrl, animation.frameCount, 0, // fromTime
                ...
                console.log(`[GenerateAssets] Extracted ${framePaths.length} frames locally (firstLastOnly=${firstLastOnly})`);
                ...
                console.log(`[GenerateAssets] Step 3: Processing ${framePaths.length} frames...`);
                for (let i = 0; i < framePaths.length; i++) {
                    ...
                    const processedBuffer = await this.bgRemovalService.removeBackgroundFromBuffer(frameBuffer);
                    const frameCdnUrl = await this.saveAsset(processedBuffer, frameKey, assetsDir);
                    ...
                    console.log(`[GenerateAssets] Saved frame ${i + 1}/${framePaths.length}: ${frameKey}`);
                }
                // Step 4: Extract audio from animation video (optional)
                try {
                    console.log(`[GenerateAssets] Step 4: Extracting audio from animation video (first 2s)...`);
                    const audioPath = await this.frameExtractionService.extractAudio(videoUrl, undefined, 0, 2);
                    ...
                    const audioCdnUrl = await this.saveAsset(audioBuffer, audioKey, assetsDir, 'wav');
                    this.updateAssetPack(assetPack, audioKey, 'audio', audioCdnUrl);
                    console.log(`[GenerateAssets] Extracted animation audio: ${audioKey}.wav`);
                }
                catch (error) {
                    console.log(`[GenerateAssets] No audio extracted from animation (might be silent video): ${error}`);
                }
                console.log(`[GenerateAssets] I2V animation ${animation.name} completed!`);
            }
            catch (error) {
                console.warn(`[GenerateAssets] I2V failed for ${animation.name}: ${error}`);
                console.log(`[GenerateAssets] Falling back to I2I for ${animation.name}...`);
                await this.generateSingleAnimationI2I(req, animation, baseImageUrl, assetPack, assetsDir, signal);
            }
        }
    }
    async generateAnimationViaI2I(req, baseImageUrl, assetPack, assetsDir, signal) {
        for (const animation of req.animations) {
            if (signal.aborted)
                break;
            await this.generateSingleAnimationI2I(req, animation, baseImageUrl, assetPack, assetsDir, signal);
    ...
    async generateSingleAnimationI2I(req, animation, baseImageUrl, assetPack, assetsDir, signal) {
        ...
        for (let i = 1; i <= animation.frameCount; i++) {
            ...
            for (let retry = 0; retry < MAX_RETRIES; retry++) {
                try {
                    if (retry > 0) {
                        console.log(`[GenerateAssets] Retry ${retry}/${MAX_RETRIES} for frame ${frameKey}...`);
                        await this.sleep(RETRY_DELAY * retry); // Exponential backoff
                    }
                    frameUrl = await this.modelRouter.editImage(baseImageUrl, framePrompt, previousFrameUrl);
                    break; // Success, exit retry loop
                }
                catch (error) {
                    lastError = error;
                    console.warn(`[GenerateAssets] I2I failed (attempt ${retry + 1}/${MAX_RETRIES}): ${lastError.message}`);
            ...
            if (!frameUrl) {
                console.warn(`[GenerateAssets] Skipping frame ${frameKey} after ${MAX_RETRIES} retries`);
                continue; // Skip this frame instead of failing entire animation
            }
            const frameBuffer = await this.bgRemovalService.removeBackgroundSafe(frameUrl);
            const frameCdnUrl = await this.saveAsset(frameBuffer, frameKey, assetsDir);
            this.updateAssetPack(assetPack, frameKey, 'animation', frameCdnUrl);
    ...
    async handleAudio(req, assetPack, assetsDir, signal) {
        if (signal.aborted)
            throw new Error('Aborted');
        console.log(`[GenerateAssets] Generating audio: ${req.key} (${req.audioType})`);
        console.log(`[GenerateAssets] Audio request details: duration=${req.duration}, genre=${req.genre}, tempo=${req.tempo}`);
        console.log(`[GenerateAssets] Output directory: ${assetsDir}`);
        const duration = req.audioType === 'sfx' ? req.duration || 1 : req.duration || 5;
        ...
        if (ffmpegAvailable) {
            try {
                console.log(`[GenerateAssets] Step 1: Generating video for audio extraction (T2V)...`);
                const videoPrompt = `
          ${req.description}.
          Music visualization, abstract, ${req.genre || 'ambient'}, ${req.tempo || 'medium'} tempo.
          High quality audio, clear sound.
        `;
                const { videoUrl } = await this.modelRouter.generateVideoFromText(videoPrompt, '720P');
                console.log(`[GenerateAssets] Video generated: ${videoUrl.substring(0, 50)}...`);
                console.log(`[GenerateAssets] Step 2: Extracting audio from video...`);
                const audioPath = await this.frameExtractionService.extractAudio(videoUrl, undefined, 0, 7);
                ...
                console.warn(`[GenerateAssets] Video-based audio generation failed: ${error}`);
            }
        }
        // Strategy 2: ABC notation → WAV via symusic
        if (!buffer) {
            console.log(`[GenerateAssets] Trying ABC notation via LLM...`);
            let abcNotation = null;
            try {
                abcNotation = await this.modelRouter.generateABC(req);
                console.log(`[GenerateAssets] ABC notation generated (${abcNotation.length} chars)`);
            }
            catch (error) {
            ...
            if (abcNotation) {
                try {
                    buffer = await this.modelRouter.generateAudioFromABC(abcNotation, audioType, duration);
        ...
        if (!buffer) {
            console.log(`[GenerateAssets] Using procedural audio generation (duration=${duration}s, type=${audioType})...`);
            try {
                buffer = await this.modelRouter.generatePlaceholderAudio(duration, audioType);
                console.log(`[GenerateAssets] Procedural audio generated successfully (${buffer.length} bytes)`);
            }
            catch (error) {
                console.error(`[GenerateAssets] Procedural audio FAILED: ${error}`);
                throw new Error(`Audio generation failed for ${req.key}: All methods failed. Error: ${error}`);
            }
        }
        ...
        console.log(`[GenerateAssets] Audio file written: ${req.key}.wav (${buffer.length} bytes)`);
        this.updateAssetPack(assetPack, req.key, 'audio', cdnUrl);
        console.log(`[GenerateAssets] Audio saved: ${req.key}.wav (${audioType}, ${duration}s)`);
    }
    // ============== Tileset Generation ==============
    async handleTileset(req, assetPack, assetsDir, signal) {
        if (signal.aborted)
            throw new Error('Aborted');
        ...
        const OUTPUT_GRID = 7;
        console.log(`[GenerateAssets] Generating tileset: ${req.key} using 9-Slice (3x3 -> 7x7) Strategy`);
...
isometric, 3D, perspective, vignette, dark corners.
    `;
        ...
        const isPixelArt = (this.params.style_anchor || '')
            .toLowerCase()
            .includes('pixel');
        const finalBuffer = await this.tilesetProcessor.expand3x3To7x7(rawBuffer, TILE_SIZE, isPixelArt);
        // 4. Save
        ...
        console.log(`[GenerateAssets] Tileset saved: ${req.key}.png (${OUTPUT_GRID}x${OUTPUT_GRID} grid, ${TILE_SIZE}px tiles)`);
    }
    // ============== Helper Methods ==============
    async loadAssetPack(packPath) {
        try {
    ...
    async saveAsset(buffer, key, assetsDir, extension = 'png') {
        const filename = `${key}.${extension}`;
        const filePath = path.join(assetsDir, filename);
        await fs.writeFile(filePath, buffer);
        return `assets/${filename}`;
    }
    getSectionName(assetType, key) {
        switch (assetType) {
            case 'background':
                return 'backgrounds';
            case 'animation': {
    ...
    updateAssetPack(pack, key, assetType, url) {
...
export class GenerateAssetsTool extends BaseDeclarativeTool {
    config;
    static Name = ToolNames.GENERATE_ASSETS;
    constructor(config) {
        super(ToolNames.GENERATE_ASSETS, ToolDisplayNames.GENERATE_ASSETS, `Generates game assets (images, animations, audio) using AI models.
       ...
       removal, I2V animation generation, and ABC-based music generation.`, Kind.Fetch, {
            type: 'object',
            properties: {
                style_anchor: {
                    type: 'string',
                    description: 'Global visual style (e.g., "16-bit pixel art, vibrant colors")',
                },
                composition_env: {
                    type: 'string',
                    description: 'Composition hints (e.g., "white background, centered, same size, no offset")',
                },
                output_dir_name: {
                    type: 'string',
                ...
                model_type: {
                    ...
                    description: 'Optional hint for the asset-generation provider family. ' +
                        ...
                        oneOf: [
                            ...
                            },
                            {
                                ...
                                properties: {
                                    ...
                                    description: {
                                        type: 'string',
                                        description: 'Tileset theme/style (e.g., "jungle terrain", "dungeon walls")',
                                    },
                                    tileset_size: {
                                        type: 'number',
