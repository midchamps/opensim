 ...
 */
export async function debugLoop(options) {
    const { projectDir, maxIterations = MAX_DEBUG_ITERATIONS, runDev = false, evolveAfter = true, } = options;
    ...
    while (iteration < maxIterations && !allPassed) {
        ...
        console.log(`  Build: ${buildResult.success ? '✓ PASS' : `✗ FAIL (${buildResult.errors.length} error(s))`}`);
        if (!buildResult.success) {
            const iterResult = await handleFailure(buildResult, protocol, projectDir, iteration, trace);
        ...
        console.log(`  Test: ${testResult.success ? '✓ PASS' : `✗ FAIL (${testResult.errors.length} error(s))`}`);
        if (!testResult.success) {
            const iterResult = await handleFailure(testResult, protocol, projectDir, iteration, trace);
            protocol = iterResult.protocol;
            trace.iterations.push(iterResult.iteration);
            continue;
        }
    ...
    if (allPassed && runDev) {
        ...
        console.log(`  Dev: ${devResult.success ? '✓ Server started' : '✗ Server failed'}`);
    }
    // ── Finalize trace ────────────────────────────────────────────────────
...
async function handleFailure(runResult, protocol, projectDir, iterationNum, trace) {
    const iterStart = Date.now();
    // Diagnose all errors
    const diagnoses = await diagnoseErrors(runResult.errors, protocol, projectDir);
    let repairAction = '';
    let matchedEntryId;
    let newEntryId;
    // Process the first diagnosable error (prioritize matched entries)
    const primaryDiag = diagnoses.find((d) => d.matched) ?? diagnoses.find((d) => d.candidateEntry) ?? diagnoses[0];
    if (primaryDiag) {
        ...
        // Record outcome to P
        const { logEntry } = recordOutcome(protocol, primaryDiag, repair, projectDir, verified);
...
//# sourceMappingURL=debug-loop.js.map