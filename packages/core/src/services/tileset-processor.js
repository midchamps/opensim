...
export class TilesetProcessor {
     ...
     */
    async expand3x3To7x7(rawBuffer, tileSize = 64, isPixelArt = true) {
        ...
        const outputTiles = new Array(49);
        // Fill with mapping
     ...
     */
    async generateInnerCorners(output) {
        const half = this.tileSize / 2;
        // Inner NW: Center + TL's BR quadrant in TL position
        output[BLOB_LAYOUT.INNER_NW] = await this.compositeInnerCorner(this.sourceTiles[C], this.sourceTiles[TL], { srcX: half, srcY: half }, // BR quadrant of TL
        { dstX: 0, dstY: 0 });
        // Inner NE: Center + TR's BL quadrant in TR position
        output[BLOB_LAYOUT.INNER_NE] = await this.compositeInnerCorner(this.sourceTiles[C], this.sourceTiles[TR], { srcX: 0, srcY: half }, // BL quadrant of TR
        { dstX: half, dstY: 0 });
        // Inner SW: Center + BL's TR quadrant in BL position
        output[BLOB_LAYOUT.INNER_SW] = await this.compositeInnerCorner(this.sourceTiles[C], this.sourceTiles[BL], { srcX: half, srcY: 0 }, // TR quadrant of BL
        { dstX: 0, dstY: half });
        // Inner SE: Center + BR's TL quadrant in BR position
        output[BLOB_LAYOUT.INNER_SE] = await this.compositeInnerCorner(this.sourceTiles[C], this.sourceTiles[BR], { srcX: 0, srcY: 0 }, // TL quadrant of BR
        { dstX: half, dstY: half });
    }
    /**
     * Composite an inner corner tile
     */
    async compositeInnerCorner(baseTile, cornerTile, src, dst) {
        const half = this.tileSize / 2;
...
//# sourceMappingURL=tileset-processor.js.map