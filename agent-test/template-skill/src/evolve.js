...
import { mergeIntoLibrary } from './merger.js';
 ...
 */
export async function evolveFromProject(projectDir) {
    ...
    try {
        ...
        console.log(`  Files: ${snapshot.files.length} | Tree: ${snapshot.fileTree.length} entries`);
        ...
        const result = mergeIntoLibrary(library, abstracted, patterns);
 ...
 */
export async function evolveBatch(projectDirs) {
    console.log(`\n${'='.repeat(60)}`);
    ...
    if (lastLib) {
        console.log(`Library v${lastLib.version} | ${lastLib.families.length} families`);
        for (const f of lastLib.families) {
            console.log(`  [${f.archetype}] stability: ${(f.stability * 100).toFixed(0)}% | ` +
