...
function createMockChildProcess(overrides = {}) {
...
describe('ProcessTransport', () => {
    ...
    describe('Construction and Initialization', () => {
        ...
        it('should throw if aborted before initialization', () => {
            ...
            expect(() => new ProcessTransport(options)).toThrow('Transport start aborted');
        });
    ...
    describe('Lifecycle Management', () => {
        ...
        it('should reject waitForExit() on non-zero exit code', async () => {
            ...
            await expect(waitPromise).rejects.toThrow('CLI process exited with code 1');
        });
        it('should reject waitForExit() on signal termination', async () => {
            ...
            await expect(waitPromise).rejects.toThrow('CLI process terminated by signal SIGTERM');
        });
    ...
    describe('Message Reading', () => {
        ...
        it('should throw if reading from transport without stdout', async () => {
            ...
            await expect(generator.next()).rejects.toThrow('Cannot read messages: process not started');
        });
    });
    describe('Message Writing', () => {
        ...
        it('should throw if writing before transport is ready', () => {
            ...
            expect(() => transport.write('test')).toThrow('Transport not ready for writing');
        });
        ...
        it('should throw if writing when aborted', () => {
            ...
            expect(() => transport.write('test')).toThrow('Cannot write: operation aborted');
        });
        it('should throw if writing to ended stream', () => {
            ...
            const transport = new ProcessTransport(options);
            mockStdin.end();
            expect(() => transport.write('test')).toThrow('Cannot write to ended stream');
        });
    ...
    describe('Error Handling', () => {
        ...
        it('should set exitError on process close with non-zero code', () => {
            ...
            mockChildProcess.emit('close', 1, null);
            expect(transport.exitError).toBeDefined();
            expect(transport.exitError?.message).toBe('CLI process exited with code 1');
        });
        it('should set exitError on process close with signal', () => {
            ...
            expect(transport.exitError).toBeDefined();
            expect(transport.exitError?.message).toBe('CLI process terminated by signal SIGKILL');
        });
...
//# sourceMappingURL=ProcessTransport.test.js.map