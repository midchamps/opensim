...
describe('OpenAIContentGenerator Timeout Handling', () => {
    ...
    beforeEach(() => {
        ...
        generator = new OpenAIContentGenerator(contentGeneratorConfig, mockConfig, mockProvider);
    });
    afterEach(() => {
        vi.restoreAllMocks();
    });
            ...
            for (const error of timeoutErrors) {
                ...
                try {
                    ...
                    const errorMessage = thrownError instanceof Error
                        ? thrownError.message
                        : String(thrownError);
                    expect(errorMessage).toMatch(/timeout after \d+s|Troubleshooting tips:/);
                }
            }
        });
    ...
    describe('generateContent timeout handling', () => {
        it('should handle timeout errors with helpful message', async () => {
            ...
            await expect(generator.generateContent(request, 'test-prompt-id')).rejects.toThrow(/Request timeout after \d+s\. Try reducing input length or increasing timeout in config\./);
        });
    ...
    describe('generateContentStream timeout handling', () => {
        it('should handle streaming timeout errors', async () => {
            ...
            await expect(generator.generateContentStream(request, 'test-prompt-id')).rejects.toThrow(/Streaming request timeout after \d+s\. Try reducing input length or increasing timeout in config\./);
        });
    ...
    describe('timeout configuration', () => {
        it('should use default timeout configuration', () => {
            ...
            new OpenAIContentGenerator(contentGeneratorConfig, mockConfig, mockProvider);
        ...
        it('should use custom timeout from config', () => {
            ...
            new OpenAIContentGenerator(contentGeneratorConfig, customConfig, customMockProvider);
            // Verify provider buildClient was called
        ...
        it('should handle missing timeout config gracefully', () => {
            ...
            new OpenAIContentGenerator(contentGeneratorConfig, noTimeoutConfig, noTimeoutMockProvider);
            // Verify provider buildClient was called
    ...
    describe('token estimation on timeout', () => {
        it('should surface a clear timeout error when request times out', async () => {
            ...
            await expect(generator.generateContent(request, 'test-prompt-id')).rejects.toThrow(/Request timeout after \d+s/);
        });
...
//# sourceMappingURL=openaiTimeoutHandling.test.js.map