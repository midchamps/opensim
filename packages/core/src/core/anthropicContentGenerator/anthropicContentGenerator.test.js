...
const importGenerator = async () => import('./anthropicContentGenerator.js');
const importConverter = async () => import('./converter.js');
describe('AnthropicContentGenerator', () => {
    ...
    describe('generateContent', () => {
        it('builds request with config sampling params (config overrides request) and thinking budget', async () => {
            ...
            const convertResponseSpy = vi
                .spyOn(AnthropicContentConverter.prototype, 'convertAnthropicResponseToGemini')
                .mockReturnValue((() => {
            ...
            expect(anthropicRequest).toEqual(expect.objectContaining({
                model: 'claude-test',
                max_tokens: 1000,
                temperature: 0.7,
                top_p: 0.9,
        ...
        it('omits thinking when request.config.thinkingConfig.includeThoughts is false', async () => {
            ...
            expect(anthropicRequest).toEqual(expect.not.objectContaining({ thinking: expect.anything() }));
        });
    });
    ...
    describe('generateContentStream', () => {
        it('requests stream=true and converts streamed events into Gemini chunks', async () => {
            const { AnthropicContentGenerator } = await importGenerator();
            anthropicState.createImpl.mockResolvedValue((async function* () {
                yield {
                    type: 'message_start',
                    message: {
                        id: 'msg-1',
            ...
            const [anthropicRequest] = anthropicState.lastCreateArgs;
            expect(anthropicRequest).toEqual(expect.objectContaining({ stream: true }));
            // Text chunk.
...
//# sourceMappingURL=anthropicContentGenerator.test.js.map