...
export function createContentGeneratorConfig(
  ...
  if (authType === AuthType.USE_OPENAI) {
    ...
    // Gemini's OpenAI-compat surface uses the Gemini schema dialect (a subset
    // of OpenAPI 3.0). Sending modern JSON Schema features like `oneOf`,
    // `const`, or `$schema` triggers 400 with an empty body. When we detect
    // the endpoint is Gemini, force the schema converter into openapi_30
    // mode so tool parameters get downgraded before the request goes out.
    const baseUrl = newContentGeneratorConfig.baseUrl ?? '';
    const model = newContentGeneratorConfig.model ?? '';
    const looksLikeGemini =
      baseUrl.includes('generativelanguage.googleapis.com') ||
      model.startsWith('gemini-') ||
      model.startsWith('imagen-');
    // Force openapi_30 for Gemini even when schemaCompliance is the default
    // `'auto'` — the settings schema seeds that value eagerly, so a plain
    // "unset" check would never fire for a user who hasn't explicitly
    // configured the field.
    if (
      looksLikeGemini &&
      (newContentGeneratorConfig.schemaCompliance === undefined ||
        newContentGeneratorConfig.schemaCompliance === 'auto')
    ) {
      newContentGeneratorConfig.schemaCompliance = 'openapi_30';
    }
    // Process-level kill-switch: schemaConverter.ts reads this to force
    // OpenAPI 3.0 conversion regardless of the mode the caller passed.
    // Needed because some code paths (logging, subagents, etc.) build
    // their own converter with the default 'auto' mode, which would leak
    // oneOf/const/$schema fields past the wire for Gemini.
    if (looksLikeGemini) {
      process.env['OPENGAME_FORCE_OPENAPI30'] = '1';
    }
    // DEBUG: surface detection result
    if (process.env['OPENGAME_DEBUG_GEMINI']) {
      process.stderr.write(
        `[opengame-debug] createContentGeneratorConfig: authType=${authType} baseUrl=${baseUrl} model=${model} looksLikeGemini=${looksLikeGemini} schemaCompliance=${newContentGeneratorConfig.schemaCompliance}\n`,
      );
    }
