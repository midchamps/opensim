...
export class SkillManager {
    ...
    async parseSkillFileInternal(filePath, level) {
        ...
        try {
            ...
            const skillError = new SkillError(`Failed to read skill file: ${error instanceof Error ? error.message : 'Unknown error'}`, SkillErrorCode.FILE_ERROR);
    ...
    getSkillsBaseDir(level) {
        const baseDir = level === 'project'
            ? path.join(this.config.getProjectRoot(), QWEN_CONFIG_DIR, SKILLS_CONFIG_DIR)
            ...
            for (const entry of entries) {
                ...
                try {
                    ...
                    if (error instanceof SkillError) {
                        ...
                        console.warn(`Failed to parse skill at ${skillDir}: ${error.message}`);
                    }
                    continue;
                }
            }
