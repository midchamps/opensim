...
describe('IdeClient', () => {
    ...
    describe('connect', () => {
        it('should connect using HTTP when port is provided in config file', async () => {
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/home/test', '.qwen', 'ide', '8080.lock'), 'utf8');
    ...
    describe('getConnectionConfigFromFile', () => {
        it('should return config from the env port lock file if it exists', async () => {
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/home/test', '.qwen', 'ide', '1234.lock'), 'utf8');
            delete process.env['QWEN_CODE_IDE_SERVER_PORT'];
        });
        ...
        it('should read legacy pid config when available', async () => {
            ...
            vi.mocked(fs.promises.readFile).mockResolvedValueOnce(JSON.stringify(config));
            const ideClient = await IdeClient.getInstance();
            const result = await ideClient.getConnectionConfigFromFile();
            expect(result).toEqual(config);
        ...
        it('should fall back to legacy port file when pid file is missing', async () => {
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'qwen-code-ide-server-12345.json'), 'utf8');
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'qwen-code-ide-server-2222.json'), 'utf8');
            delete process.env['QWEN_CODE_IDE_SERVER_PORT'];
        });
