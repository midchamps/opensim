 ...
 */
import { QWEN_DIR, Storage, Config, ExtensionInstallEvent, ExtensionUninstallEvent, ExtensionDisableEvent, ExtensionEnableEvent, logExtensionEnable, logExtensionInstallEvent, logExtensionUninstall, logExtensionDisable, } from '@opengame/opengame-core';
...
export function loadExtension(context) {
...
export async function installExtension(installMetadata, requestConsent, cwd = process.cwd(), previousExtensionConfig) {
...
function extensionConsentString(extensionConfig) {
    ...
    if (extensionConfig.contextFileName) {
        output.push(`This extension will append info to your QWEN.md context using ${extensionConfig.contextFileName}`);
    }
    if (extensionConfig.excludeTools) {
        output.push(`This extension will exclude the following core tools: ${extensionConfig.excludeTools}`);
    }
    return output.join('\n');
}
/**
...
export function toOutputString(extension, workspaceDir) {
    ...
    const workspaceEnabled = manager.isEnabled(extension.config.name, workspaceDir);
