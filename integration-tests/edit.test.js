...
describe('edit', () => {
    it('should be able to edit content in a file', async () => {
        ...
        validateModelOutput(result, ['edited', 'file_to_edit.txt'], 'Edit content test');
    ...
    it('should handle $ literally when replacing text ending with $', async () => {
        ...
        rig.createFile(fileName, originalContent);
        const prompt = "Open regex.yml and append ' # updated' after the line containing ^[sv]d[a-z]$ without breaking the $ character.";
        const result = await rig.run(prompt);
        const foundToolCall = await rig.waitForToolCall('edit');
        if (!foundToolCall) {
            printDebugInfo(rig, result);
    ...
    it.skip('should fail safely when old_string is not found', async () => {
        ...
        expect(readAttempt || editAttempt, 'Expected model to attempt a read_file or edit').toBeDefined();
        ...
        if (editAttempt) {
            ...
            expect(editAttempt.toolRequest.success, 'If edit is called, it must fail').toBe(false);
        }
