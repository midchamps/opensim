...
import { ShellTool, EditTool, WriteFileTool, DEFAULT_QWEN_MODEL, OutputFormat, } from '@opengame/opengame-core';
...
vi.mock('@opengame/opengame-core', async () => {
    const actualServer = await vi.importActual('@opengame/opengame-core');
    return {
        ...actualServer,
        IdeClient: {
...
describe('parseArguments', () => {
    ...
    it('should throw an error when include-partial-messages is used without stream-json output', async () => {
        ...
        const mockConsoleError = vi
            ...
            .mockImplementation(() => { });
        await expect(parseArguments({})).rejects.toThrow('process.exit called');
        expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining('--include-partial-messages requires --output-format stream-json'));
        mockExit.mockRestore();
        mockConsoleError.mockRestore();
    });
...
describe('loadCliConfig', () => {
    ...
    it('should propagate stream-json formats to config', async () => {
        ...
        const argv = await parseArguments({});
        const settings = {};
        const config = await loadCliConfig(settings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
    ...
    it('should set showMemoryUsage to false by default from settings if CLI flag is not present', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const settings = { ui: { showMemoryUsage: false } };
        const config = await loadCliConfig(settings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
        expect(config.getShowMemoryUsage()).toBe(false);
    });
...
describe('Hierarchical Memory Loading (config.ts) - Placeholder Suite', () => {
    ...
    it('should pass extension context file paths to loadServerHierarchicalMemory', async () => {
        ...
        const argv = await parseArguments({});
        await loadCliConfig(settings, extensions, new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
...
describe('mergeExcludeTools', () => {
    ...
    it('should merge excludeTools from settings and extensions', async () => {
        ...
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const config = await loadCliConfig(settings, extensions, new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
...
describe('Approval mode tool exclusion logic', () => {
    ...
    it('should throw an error for invalid approval mode values in loadCliConfig', async () => {
        ...
        const settings = {};
        const extensions = [];
        await expect(loadCliConfig(settings, extensions, new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), invalidArgv.extensions), invalidArgv)).rejects.toThrow('Invalid approval mode: invalid_mode. Valid values are: plan, default, auto-edit, yolo');
    });
});
describe('loadCliConfig with allowed-mcp-server-names', () => {
    ...
    it('should allow all MCP servers if the flag is not provided', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const config = await loadCliConfig(baseSettings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
...
describe('loadCliConfig extensions', () => {
    ...
    it('should not filter extensions if --extensions flag is not used', async () => {
        ...
        const argv = await parseArguments({});
        const settings = {};
        const config = await loadCliConfig(settings, mockExtensions, new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
...
describe('loadCliConfig model selection', () => {
    it.skip('selects a model from settings.json if provided', async () => {
        ...
        }, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
...
describe('loadCliConfig useBuiltinRipgrep', () => {
    ...
    it('should be false when useBuiltinRipgrep is set to false in settings', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const settings = { tools: { useBuiltinRipgrep: false } };
        const config = await loadCliConfig(settings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
    ...
    it('should be true when useBuiltinRipgrep is explicitly set to true in settings', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const settings = { tools: { useBuiltinRipgrep: true } };
        const config = await loadCliConfig(settings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
        expect(config.getUseBuiltinRipgrep()).toBe(true);
    });
});
...
describe('loadCliConfig tool exclusions', () => {
    ...
    it('should not exclude interactive tools in interactive mode without YOLO', async () => {
        ...
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const config = await loadCliConfig({}, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
...
describe('loadCliConfig approval mode', () => {
    ...
    it('should use approval mode from settings when CLI flags are not provided', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const settings = { tools: { approvalMode: 'plan' } };
        const config = await loadCliConfig(settings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
        expect(config.getApprovalMode()).toBe(ServerConfig.ApprovalMode.PLAN);
    });
    it('should normalize approval mode values from settings', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const settings = { tools: { approvalMode: 'AutoEdit' } };
        const config = await loadCliConfig(settings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
        expect(config.getApprovalMode()).toBe(ServerConfig.ApprovalMode.AUTO_EDIT);
    });
    it('should throw when approval mode in settings is invalid', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const settings = { tools: { approvalMode: 'invalid_mode' } };
        await expect(loadCliConfig(settings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv)).rejects.toThrow('Invalid approval mode: invalid_mode. Valid values are: plan, default, auto-edit, yolo');
    });
...
describe('Output format', () => {
    ...
    it('should use the format from settings', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const config = await loadCliConfig({ output: { format: OutputFormat.JSON } }, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
...
describe('Telemetry configuration via environment variables', () => {
    ...
    it('should prioritize GEMINI_TELEMETRY_TARGET over settings', async () => {
        ...
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const settings = { telemetry: { target: 'local' } };
        const config = await loadCliConfig(settings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
        expect(config.getTelemetryTarget()).toBe('gcp');
    });
    it('should throw when GEMINI_TELEMETRY_TARGET is invalid', async () => {
        ...
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const settings = { telemetry: { target: 'gcp' } };
        await expect(loadCliConfig(settings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv)).rejects.toThrow(/Invalid telemetry configuration: .*Invalid telemetry target/i);
        vi.unstubAllEnvs();
    });
    ...
    it("should treat GEMINI_TELEMETRY_ENABLED='0' as false", async () => {
        ...
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const config = await loadCliConfig({ telemetry: { enabled: true } }, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
        expect(config.getTelemetryEnabled()).toBe(false);
    });
    ...
    it("should treat GEMINI_TELEMETRY_LOG_PROMPTS='false' as false", async () => {
        ...
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const config = await loadCliConfig({ telemetry: { logPrompts: true } }, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), argv);
        expect(config.getTelemetryLogPromptsEnabled()).toBe(false);
    });
});
