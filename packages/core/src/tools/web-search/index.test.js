...
describe('WebSearchTool', () => {
    ...
    describe('formatSearchResults', () => {
        it('should use answer when available and append sources', async () => {
            ...
            mockConfig.getWebSearchConfig.mockReturnValue(webSearchConfig);
            ...
            expect(result.llmContent).toContain('This is a concise answer from the search provider.');
            expect(result.llmContent).toContain('Sources:');
            expect(result.llmContent).toContain('[1] Result 1 (https://example.com/1)');
            expect(result.llmContent).toContain('[2] Result 2 (https://example.com/2)');
        });
        it('should build informative summary when answer is not available', async () => {
            ...
            expect(result.llmContent).toContain('This is a helpful snippet from the first result.');
            expect(result.llmContent).toContain('Source: https://example.com/1');
            expect(result.llmContent).toContain('2. **Google Result 2**');
            expect(result.llmContent).toContain('This is a helpful snippet from the second result.');
    ...
    describe('validation', () => {
        it('should throw validation error when query is empty', () => {
            const tool = new WebSearchTool(mockConfig);
            expect(() => tool.build({ query: '' })).toThrow("The 'query' parameter cannot be empty");
        });
        it('should throw validation error when provider is empty string', () => {
            const tool = new WebSearchTool(mockConfig);
            expect(() => tool.build({ query: 'test', provider: '' })).toThrow("The 'provider' parameter cannot be empty");
        });
    });
    describe('configuration', () => {
        it('should return error when web search is not configured', async () => {
            mockConfig.getWebSearchConfig.mockReturnValue(null);
            const tool = new WebSearchTool(mockConfig);
            const invocation = tool.build({ query: 'test query' });
            const result = await invocation.execute(new AbortController().signal);
        ...
        it('should return descriptive message in getDescription when web search is not configured', () => {
            ...
            expect(description).toBe(' (Web search is disabled - configure a provider in settings.json)');
        });
