...
${finalExclusionPatternsForDescription
        ...
        if (qwenIgnorePatterns.length > 0) {
            const geminiPatternsInEffect = qwenIgnorePatterns.filter((p) => finalExclusionPatternsForDescription.includes(p)).length;
...
export class ReadManyFilesTool extends BaseDeclarativeTool {
    config;
    static Name = ToolNames.READ_MANY_FILES;
    constructor(config) {
        ...
        super(ReadManyFilesTool.Name, ToolDisplayNames.READ_MANY_FILES, `Reads content from multiple files specified by paths or glob patterns within a configured target directory. For text files, it concatenates their content into a single string. It is primarily designed for text-based files. However, it can also process image (e.g., .png, .jpg) and PDF (.pdf) files if their file names or extensions are explicitly included in the 'paths' argument. For these explicitly requested non-text files, their data is read and included in a format suitable for model consumption (e.g., base64 encoded).
