...
import { GitService, logSlashCommand, makeSlashCommandEvent, SlashCommandStatus, ToolConfirmationOutcome, IdeClient, } from '@opengame/opengame-core';
 ...
 */
export const useSlashCommandProcessor = (config, settings, addItem, clearItems, loadHistory, refreshStatic, toggleVimEnabled, setIsProcessing, setGeminiMdFileCount, actions, extensionsUpdateState, isConfigInitialized, logger) => {
    ...
    const handleSlashCommand = useCallback(async (rawQuery, oneTimeShellAllowlist, overwriteConfirmed) => {
        ...
        addItemWithRecording({ type: MessageType.USER, text: trimmed }, userMessageTimestamp);
        let hasError = false;
            ...
            if (config?.getChatRecordingService) {
                ...
                try {
                    ...
                    if (config.getDebugMode()) {
                        console.error('[slashCommand] Failed to record slash command:', recordError);
                    }
                }
            }
