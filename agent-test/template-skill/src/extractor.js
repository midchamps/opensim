...
// -----------------------------------------------------------------------------
function extractDirectoryPattern(snapshot) {
    const srcFiles = snapshot.files.filter((f) => f.relativePath.startsWith('src/') && f.extension === '.ts');
    const dirMap = {};
...
// -----------------------------------------------------------------------------
const CLASS_REGEX = /^(?:export\s+)?(?:(abstract)\s+)?class\s+(\w+)(?:\s+extends\s+([\w.]+))?/gm;
const METHOD_REGEX = /^\s*(public|protected|private)?\s*(abstract\s+)?(override\s+)?(?:async\s+)?(\w+)\s*\(([^)]*)\)\s*(?::\s*[\w<>[\]| ,]+)?\s*[{;]/gm;
function extractClasses(snapshot) {
    const classes = [];
...
function extractMethods(fileContent, _className) {
    const methods = [];
    let match;
...
// -----------------------------------------------------------------------------
const IMPORT_REGEX = /import\s+(?:(?:type\s+)?{([^}]+)}|(\w+))\s+from\s+['"]([^'"]+)['"]/g;
function extractImports(snapshot) {
    const edges = [];
    for (const file of snapshot.files) {
        if (file.extension !== '.ts')
            continue;
...
// -----------------------------------------------------------------------------
async function extractConfigExtensions(gameConfig) {
    if (!gameConfig)
        return [];
...
// -----------------------------------------------------------------------------
function collectCodeSnippets(snapshot) {
    const snippets = {};
    const keyPatterns = [
        /Base\w+Scene\.ts$/,
        /Base\w+\.ts$/,
 ...
 */
export async function extractPatterns(snapshot, classification) {
    ...
    console.log(`[Extractor] Done: ${classes.length} classes, ${hooks.length} hooks, ` +
