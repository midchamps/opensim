...
class GameTypeClassifierInvocation extends BaseToolInvocation {
    ...
    get modelConfig() {
        ...
        if (!this.resolvedModelConfig) {
            this.resolvedModelConfig = GameTypeClassifierTool.resolveModelConfig(this.config);
        }
    ...
    async execute(signal) {
        try {
            ...
            const result = await this.callClassifierModel(systemPrompt, userPrompt, signal);
...
Remember: Think about GRAVITY, PERSPECTIVE, and MOVEMENT TYPE. Output JSON only.`;
    ...
    async callClassifierModel(systemPrompt, userPrompt, signal) {
        const payload = {
...
export class GameTypeClassifierTool extends BaseDeclarativeTool {
    config;
    modelConfig;
    static Name = ToolNames.GAME_TYPE_CLASSIFIER;
    /**
    ...
    modelConfig) {
        super(GameTypeClassifierTool.Name, ToolDisplayNames.GAME_TYPE_CLASSIFIER, `Classifies a game idea into an archetype based on PHYSICS and PERSPECTIVE (not genre name). Returns: platformer (side+gravity), top_down (free movement), grid_logic (discrete tiles), tower_defense (path+waves), or ui_heavy (no physics). Call this FIRST before scaffolding templates.`, Kind.Think, {
            type: 'object',
            properties: {
                game_description: {
                    type: 'string',
                    description: 'The user\'s game idea or description. Can include genre, mechanics, or example games. Example: "I want a game like Terraria" or "A puzzle game where you push boxes"',
                },
            },
    ...
    createInvocation(params) {
        return new GameTypeClassifierInvocation(this.config, params, this.modelConfig);
    }
}
//# sourceMappingURL=game-type-classifier.js.map