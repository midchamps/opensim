...
function getTestCase() {
    ...
    const testCase = allTestCases[testCaseName];
    if (!testCase) {
...
async function test() {
    ...
    try {
        ...
        for await (const message of result) {
            ...
            if (isSDKAssistantMessage(message)) {
                ...
                if (text.trim()) {
                    messages.push(text);
                    console.log('🤖 Assistant:', text.slice(0, 300) + (text.length > 300 ? '...' : ''));
                    if (text.includes('API Error') || text.includes('[API Error')) {
                        console.error(`\n⚠️ API Error in assistant response:`);
                        const errorMatch = text.match(/\[API Error[^\]]*\]|API Error[^.\n]*/g);
                        if (errorMatch) {
                ...
                if (!success) {
                    ...
                    if (errObj?.type)
                        console.error(`   Type: ${errObj.type}`);
                    ...
                    if (errObj?.request_id || errObj?.requestId)
                        console.error(`   Request ID: ${errObj.request_id || errObj.requestId}`);
                    if (fullMsg.model)
                        console.error(`   Model: ${fullMsg.model}`);
                    if (errObj) {
        ...
        if (e.request_id || e.requestId || e.headers?.['x-request-id']) {
            console.error(`   Request ID: ${e.request_id || e.requestId || e.headers?.['x-request-id']}`);
        }
        if (e.model)
            console.error(`   Model: ${e.model}`);
        if (e.code)
    ...
    await fs.writeFile(path.join(outputDir, 'result.json'), JSON.stringify(result, null, 2), 'utf-8');
    await fs.writeFile(path.join(outputDir, 'conversation.txt'), messages.join('\n\n---\n\n'), 'utf-8');
    return result;
}
