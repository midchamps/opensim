...
import { Logger, uiTelemetryService, } from '@opengame/opengame-core';
...
export const handleSlashCommand = async (rawQuery, abortController, config, settings, allowedBuiltinCommandNames = [
    ...
    if (!commandToExecute) {
        ...
        if (knownCommand) {
            ...
            return {
                type: 'unsupported',
                reason: t('The command "/{{command}}" is not supported in non-interactive mode.', { command: knownCommand.name }),
 ...
 */
export const getAvailableCommands = async (config, abortSignal, allowedBuiltinCommandNames = [
    ...ALLOWED_BUILTIN_COMMANDS_NON_INTERACTIVE,
]) => {
    try {
        const allowedBuiltinSet = new Set(allowedBuiltinCommandNames ?? []);
