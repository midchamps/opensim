...
describe('file-system', () => {
    it('should be able to read a file', async () => {
        ...
        const result = await rig.run(`read the file test.txt and show me its contents`);
    ...
    it('should be able to write a file', async () => {
        ...
        expect(foundToolCall, 'Expected to find a write_file or edit tool call').toBeTruthy();
    ...
    it.skip('should replace multiple instances of a string', async () => {
        ...
        const result = await rig.run(`replace "test line" with "new line" in ${fileName}`);
        ...
        expect(foundToolCall, 'Expected to find a replace or write_file tool call').toBeTruthy();
        const toolLogs = rig.readToolLogs();
        const successfulEdit = toolLogs.some((log) => (log.toolRequest.name === 'replace' ||
            log.toolRequest.name === 'write_file') &&
            log.toolRequest.success);
        if (!successfulEdit) {
            console.error('Expected a successful edit tool call, but none was found.');
            printDebugInfo(rig, result);
        }
    ...
    it('should fail safely when trying to edit a non-existent file', async () => {
        ...
        if (editAttempt) {
            console.error('A edit_file attempt was made when no file should be written.');
            printDebugInfo(rig, result);
        }
        expect(editAttempt, 'edit_file should not have been called').toBeUndefined();
