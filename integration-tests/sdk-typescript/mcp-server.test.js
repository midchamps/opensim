...
import { query, isSDKAssistantMessage, isSDKResultMessage, isSDKSystemMessage, isSDKUserMessage, } from '@opengame/sdk';
import { SDKTestHelper, createMCPServer, extractText, findToolUseBlocks, createSharedTestOptions, } from './test-helper.js';
...
describe('MCP Server Integration (E2E)', () => {
    ...
    describe('Basic MCP Tool Usage', () => {
        it('should use MCP add tool to add two numbers', async () => {
            const q = query({
                prompt: 'Use the add tool to calculate 5 + 10. Just give me the result.',
                options: {
                    ...SHARED_TEST_OPTIONS,
                    cwd: testDir,
                    debug: false,
        ...
        it('should use MCP multiply tool to multiply two numbers', async () => {
            const q = query({
                prompt: 'Use the multiply tool to calculate 6 * 7. Just give me the result.',
                options: {
                    ...SHARED_TEST_OPTIONS,
                    cwd: testDir,
                    debug: false,
    ...
    describe('MCP Server Discovery', () => {
        it('should list MCP servers in system init message', async () => {
            ...
            try {
                ...
                const testServer = systemMessage.mcp_servers?.find((server) => server.name === 'test-math-server');
    ...
    describe('Complex MCP Operations', () => {
        it('should chain multiple MCP tool calls', async () => {
            const q = query({
                prompt: 'First use add to calculate 10 + 5, then multiply the result by 2. Give me the final answer.',
                options: {
                    ...SHARED_TEST_OPTIONS,
                    cwd: testDir,
                    debug: false,
        ...
        it('should handle multiple calls to the same MCP tool', async () => {
            const q = query({
                prompt: 'Use the add tool twice: first add 1 + 2, then add 3 + 4. Tell me both results.',
                options: {
                    ...SHARED_TEST_OPTIONS,
                    cwd: testDir,
                    debug: false,
            ...
            try {
                for await (const message of q) {
                    ...
                    if (isSDKUserMessage(message)) {
                        ...
                        const toolResultBlock = contentArray.find((block) => block.type === 'tool_result');
                        if (toolResultBlock) {
