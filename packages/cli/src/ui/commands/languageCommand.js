...
import { setLanguageAsync, getCurrentLanguage, detectSystemLanguage, getLanguageNameFromLocale, t, } from '../../i18n/index.js';
...
function extractLlmOutputLanguageFromRuleFileContent(content) {
    ...
    const markerMatch = content.match(new RegExp(String.raw `<!--\s*${LLM_OUTPUT_LANGUAGE_MARKER_PREFIX}\s*(.*?)\s*-->`, 'i'));
    if (markerMatch?.[1]) {
 ...
 */
async function setUiLanguage(context, lang) {
...
export const languageCommand = {
    ...
    action: async (context, args) => {
        ...
        if (firstArg === 'ui' || firstArg === 'output') {
            const subCommand = languageCommand.subCommands?.find((s) => s.name === firstArg);
                ...
                if (!trimmedArgs) {
                    return {
                        ...
                        content: [
                            ...
                            '',
                            t('Available options:'),
                            ...SUPPORTED_LANGUAGES.map((o) => `  - ${o.id}: ${t(o.fullName)}`),
                            '',
 ...
 */
function createUiLanguageSubCommand(option) {
    return {
        name: option.id,
        get description() {
