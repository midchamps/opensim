...
import { SubAgentEventType, ToolConfirmationOutcome, TodoWriteTool, } from '@opengame/opengame-core';
...
describe('SubAgentTracker', () => {
    ...
    describe('setup', () => {
        ...
        it('should register event listeners', () => {
            ...
            expect(onSpy).toHaveBeenCalledWith(SubAgentEventType.TOOL_CALL, expect.any(Function));
            expect(onSpy).toHaveBeenCalledWith(SubAgentEventType.TOOL_RESULT, expect.any(Function));
            expect(onSpy).toHaveBeenCalledWith(SubAgentEventType.TOOL_WAITING_APPROVAL, expect.any(Function));
        });
        it('should remove event listeners on cleanup', () => {
            const offSpy = vi.spyOn(eventEmitter, 'off');
            const cleanups = tracker.setup(eventEmitter, abortController.signal);
            cleanups[0]();
            expect(offSpy).toHaveBeenCalledWith(SubAgentEventType.TOOL_CALL, expect.any(Function));
            expect(offSpy).toHaveBeenCalledWith(SubAgentEventType.TOOL_RESULT, expect.any(Function));
            expect(offSpy).toHaveBeenCalledWith(SubAgentEventType.TOOL_WAITING_APPROVAL, expect.any(Function));
        });
    });
    ...
    describe('tool result handling', () => {
        it('should emit tool_call_update on TOOL_RESULT event', async () => {
            ...
            eventEmitter.emit(SubAgentEventType.TOOL_CALL, createToolCallEvent({
                name: 'read_file',
                callId: 'call-123',
                args: { path: '/test.ts' },
            }));
        ...
        it('should clean up state after result', async () => {
            ...
            eventEmitter.emit(SubAgentEventType.TOOL_RESULT, createToolResultEvent({
                name: 'test_tool',
                callId: 'call-cleanup',
                success: true,
            }));
