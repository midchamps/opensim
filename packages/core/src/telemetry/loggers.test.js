...
import { EVENT_API_REQUEST, EVENT_API_RESPONSE, EVENT_CLI_CONFIG, EVENT_FLASH_FALLBACK, EVENT_TOOL_CALL, EVENT_USER_PROMPT, EVENT_MALFORMED_JSON_RESPONSE, EVENT_FILE_OPERATION, EVENT_RIPGREP_FALLBACK, EVENT_EXTENSION_ENABLE, EVENT_EXTENSION_DISABLE, EVENT_EXTENSION_INSTALL, EVENT_EXTENSION_UNINSTALL, } from './constants.js';
import { logApiRequest, logApiResponse, logStartSession, logUserPrompt, logToolCall, logFlashFallback, logChatCompression, logMalformedJsonResponse, logFileOperation, logRipgrepFallback, logToolOutputTruncated, logExtensionEnable, logExtensionDisable, logExtensionInstallEvent, logExtensionUninstall, } from './loggers.js';
import * as metrics from './metrics.js';
import { QwenLogger } from './qwen-logger/qwen-logger.js';
import * as sdk from './sdk.js';
import { ToolCallDecision } from './tool-call-decision.js';
import { ApiRequestEvent, ApiResponseEvent, FlashFallbackEvent, StartSessionEvent, ToolCallEvent, UserPromptEvent, RipgrepFallbackEvent, MalformedJsonResponseEvent, makeChatCompressionEvent, FileOperationEvent, ToolOutputTruncatedEvent, ExtensionEnableEvent, ExtensionDisableEvent, ExtensionInstallEvent, ExtensionUninstallEvent, } from './types.js';
...
describe('loggers', () => {
    ...
    describe('logChatCompression', () => {
        ...
        it('logs the chat compression event to QwenLogger', () => {
            ...
            expect(QwenLogger.prototype.logChatCompressionEvent).toHaveBeenCalledWith(event);
        });
    ...
    describe('logUserPrompt', () => {
        ...
        it('should not log prompt if disabled', () => {
            ...
            const event = new UserPromptEvent(11, 'prompt-id-9', AuthType.USE_GEMINI, 'test-prompt');
            logUserPrompt(mockConfig, event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
                body: 'User prompt. Length: 11.',
                attributes: {
    ...
    describe('logApiResponse', () => {
        ...
        it('should log an API response with all fields', () => {
            ...
            const event = new ApiResponseEvent('test-response-id', 'test-model', 100, 'prompt-id-1', AuthType.USE_GEMINI, usageData, 'test-response');
    ...
    describe('logRipgrepFallback', () => {
        ...
        it('should log ripgrep fallback event', () => {
            const event = new RipgrepFallbackEvent(false, false, 'ripgrep is not available');
    ...
    describe('logMalformedJsonResponse', () => {
        ...
        it('logs the event to Clearcut and OTEL', () => {
            ...
            expect(QwenLogger.prototype.logMalformedJsonResponseEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logExtensionInstall', () => {
        ...
        it('should log extension install event', () => {
            ...
            expect(QwenLogger.prototype.logExtensionInstallEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logExtensionUninstall', () => {
        ...
        it('should log extension uninstall event', () => {
            ...
            expect(QwenLogger.prototype.logExtensionUninstallEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logExtensionEnable', () => {
        ...
        it('should log extension enable event', () => {
            ...
            expect(QwenLogger.prototype.logExtensionEnableEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logExtensionDisable', () => {
        ...
        it('should log extension disable event', () => {
            ...
            expect(QwenLogger.prototype.logExtensionDisableEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
