...
describe('GlobTool', () => {
    ...
    describe('execute', () => {
        ...
        it('should find files even if workspace path casing differs from glob results (Windows/macOS)', async () => {
            ...
            const mismatchedConfig = {
                ...
                getWorkspaceContext: () => createMockWorkspaceContext(mismatchedRootDir),
            };
    ...
    describe('validateToolParams', () => {
        ...
        it("should return error if search path resolves outside the tool's root directory", () => {
            ...
            expect(specificGlobTool.validateToolParams(paramsOutside)).toContain('Path is not within workspace');
        });
        it('should return error if specified search path does not exist', async () => {
            ...
            expect(globTool.validateToolParams(params)).toContain('Path does not exist');
        });
        it('should return error if specified search path is a file, not a directory', async () => {
            const params = { pattern: '*.txt', path: 'fileA.txt' };
            expect(globTool.validateToolParams(params)).toContain('Path is not a directory');
        });
    });
    describe('workspace boundary validation', () => {
        it('should validate search paths are within workspace boundaries', () => {
            ...
            expect(globTool.validateToolParams(invalidPath)).toContain('Path is not within workspace');
        });
    ...
    describe('ignore file handling', () => {
        it('should respect .gitignore files by default', async () => {
            ...
            await fs.writeFile(path.join(tempRootDir, 'a.ignored.txt'), 'ignored content');
            await fs.writeFile(path.join(tempRootDir, 'b.notignored.txt'), 'not ignored content');
            const params = { pattern: '*.txt' };
            const invocation = globTool.build(params);
            const result = await invocation.execute(abortSignal);
        ...
        it('should respect .qwenignore files by default', async () => {
            await fs.writeFile(path.join(tempRootDir, '.qwenignore'), '*.qwenignored.txt');
            await fs.writeFile(path.join(tempRootDir, 'a.qwenignored.txt'), 'ignored content');
            await fs.writeFile(path.join(tempRootDir, 'b.notignored.txt'), 'not ignored content');
    ...
    describe('file count truncation', () => {
        it('should truncate results when more than 100 files are found', async () => {
            ...
            for (let i = 1; i <= 150; i++) {
                await fs.writeFile(path.join(tempRootDir, `file${i}.trunctest`), `content${i}`);
            }
            const params = { pattern: '*.trunctest' };
            const invocation = globTool.build(params);
            const result = await invocation.execute(abortSignal);
            const llmContent = partListUnionToString(result.llmContent);
            ...
            expect(result.returnDisplay).toBe('Found 150 matching file(s) (truncated)');
        });
        it('should not truncate when exactly 100 files are found', async () => {
            // Create exactly 100 test files
            for (let i = 1; i <= 100; i++) {
                await fs.writeFile(path.join(tempRootDir, `exact${i}.trunctest`), `content${i}`);
            }
            const params = { pattern: '*.trunctest' };
            const invocation = globTool.build(params);
            const result = await invocation.execute(abortSignal);
        ...
        it('should not truncate when fewer than 100 files are found', async () => {
            ...
            for (let i = 1; i <= 50; i++) {
                await fs.writeFile(path.join(tempRootDir, `small${i}.trunctest`), `content${i}`);
            }
            const params = { pattern: '*.trunctest' };
            const invocation = globTool.build(params);
            const result = await invocation.execute(abortSignal);
        ...
        it('should use correct singular/plural in truncation message for 1 file truncated', async () => {
            ...
            for (let i = 1; i <= 101; i++) {
                await fs.writeFile(path.join(tempRootDir, `singular${i}.trunctest`), `content${i}`);
            }
            const params = { pattern: '*.trunctest' };
            const invocation = globTool.build(params);
            const result = await invocation.execute(abortSignal);
        ...
        it('should use correct plural in truncation message for multiple files truncated', async () => {
            ...
            for (let i = 1; i <= 105; i++) {
                await fs.writeFile(path.join(tempRootDir, `plural${i}.trunctest`), `content${i}`);
            }
            const params = { pattern: '*.trunctest' };
            const invocation = globTool.build(params);
            const result = await invocation.execute(abortSignal);
