...
import { z } from 'zod';
import { query, tool, createSdkMcpServer, isSDKAssistantMessage, isSDKResultMessage, isSDKSystemMessage, } from '@opengame/sdk';
import { SDKTestHelper, extractText, findToolUseBlocks, createSharedTestOptions, } from './test-helper.js';
...
describe('SDK MCP Server Integration (E2E)', () => {
    ...
    describe('Basic SDK MCP Tool Usage', () => {
        it('should use SDK MCP tool to perform a simple calculation', async () => {
            ...
            const calculatorTool = tool('calculate_sum', 'Calculate the sum of two numbers', z.object({
            ...
            const q = query({
                prompt: 'Use the calculate_sum tool to add 25 and 17. Output the result of tool only.',
                options: {
                    ...SHARED_TEST_OPTIONS,
                    cwd: testDir,
                    mcpServers: {
        ...
        it('should use SDK MCP tool with string operations', async () => {
            ...
            const stringTool = tool('reverse_string', 'Reverse a string', {
    ...
    describe('Multiple SDK MCP Tools', () => {
        it('should use multiple tools from the same SDK MCP server', async () => {
            ...
            const addTool = tool('sdk_add', 'Add two numbers', twoNumbersSchema, async (args) => ({
                content: [{ type: 'text', text: String(args.a + args.b) }],
            }));
            const multiplyTool = tool('sdk_multiply', 'Multiply two numbers', twoNumbersSchema, async (args) => ({
                content: [{ type: 'text', text: String(args.a * args.b) }],
            }));
            const serverConfig = createSdkMcpServer({
                name: 'sdk-math',
            ...
            const q = query({
                prompt: 'First use sdk_add to calculate 10 + 5, then use sdk_multiply to multiply the result by 3. Give me the final answer.',
                options: {
                    ...SHARED_TEST_OPTIONS,
                    cwd: testDir,
                    debug: false,
    ...
    describe('SDK MCP Server Discovery', () => {
        it('should list SDK MCP servers in system init message', async () => {
            ...
            const echoTool = tool('echo', 'Echo a message', {
            ...
            try {
                ...
                const sdkServer = systemMessage.mcp_servers?.find((server) => server.name === 'sdk-echo');
    ...
    describe('SDK MCP Tool Error Handling', () => {
        it('should handle tool errors gracefully', async () => {
            ...
            const errorTool = tool('maybe_fail', 'A tool that may fail based on input', {
            ...
            const q = query({
                prompt: 'Use the maybe_fail tool with shouldFail set to true. Tell me what happens.',
                options: {
                    ...SHARED_TEST_OPTIONS,
                    cwd: testDir,
                    debug: false,
    ...
    describe('Async Tool Handlers', () => {
        it('should handle async tool handlers with delays', async () => {
            ...
            const delayedTool = tool('delayed_response', 'Returns a value after a delay', {
            ...
            const q = query({
                prompt: 'Use the delayed_response tool with delay=50 and value="test_async". Tell me the result.',
                options: {
                    ...SHARED_TEST_OPTIONS,
                    cwd: testDir,
                    debug: false,
            ...
            try {
                for await (const message of q) {
                    messages.push(message);
                    if (isSDKAssistantMessage(message)) {
                        const toolUseBlocks = findToolUseBlocks(message, 'delayed_response');
                        if (toolUseBlocks.length > 0) {
                            foundToolUse = true;
                        }
                        assistantText += extractText(message.message.content);
...
//# sourceMappingURL=sdk-mcp-server.test.js.map