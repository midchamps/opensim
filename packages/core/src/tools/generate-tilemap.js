...
class GenerateTilemapInvocation extends BaseToolInvocation {
    ...
    async execute(_signal) {
        ...
        if (this.params.maps && this.params.maps.length > 0) {
            ...
            return {
                llmContent: 'Error: Either maps[] array or legacy parameters must be provided.',
        ...
        for (const mapDef of maps) {
            try {
                const result = await this.generateSingleMap(mapDef, absoluteAssetsDir, assetPackPath, tileSize, TILESET_COLS, TOTAL_TILES, imageWidth, imageHeight, useAutoTiling, autoTileChars);
        ...
        const objectsInfo = allObjects.length > 0
        ...
        const objectLayerCode = allObjects.length > 0
            ? `
        // Read spawn points
...
`;
    ...
    async generateSingleMap(mapDef, absoluteAssetsDir, assetPackPath, tileSize, TILESET_COLS, TOTAL_TILES, imageWidth, imageHeight, useAutoTiling, autoTileChars) {
        ...
        let finalData;
        if (useAutoTiling) {
            const solidGrid = AutoTiler.createSolidGrid(rows, mapDef.legend, autoTileChars);
        ...
        await fs.writeFile(filePath, this.formatTilemapJson(tilemapData));
    ...
    formatTilemapJson(tilemapData) {
        ...
        json += `      "y": 0\n`;
        json += `    }`;
        const objectsLayer = tilemapData.layers.find((l) => l.type === 'objectgroup');
        if (objectsLayer) {
    ...
    async updateAssetPack(packPath, key, filename) {
        let pack = { gameAssets: { files: [] } };
        try {
...
export class GenerateTilemapTool extends BaseDeclarativeTool {
    config;
    static Name = ToolNames.GENERATE_TILEMAP;
    constructor(config) {
        super(GenerateTilemapTool.Name, ToolDisplayNames.GENERATE_TILEMAP, 'Converts ASCII layouts into Phaser Tilemap JSON with 47-tile blob autotiling (bitmasking).', Kind.Execute, {
            type: 'object',
            properties: {
                tileset_key: {
                    type: 'string',
                    description: 'Key of tileset image (must match generate_assets key).',
                },
                tile_size: {
                    type: 'number',
                    default: 64,
                ...
                tileset_grid_size: {
                    type: 'number',
                    default: 7,
                    description: 'Tileset grid. 7 for 47-tile blob (auto-tiling), 3 for 9-slice.',
                },
                auto_tiling: {
                    type: 'boolean',
                    description: 'Enable bitmasking auto-tiling (default: true for grid >= 7).',
                },
                auto_tile_chars: {
                    type: 'array',
                    items: { type: 'string' },
                ...
                mode: {
                    type: 'string',
                    enum: ['floor', 'walls'],
                    description: 'Dual-tileset mode for top-down games. "floor": auto-tiles walkable chars (everything except #). "walls": auto-tiles wall chars (#). When set, legend and auto_tile_chars are auto-computed.',
                },
                maps: {
                    type: 'array',
...
//# sourceMappingURL=generate-tilemap.js.map