 ...
 */
export const createTrigger = (scene, owner, x, y, width, height, origin = { x: 0.5, y: 0.5 }) => {
    const zoneWithOwner = scene.add
        .zone(x, y, width, height)
        .setOrigin(origin.x, origin.y);
    zoneWithOwner.owner = owner;
    scene.physics.add.existing(zoneWithOwner);
    zoneWithOwner.body.setAllowGravity(false);
    zoneWithOwner.body.setImmovable(true);
 ...
 */
export const updateMeleeTrigger = (character, meleeTrigger, facingDirection, attackRange, attackWidth) => {
    if (facingDirection !== 'up' &&
        facingDirection !== 'down' &&
        facingDirection !== 'left' &&
        facingDirection !== 'right') {
        throw new Error('updateMeleeTrigger: facingDirection must be up, down, left, or right');
    }
    const characterBody = character.body;
    const triggerBody = meleeTrigger.body;
    let triggerX = 0;
    let triggerY = 0;
 ...
 */
export function createProjectile(scene, x, y, textureKey, targetSize = PROJECTILE_SIZES.BULLET_SMALL, hasGravity = false, damage) {
    ...
    projectile.body.setAllowGravity(hasGravity);
    if (damage !== undefined) {
 ...
 */
export function createProjectileAtAngle(scene, x, y, textureKey, angle, speed, targetSize = PROJECTILE_SIZES.BULLET_SMALL, damage) {
    const projectile = createProjectile(scene, x, y, textureKey, targetSize, false, damage);
 ...
 */
export function scaleCollectible(sprite, targetSize = COLLECTIBLE_SIZES.COIN) {
