...
export class BackgroundRemovalService {
    ...
    async removeBackground(imageUrl) {
        console.log(`[BackgroundRemoval] Processing: ${imageUrl.substring(0, 50)}...`);
     ...
     */
    async removeBackgroundSafe(imageUrl) {
        console.log(`[BackgroundRemoval] Processing (safe): ${imageUrl.substring(0, 60)}...`);
        ...
        try {
            ...
            console.log(`[BackgroundRemoval] Downloaded image: ${imageBuffer.length} bytes`);
        }
        catch (fetchError) {
        ...
        try {
            ...
            console.warn(`[BackgroundRemoval] Background removal failed:`, bgRemovalError);
            console.warn(`[BackgroundRemoval] Returning original image (no bg removal)`);
     ...
     * Returns a Buffer with transparent background
     */
    async removeBackgroundFromBuffer(imageBuffer) {
        const backendToUse = this.backend;
        console.log(`[BackgroundRemoval] Processing buffer (${imageBuffer.length} bytes) with ${backendToUse}...`);
        if (backendToUse === 'rembg') {
            ...
            if (available) {
                ...
                console.warn(`[BackgroundRemoval] rembg not available, falling back to imgly`);
            }
        }
        // Use imgly (default)
     ...
     */
    async removeBackgroundWithImgly(imageBuffer) {
        try {
            ...
            const imglyPackagePath = require.resolve('@imgly/background-removal-node');
            const imglyDistPath = path.dirname(imglyPackagePath);
            const localModelPath = `file://${imglyDistPath}/`;
            console.log(`[BackgroundRemoval] Using imgly model path: ${localModelPath}`);
            const config = {
            ...
            console.log(`[BackgroundRemoval] Success (imgly), output size: ${buffer.length} bytes`);
            return buffer;
        }
        catch (error) {
     ...
     */
    async removeBackgroundWithRembg(imageBuffer) {
...
`;
            await new Promise((resolve, reject) => {
                const proc = spawn(pythonPath, ['-c', pythonScript, inputPath, outputPath], {
            ...
            console.log(`[BackgroundRemoval] Success (rembg/u2net), output size: ${resultBuffer.length} bytes`);
            return resultBuffer;
        }
        finally {
            // Cleanup temp files
 ...
 */
export async function removeBackgroundFromUrl(imageUrl, projectRoot, fallbackOnError = true) {
