...
describe('QwenOAuthProgress', () => {
    ...
    const createMockDeviceAuth = (overrides = {}) => ({
    ...
    const renderComponent = (props = {}) => render(_jsx(QwenOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, ...props }));
    beforeEach(() => {
        vi.clearAllMocks();
        vi.useFakeTimers();
    ...
    describe('Authenticated state (with deviceAuth)', () => {
        ...
        it('should display correct URL in Static component when QR code is generated', async () => {
            ...
            rerender(_jsx(QwenOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: customAuth }));
            expect(lastFrame()).toContain('https://custom.com/auth?code=XYZ789');
        });
        it('should format time correctly', () => {
            const deviceAuthWithCustomTime = {
                ...mockDeviceAuth,
                expires_in: 125, // 2 minutes and 5 seconds
            };
            const { lastFrame } = render(_jsx(QwenOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: deviceAuthWithCustomTime }));
            const output = lastFrame();
    ...
    describe('Timer functionality', () => {
        it('should countdown and call onTimeout when timer expires', async () => {
            const deviceAuthWithShortTime = {
                ...mockDeviceAuth,
                expires_in: 2, // 2 seconds
            };
            const { rerender } = render(_jsx(QwenOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: deviceAuthWithShortTime }));
            // Advance timer by 1 second
            vi.advanceTimersByTime(1000);
            rerender(_jsx(QwenOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: deviceAuthWithShortTime }));
        ...
        it('should update time remaining display', async () => {
            const { lastFrame, rerender } = render(_jsx(QwenOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: mockDeviceAuth }));
            // Initial time should be 5:00
            expect(lastFrame()).toContain('Time remaining: 5:00');
            // Advance by 1 second
            vi.advanceTimersByTime(1000);
            rerender(_jsx(QwenOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: mockDeviceAuth }));
        ...
        it('should use default 300 second timeout when deviceAuth is null', () => {
            const { lastFrame } = render(_jsx(QwenOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel }));
    ...
    describe('QR Code functionality', () => {
        it('should generate QR code when deviceAuth is provided', async () => {
            ...
            render(_jsx(QwenOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: mockDeviceAuth }));
            expect(mockGenerate).toHaveBeenCalledWith(mockDeviceAuth.verification_uri_complete, { small: true }, expect.any(Function));
        });
        ...
        it('should handle QR code generation errors gracefully', async () => {
            ...
            const { lastFrame } = render(_jsx(QwenOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: mockDeviceAuth }));
        ...
        it('should not generate QR code when deviceAuth is null', async () => {
            const qrcode = await import('qrcode-terminal');
            const mockGenerate = vi.mocked(qrcode.default.generate);
            render(_jsx(QwenOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel }));
    ...
    describe('Props changes', () => {
        it('should display initial timer value from deviceAuth', () => {
            const deviceAuthWith10Min = {
                ...mockDeviceAuth,
                expires_in: 600, // 10 minutes
            };
            const { lastFrame } = render(_jsx(QwenOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: deviceAuthWith10Min }));
            expect(lastFrame()).toContain('Time remaining: 10:00');
        });
        it('should reset to loading state when deviceAuth becomes null', () => {
            const { rerender, lastFrame } = render(_jsx(QwenOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: mockDeviceAuth }));
            // Initially shows waiting for authorization
            expect(lastFrame()).toContain('Waiting for authorization');
            rerender(_jsx(QwenOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel }));
    ...
    describe('Timeout state', () => {
        it('should render timeout state when authStatus is timeout', () => {
            ...
            expect(output).toContain('Press any key to return to authentication type selection.');
        });
        it('should render default timeout message when no authMessage provided', () => {
            ...
            const output = lastFrame();
            expect(output).toContain('Qwen OAuth Authentication Timeout');
            expect(output).toContain('OAuth token expired (over 300 seconds). Please select authentication method again.');
        });
...
//# sourceMappingURL=QwenOAuthProgress.test.js.map