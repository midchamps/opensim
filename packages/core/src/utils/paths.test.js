...
import { escapePath, resolvePath, validatePath, resolveAndValidatePath, unescapePath, isSubpath, } from './paths.js';
function createConfigStub({ targetDir, allowedDirectories, }) {
    const resolvedTargetDir = path.resolve(targetDir);
    const resolvedDirectories = allowedDirectories.map((dir) => path.resolve(dir));
    const workspaceContext = {
        isPathWithinWorkspace(testPath) {
...
describe('validatePath', () => {
    ...
    let config;
    beforeAll(() => {
        workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'validate-path-test-'));
    ...
    it('throws when path is outside workspace boundaries', () => {
        ...
        expect(() => validatePath(config, outsidePath)).toThrowError(/Path is not within workspace/);
    });
    it('throws when path does not exist', () => {
        const nonExistentPath = path.join(workspaceRoot, 'nonexistent');
        expect(() => validatePath(config, nonExistentPath)).toThrowError(/Path does not exist:/);
    });
    it('throws when path is a file, not a directory (default behavior)', () => {
        const filePath = path.join(workspaceRoot, 'test-file.txt');
        fs.writeFileSync(filePath, 'content');
        try {
            expect(() => validatePath(config, filePath)).toThrowError(/Path is not a directory/);
        }
        finally {
            fs.rmSync(filePath);
        }
    ...
    it('allows files when allowFiles option is true', () => {
        ...
        fs.writeFileSync(filePath, 'content');
        try {
            expect(() => validatePath(config, filePath, { allowFiles: true })).not.toThrow();
        }
        finally {
            fs.rmSync(filePath);
        }
...
describe('resolveAndValidatePath', () => {
    ...
    it('allows absolute paths that are permitted by the workspace context', () => {
        const extraDir = fs.mkdtempSync(path.join(os.tmpdir(), 'resolve-and-validate-extra-'));
        try {
            const configWithExtra = createConfigStub({
                targetDir: workspaceRoot,
                allowedDirectories: [workspaceRoot, extraDir],
    ...
    it('expands tilde-prefixed paths using the home directory', () => {
        const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'resolve-and-validate-home-'));
        ...
        try {
            ...
            expect(resolveAndValidatePath(configWithHome, '~/project')).toBe(homeSubdir);
    ...
    it('throws when the path resolves outside of the workspace', () => {
        expect(() => resolveAndValidatePath(config, '../outside')).toThrowError(/Path is not within workspace/);
    });
    it('throws when the path does not exist', () => {
        expect(() => resolveAndValidatePath(config, 'missing')).toThrowError(/Path does not exist:/);
    });
    it('throws when the path points to a file (default behavior)', () => {
        const filePath = path.join(workspaceRoot, 'file.txt');
        fs.writeFileSync(filePath, 'content');
        try {
            expect(() => resolveAndValidatePath(config, 'file.txt')).toThrowError(`Path is not a directory: ${filePath}`);
        }
        finally {
            fs.rmSync(filePath);
        }
