...
describe('extension tests', () => {
    ...
    beforeEach(() => {
        tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-code-test-home-'));
        tempWorkspaceDir = fs.mkdtempSync(path.join(tempHomeDir, 'qwen-code-test-workspace-'));
    ...
    describe('annotateActiveExtensions', () => {
        ...
        it('should mark all extensions as active if no enabled extensions are provided', () => {
            const activeExtensions = annotateActiveExtensions(extensions, '/path/to/workspace', new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir()));
            expect(activeExtensions).toHaveLength(3);
            expect(activeExtensions.every((e) => e.isActive)).toBe(true);
        });
        it('should mark only the enabled extensions as active', () => {
            const activeExtensions = annotateActiveExtensions(extensions, '/path/to/workspace', new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), ['ext1', 'ext3']));
            expect(activeExtensions).toHaveLength(3);
        ...
        it('should mark all extensions as inactive when "none" is provided', () => {
            const activeExtensions = annotateActiveExtensions(extensions, '/path/to/workspace', new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), ['none']));
            expect(activeExtensions).toHaveLength(3);
            expect(activeExtensions.every((e) => !e.isActive)).toBe(true);
        });
        it('should handle case-insensitivity', () => {
            const activeExtensions = annotateActiveExtensions(extensions, '/path/to/workspace', new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), ['EXT1']));
            expect(activeExtensions.find((e) => e.name === 'ext1')?.isActive).toBe(true);
        });
        it('should log an error for unknown extensions', () => {
            const consoleSpy = vi
                .spyOn(console, 'error')
                .mockImplementation(() => { });
            annotateActiveExtensions(extensions, '/path/to/workspace', new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), ['ext4']));
            expect(consoleSpy).toHaveBeenCalledWith('Extension not found: ext4');
            consoleSpy.mockRestore();
        });
        describe('autoUpdate', () => {
            it('should be false if autoUpdate is not set in install metadata', () => {
                const activeExtensions = annotateActiveExtensions(extensions, tempHomeDir, new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir()));
            ...
            it('should be true if autoUpdate is true in install metadata', () => {
                ...
                const activeExtensions = annotateActiveExtensions(extensionsWithAutoUpdate, tempHomeDir, new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir()));
    ...
    describe('installExtension', () => {
        it('should install an extension from a local path', async () => {
            ...
            await installExtension({ source: sourceExtDir, type: 'local' }, async (_) => true);
        ...
        it('should throw an error if the extension already exists', async () => {
            ...
            await installExtension({ source: sourceExtDir, type: 'local' }, async (_) => true);
            await expect(installExtension({ source: sourceExtDir, type: 'local' }, async (_) => true)).rejects.toThrow('Extension "my-local-extension" is already installed. Please uninstall it first.');
        });
        it('should throw an error and cleanup if qwen-extension.json is missing', async () => {
            ...
            await expect(installExtension({ source: sourceExtDir, type: 'local' }, async (_) => true)).rejects.toThrow(`Configuration file not found at ${configPath}`);
        ...
        it('should throw an error for invalid JSON in qwen-extension.json', async () => {
            ...
            await expect(installExtension({ source: sourceExtDir, type: 'local' }, async (_) => true)).rejects.toThrow(new RegExp(`^Failed to load extension config from ${configPath.replace(/\\/g, '\\\\')}`));
        });
        it('should throw an error for missing name in qwen-extension.json', async () => {
            ...
            await expect(installExtension({ source: sourceExtDir, type: 'local' }, async (_) => true)).rejects.toThrow(`Invalid configuration in ${configPath}: missing "name"`);
        });
        it('should install an extension from a git URL', async () => {
            ...
            await installExtension({ source: gitUrl, type: 'git' }, async (_) => true);
            expect(fs.existsSync(targetExtDir)).toBe(true);
            expect(fs.existsSync(metadataPath)).toBe(true);
            const metadata = JSON.parse(fs.readFileSync(metadataPath, 'utf-8'));
            expect(metadata).toEqual({
        ...
        it('should install a linked extension', async () => {
            ...
            await installExtension({ source: sourceExtDir, type: 'link' }, async (_) => true);
            expect(fs.existsSync(targetExtDir)).toBe(true);
            expect(fs.existsSync(metadataPath)).toBe(true);
        ...
        it('should show users information on their mcp server when installing', async () => {
            ...
            mockQuestion.mockImplementation((_query, callback) => callback('y'));
            await expect(installExtension({ source: sourceExtDir, type: 'local' }, requestConsentNonInteractive)).resolves.toBe('my-local-extension');
            expect(consoleInfoSpy).toHaveBeenCalledWith(`Installing extension "my-local-extension".
...
This extension will run the following MCP servers:
  ...
  * test-server-2 (remote): https://google.com`);
        ...
        it('should continue installation if user accepts prompt for local extension with mcp servers', async () => {
            ...
            mockQuestion.mockImplementation((_query, callback) => callback('y'));
            await expect(installExtension({ source: sourceExtDir, type: 'local' }, requestConsentNonInteractive)).resolves.toBe('my-local-extension');
            expect(mockQuestion).toHaveBeenCalledWith(expect.stringContaining('Do you want to continue? [Y/n]: '), expect.any(Function));
        });
        it('should cancel installation if user declines prompt for local extension with mcp servers', async () => {
            ...
            await expect(installExtension({ source: sourceExtDir, type: 'local' }, requestConsentNonInteractive)).rejects.toThrow('Installation cancelled for "my-local-extension".');
            expect(mockQuestion).toHaveBeenCalledWith(expect.stringContaining('Do you want to continue? [Y/n]: '), expect.any(Function));
        });
        ...
        it('should ignore consent flow if not required', async () => {
            ...
            await expect(installExtension({ source: sourceExtDir, type: 'local' }, mockRequestConsent, process.cwd(), 
        ...
        it('should throw an error for invalid extension names', async () => {
            ...
            await expect(installExtension({ source: sourceExtDir, type: 'local' })).rejects.toThrow('Invalid extension name: "bad_name"');
        });
    });
    describe('uninstallExtension', () => {
        ...
        it('should uninstall an extension by name and retain existing extensions', async () => {
            ...
            await uninstallExtension('my-local-extension');
            expect(fs.existsSync(sourceExtDir)).toBe(false);
            expect(loadExtensions(new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir()))).toHaveLength(1);
            expect(fs.existsSync(otherExtDir)).toBe(true);
        });
        it('should throw an error if the extension does not exist', async () => {
            await expect(uninstallExtension('nonexistent-extension')).rejects.toThrow('Extension not found.');
        });
        ...
        it('should fail to uninstall by URL if an extension has no install metadata', async () => {
            ...
            await expect(uninstallExtension('https://github.com/google/no-metadata-extension')).rejects.toThrow('Extension not found.');
        });
    });
    describe('performWorkspaceExtensionMigration', () => {
        let workspaceExtensionsDir;
        beforeEach(() => {
            workspaceExtensionsDir = path.join(tempWorkspaceDir, EXTENSIONS_DIRECTORY_NAME);
        ...
        it('should install the extensions in the user directory', async () => {
            ...
            const failed = await performWorkspaceExtensionMigration(extensionsToMigrate, async (_) => true);
        ...
        it('should return the names of failed installations', async () => {
            ...
            const failed = await performWorkspaceExtensionMigration(extensions, async (_) => true);
