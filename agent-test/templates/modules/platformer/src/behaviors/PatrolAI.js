...
export class PatrolAI extends BaseBehavior {
    ...
    update() {
        ...
        // Apply movement
        const velocityX = this.facingDirection === 'right' ? this.speed : -this.speed;
     ...
     */
    checkCliff(owner, scene) {
        const groundLayer = scene.groundLayer;
        if (!groundLayer)
            return false;
        const body = owner.body;
        // Calculate check position
        const checkX = this.facingDirection === 'right'
