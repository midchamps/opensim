...
import { LIBRARY_JSON_PATH, FAMILIES_PATH, META_TEMPLATE_PATH, OUTPUT_PATH, } from './config.js';
// =============================================================================
...
export async function initializeLibrary() {
    ...
    console.log(`[LibraryManager] Initialized empty library at ${LIBRARY_JSON_PATH}`);
    return library;
}
/**
 ...
 */
export async function loadOrInitLibrary() {
...
export async function saveLibrary(library) {
    ...
    await fs.writeFile(LIBRARY_JSON_PATH, JSON.stringify(manifest, null, 2), 'utf-8');
 ...
 */
async function saveFamilyFiles(family) {
    ...
    await fs.writeFile(path.join(familyDir, 'family.json'), JSON.stringify(metadata, null, 2), 'utf-8');
 ...
 */
export function getLibrarySummary(library) {
    const lines = [];
    ...
    if (library.families.length === 0) {
        lines.push('  (no families yet -- run evolve to start accumulating experience)');
    }
    for (const family of library.families) {
        lines.push(`  [${family.archetype}] ${family.id}`);
        lines.push(`    Stability: ${(family.stability * 100).toFixed(0)}%`);
        lines.push(`    Contributing projects: ${family.contributingProjects.length}`);
    ...
    if (library.evolutionLog.length > 0) {
        ...
        for (const entry of recent) {
            lines.push(`  [${entry.timestamp}] ${entry.action} -> ${entry.familyId} ` +
 ...
 */
export function findFamily(library, archetype) {
    return library.families.find((f) => f.archetype === archetype);
}
//# sourceMappingURL=library-manager.js.map