...
export class AutoTiler {
    ...
    height;
    solidGrid;
    constructor(width, height, solidGrid) {
        this.width = width;
        this.height = height;
    ...
    getTileId(x, y) {
        ...
        const tileIndex = this.computeTileIndex(n, e, s, w, hasNW, hasNE, hasSW, hasSE, edgeCount);
     ...
     */
    computeTileIndex(n, e, s, w, hasNW, hasNE, hasSW, hasSE, edgeCount) {
        // === NO NEIGHBORS (Island) ===
        ...
        if (edgeCount === 1) {
            if (n)
                return 45; // South edge (open at top, closed at bottom)
            if (s)
                return 3; // North edge (open at bottom, closed at top)
            if (w)
                return 6; // Right edge (use TR corner as closest match)
            if (e)
                return 0; // Left edge (use TL corner as closest match)
        }
        // === TWO OPPOSITE NEIGHBORS (Corridors) ===
        // Use center tile for corridors
        if (edgeCount === 2) {
            if (n && s && !e && !w)
                return 17; // Vertical corridor
            if (e && w && !n && !s)
                return 17; // Horizontal corridor
        }
        // === TWO ADJACENT NEIGHBORS (Outer Corners) ===
        if (edgeCount === 2) {
            if (n && e)
                return 42; // BL corner (open to NE)
            if (n && w)
                return 48; // BR corner (open to NW)
            if (s && e)
                return 0; // TL corner (open to SE)
            if (s && w)
                return 6; // TR corner (open to SW)
        }
        // === THREE NEIGHBORS (T-junctions / Edge pieces) ===
        if (edgeCount === 3) {
            if (!n)
                return 3; // Top edge (no N neighbor)
            if (!s)
                return 45; // Bottom edge (no S neighbor)
            if (!w)
                return 27; // Right edge (no W neighbor)
            if (!e)
                return 21; // Left edge (no E neighbor)
        }
        // === FOUR NEIGHBORS (Center or Inner Corners) ===
        if (edgeCount === 4) {
            // Count missing corners
            const missingCorners = (!hasNW ? 1 : 0) +
                (!hasNE ? 1 : 0) +
                (!hasSW ? 1 : 0) +
                (!hasSE ? 1 : 0);
            ...
            if (missingCorners === 1) {
                if (!hasNW)
                    return 33; // Inner corner NW
                if (!hasNE)
                    return 34; // Inner corner NE
                if (!hasSW)
                    return 40; // Inner corner SW
                if (!hasSE)
                    return 41; // Inner corner SE
            }
            // Two missing corners
            if (missingCorners === 2) {
                ...
                if (!hasNW && !hasNE)
                    return 3; // Top edge
                if (!hasSW && !hasSE)
                    return 45; // Bottom edge
                if (!hasNW && !hasSW)
                    return 21; // Left edge
                if (!hasNE && !hasSE)
                    return 27; // Right edge
            }
            // Three or four missing corners = use edges
            if (missingCorners >= 3) {
                // Find which corner IS present and use opposite edge
                if (hasNW)
                    return 27; // Only NW present, use right edge
                if (hasNE)
                    return 21; // Only NE present, use left edge
                if (hasSW)
                    return 3; // Only SW present, use top edge
                if (hasSE)
                    return 45; // Only SE present, use bottom edge
     ...
     */
    static createSolidGrid(layout, legend, autoTileChars) {
        ...
        for (let y = 0; y < height; y++) {
            ...
            for (let x = 0; x < width; x++) {
                ...
                const isSolid = autoTileChars.includes(char) ||
...
//# sourceMappingURL=auto-tiler.js.map